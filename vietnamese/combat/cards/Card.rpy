# : Translation updated at 2021-12-24 07:09

translate vietnamese strings:

    # game/combat/cards/Card.rpy:21
    old "Sorry, you have faulty card in inventory. Card -> "
    new "Xin lỗi, bạn có thẻ bị lỗi trong kho. Thẻ -> " # TL

    # game/combat/cards/Card.rpy:53
    old "Error generating card description!"
    new "Lỗi khi tạo mô tả thẻ!" # TL

    # game/combat/cards/Card.rpy:54
    old "\n Missing generate_description() method."
    new "\n Thiếu phương thức tạo mô tả ()." # TL

    # game/combat/cards/Card.rpy:78
    old "There is no tip with id: "
    new "Không có mẹo nào với id: " # TL # Victorius - AI

    # game/combat/cards/Card.rpy:78
    old ", card: "
    new ", bài: " # TL

    # game/combat/cards/Card.rpy:354
    old "Error playing card! \n Missing play() method."
    new "Lỗi khi chơi bài! \n Thiếu phương thức play()." # TL # Victorius - AI
# : Translation updated at 2022-03-05 18:23

translate vietnamese strings:

    # game/combat/cards/Card.rpy:246
    old " true"
    new " Sát thương chuẩn" # TL

    # game/combat/cards/Card.rpy:249
    old " unblockable"
    new " không thể chặn đòn" # TL

    # game/combat/cards/Card.rpy:252
    old " unavoidable"
    new " không thể né đòn" # TL
