# : Translation updated at 2021-12-24 07:09

# game/places/wanderers/prostitutes/amanda.rpy:16
translate vietnamese w_booth_amanda_2c482652:

    # amanda "Hello, [me_amanda!t]. I'm at your service."
    amanda "Chào [me_amanda!t]. Em sẵn sàng phục vụ [me_amanda!t] rồi đây." # TL

# game/places/wanderers/prostitutes/amanda.rpy:27
translate vietnamese w_booth_amanda_menu1_0cb75142:

    # amanda "Of course, [me_amanda]."
    amanda "Tất nhiên rồi, [me_amanda!t]." # TL

# game/places/wanderers/prostitutes/amanda.rpy:32
translate vietnamese w_booth_amanda_menu1_04f4733b:

    # amanda "You won't get refund."
    amanda "[me_amanda!t] sẽ không được hoàn tiền đâu." # TL

# game/places/wanderers/prostitutes/amanda.rpy:33
translate vietnamese w_booth_amanda_menu1_3cdbfa0d:

    # me "I see..."
    me "Anh hiểu rồi..." # TL

# game/places/wanderers/prostitutes/amanda.rpy:37
translate vietnamese w_booth_amanda_menu1_cda50697:

    # amanda "Take care, [me_amanda!t]."
    amanda "Bảo trọng nha, [me_amanda!t]." # TL

# game/places/wanderers/prostitutes/amanda.rpy:42
translate vietnamese w_booth_amanda_menu1_0aa909f6:

    # amanda "Thank you for choosing to stay with me, [me_amanda]."
    amanda "Cảm ơn vì đã chọn ở lại với em, [me_amanda!t]." # TL

# game/places/wanderers/prostitutes/amanda.rpy:51
translate vietnamese w_booth_amanda_end_72823134:

    # "[amanda] left."
    "[amanda] Rời đi." # TL

# game/places/wanderers/prostitutes/amanda.rpy:53
translate vietnamese w_booth_amanda_end_b1d0189a:

    # th "It was quite nice."
    th "Cũng khá tuyệt đấy." # TL

# game/places/wanderers/prostitutes/amanda.rpy:54
translate vietnamese w_booth_amanda_end_299cdfad:

    # th "Time for me to go, though."
    th "Cũng đến lúc mình phải đi rồi." # TL

# game/places/wanderers/prostitutes/amanda.rpy:68
translate vietnamese w_booth_amanda_dt_5082cb2d:

    # "[amanda] doesn't waste time, and starts to blow you right away."
    "[amanda] không lãng phí thêm một giây và bắt đầu thổi kèn cho bạn ngay lập tức." # TL

# game/places/wanderers/prostitutes/amanda.rpy:78
translate vietnamese w_booth_amanda_dt_slow_caad1bd4:

    # me "Damn, girl, enthusiastic aren't you."
    me "Nhiệt tình đấy." # TL

# game/places/wanderers/prostitutes/amanda.rpy:79
translate vietnamese w_booth_amanda_dt_slow_0b46abb9:

    # amanda "Mhmm."
    amanda "Mhmm." # TL # Victorius - AI

# game/places/wanderers/prostitutes/amanda.rpy:80
translate vietnamese w_booth_amanda_dt_slow_e54fec88:

    # me "Ohh... More, babe."
    me "Ohh... nữa đi baby." # TL

# game/places/wanderers/prostitutes/amanda.rpy:81
translate vietnamese w_booth_amanda_dt_slow_237b88e9:

    # amanda "Mghm."
    amanda "Mghm." # TL # Victorius - AI

# game/places/wanderers/prostitutes/amanda.rpy:82
translate vietnamese w_booth_amanda_dt_slow_832a3332:

    # me "Don't stop."
    me "Đừng dừng lại." # TL

# game/places/wanderers/prostitutes/amanda.rpy:98
translate vietnamese w_booth_amanda_dt_fast_9a2e227d:

    # me "Ahh. That's nice."
    me "Ahh. tốt đấy." # TL

# game/places/wanderers/prostitutes/amanda.rpy:99
translate vietnamese w_booth_amanda_dt_fast_237b88e9:

    # amanda "Mghm."
    amanda "Mghm." # TL # Victorius - AI

# game/places/wanderers/prostitutes/amanda.rpy:100
translate vietnamese w_booth_amanda_dt_fast_5919fd22:

    # me "Yeah, just like that."
    me "Yeah, cứ như vậy đi." # TL

# game/places/wanderers/prostitutes/amanda.rpy:101
translate vietnamese w_booth_amanda_dt_fast_2d2dd2c2:

    # "Her warm mouth keeps sucking on your dick like it's made of candy."
    "Cái miệng ấm áp của cô ấy tiếp tục mút cu bạn như thể nó được làm từ kẹo." # TL

# game/places/wanderers/prostitutes/amanda.rpy:102
translate vietnamese w_booth_amanda_dt_fast_aba72b93:

    # me "Ahh! You know how to use your tongue.."
    me "Ahh! Em biết cách để dùng lưỡi của mình đấy..." # TL

# game/places/wanderers/prostitutes/amanda.rpy:103
translate vietnamese w_booth_amanda_dt_fast_c722aaf5:

    # amanda "Mmmghm."
    amanda "Mmmghm." # TL # Victorius - AI

# game/places/wanderers/prostitutes/amanda.rpy:122
translate vietnamese w_booth_amanda_dt_before_5115f097:

    # amanda "Mgghm!"
    amanda "Mgghm!" # TL # Victorius - AI

# game/places/wanderers/prostitutes/amanda.rpy:123
translate vietnamese w_booth_amanda_dt_before_993b1122:

    # "[amanda] takes your dick really deep. Her gag reflex must be nonexistent."
    "[amanda] đưa con cặc của bạn vào sâu hơn. Cô ấy hẳn không biết cảm giác buồn nôn là gì." # TL

# game/places/wanderers/prostitutes/amanda.rpy:124
translate vietnamese w_booth_amanda_dt_before_3f6eeae6:

    # me "Nice! Ahh!"
    me "Tuyệt lắm! Ahh!" # TL

# game/places/wanderers/prostitutes/amanda.rpy:125
translate vietnamese w_booth_amanda_dt_before_f86e4ecc:

    # amanda "Mmmm!"
    amanda "Mmmm!" # TL # Victorius - AI

# game/places/wanderers/prostitutes/amanda.rpy:126
translate vietnamese w_booth_amanda_dt_before_b353ee2a:

    # "Your dick is throbbing, your breath short. You can cum anytime."
    "Con cặc của bạn đang giật lên, hơi thở của bạn trở nên gấp gáp. Bạn có thể xuất tinh bất cứ lúc nào." # TL

# game/places/wanderers/prostitutes/amanda.rpy:148
translate vietnamese w_booth_amanda_dt_before2_5115f097:

    # amanda "Mgghm!"
    amanda "Mgghm!" # TL # Victorius - AI

# game/places/wanderers/prostitutes/amanda.rpy:149
translate vietnamese w_booth_amanda_dt_before2_993b1122:

    # "[amanda] takes your dick really deep. Her gag reflex must be nonexistent."
    "[amanda] đưa con cặc của bạn vào sâu hơn. Cô ấy hẳn phải không biết cảm giác buồn nôn là gì." # TL

# game/places/wanderers/prostitutes/amanda.rpy:150
translate vietnamese w_booth_amanda_dt_before2_3f6eeae6:

    # me "Nice! Ahh!"
    me "Tuyệt lắm! Ahh!" # TL

# game/places/wanderers/prostitutes/amanda.rpy:151
translate vietnamese w_booth_amanda_dt_before2_f86e4ecc:

    # amanda "Mmmm!"
    amanda "Mmmm!" # TL # Victorius - AI

# game/places/wanderers/prostitutes/amanda.rpy:152
translate vietnamese w_booth_amanda_dt_before2_b353ee2a:

    # "Your dick is throbbing, your breath short. You can cum anytime."
    "Con cặc của bạn đang giật lên, hơi thở gấp gáp. Bạn có thể xuất tinh bất cứ lúc nào." # TL

# game/places/wanderers/prostitutes/amanda.rpy:169
translate vietnamese w_booth_amanda_dt_cum_372a4323:

    # "You're cumming!"
    "Bạn đang ra!" # TL

# game/places/wanderers/prostitutes/amanda.rpy:170
translate vietnamese w_booth_amanda_dt_cum_3217e8b9:

    # me "Ahh!!"
    me "Ahh!!" # TL # Victorius - AI

# game/places/wanderers/prostitutes/amanda.rpy:171
translate vietnamese w_booth_amanda_dt_cum_83b8d24c:

    # amanda "Ahmmm!!"
    amanda "Ahmmm!!" # TL # Victorius - AI

# game/places/wanderers/prostitutes/amanda.rpy:179
translate vietnamese w_booth_amanda_dt_cum_00e07095:

    # amanda "I hope you liked your blowjob, [me_amanda]."
    amanda "Em mong là [me_amanda!t] thích." # TL

# game/places/wanderers/prostitutes/amanda.rpy:180
translate vietnamese w_booth_amanda_dt_cum_6c57e085:

    # me "I did, you're quite the talent."
    me "Rất hài lòng luôn, em khá là tài năng đấy." # TL

# game/places/wanderers/prostitutes/amanda.rpy:181
translate vietnamese w_booth_amanda_dt_cum_8d3d317d:

    # amanda "Thank you. I need to go, though. I hope you'll choose me again."
    amanda "Cảm ơn nha. Nhưng em cần phải đi rồi. Em mong là [me_amanda!t] sẽ chọn em lần nữa." # TL

# game/places/wanderers/prostitutes/amanda.rpy:182
translate vietnamese w_booth_amanda_dt_cum_fca811a5:

    # me "Sure."
    me "Được thôi." # TL

# game/places/wanderers/prostitutes/amanda.rpy:183
translate vietnamese w_booth_amanda_dt_cum_cda50697:

    # amanda "Take care, [me_amanda]."
    amanda "Bảo trọng nha [me_amanda!t]." # TL

# game/places/wanderers/prostitutes/amanda.rpy:184
translate vietnamese w_booth_amanda_dt_cum_9370fd7a:

    # me "See ya."
    me "Gặp em sau." # TL

translate vietnamese strings:

    # game/places/wanderers/prostitutes/amanda.rpy:18
    old "Deepthroat"
    new "Bún cua" # TL

    # game/places/wanderers/prostitutes/amanda.rpy:18
    old "Call me.."
    new "Gọi anh là..." # TL

    # game/places/wanderers/prostitutes/amanda.rpy:18
    old "I've changed my mind"
    new "Anh đổi ý rồi" # TL

    # game/places/wanderers/prostitutes/amanda.rpy:35
    old "I'm sure"
    new "Anh chắc mà" # TL

# : Translation updated at 2022-02-16 09:38

translate vietnamese strings:

    # game/places/wanderers/prostitutes/amanda.rpy:2
    old "sir"
    new "Anh" # TL

# : Translation updated at 2022-03-11 07:54

translate vietnamese strings:

    # game/places/wanderers/prostitutes/amanda.rpy:24
    old "I want you to call me.."
    new "Anh muốn em gọi anh là..." # TL
