# TODO: Translation updated at 2021-12-24 07:09

# game/places/wanderers/brothel.rpy:39
translate vietnamese wanderers_brothel_753a405b:

    # madam_hof "Welcome again, [me]. How can I help you today?"
    madam_hof "Chào mừng một lần nữa, [me]. Hôm nay tôi giúp gì được cho chú em?"

# game/places/wanderers/brothel.rpy:42
translate vietnamese wanderers_brothel_5e87e989:

    # madam_hof "It seems you've took a liking to our girls, I'll give you 5% discount on all future visits."
    madam_hof "Có vẻ như chú em đã thích các cô gái của tôi, tôi sẽ giảm giá cho chú em 5% cho tất cả các lần ghé thăm trong tương lai."

# game/places/wanderers/brothel.rpy:43
translate vietnamese wanderers_brothel_12cf7b69:

    # me "Thanks."
    me "Cảm ơn."

# game/places/wanderers/brothel.rpy:47
translate vietnamese wanderers_brothel_ce34d8e9:

    # madam_hof "It seems I was right about you being a regular. Let's increase your discount to 15% instead."
    madam_hof "Có vẻ như tôi đã đúng khi chú em là một người thường xuyên. Thay vào đó, hãy tăng giảm giá của chú em lên 15\%."

# game/places/wanderers/brothel.rpy:48
translate vietnamese wanderers_brothel_48ce703f:

    # me "Yeah, I was getting by pretty often. Thanks, I'll put it to a good use."
    me "Yeah, Tôi đã đến khá thường xuyên. Cảm ơn, tôi sẽ sử dụng nó tốt."

# game/places/wanderers/brothel.rpy:52
translate vietnamese wanderers_brothel_427075a6:

    # madam_hof "Our girls keep talking about you, you're quite a stallion. Let me give you 25% discount."
    madam_hof "Các cô gái của tôi tiếp tục nói về chú em, chú em đúng là một con ngựa giống. Hãy để tôi giảm giá cho chú em 25\%."

# game/places/wanderers/brothel.rpy:53
translate vietnamese wanderers_brothel_1522ce89:

    # me "Yeah, your girls know what's good. Thanks for a discount."
    me "Yeah, các cô gái của chị biết điều gì tốt. Cảm ơn vì đã giảm giá."

# game/places/wanderers/brothel.rpy:69
translate vietnamese wanderers_brothel_menu1_2b07d2b5:

    # madam_hof "New here, aren't you boy?"
    madam_hof "Mới đến, phải không cậu bé?"

# game/places/wanderers/brothel.rpy:70
translate vietnamese wanderers_brothel_menu1_ac0e68b1:

    # me "Yeah, is it so obvious?"
    me "Yeah, Không phải nó quá rõ ràng à?"

# game/places/wanderers/brothel.rpy:73
translate vietnamese wanderers_brothel_menu1_963b6575:

    # madam_hof "You look too honest to be local, and too clean."
    madam_hof "Trông chú em quá trung thực để trở thành người địa phương và quá sạch sẽ."

# game/places/wanderers/brothel.rpy:74
translate vietnamese wanderers_brothel_menu1_3820771a:

    # me "..."
    me "..."

# game/places/wanderers/brothel.rpy:75
translate vietnamese wanderers_brothel_menu1_c02b3a89:

    # me "So what do you do here?"
    me "Vậy chị làm gì ở đây?"

# game/places/wanderers/brothel.rpy:78
translate vietnamese wanderers_brothel_menu1_d3f62b62:

    # madam_hof "Oh, boy.. It's written everywhere."
    madam_hof "Ôi, cậu bé .. Nó được viết ở khắp mọi nơi."

# game/places/wanderers/brothel.rpy:79
translate vietnamese wanderers_brothel_menu1_5cc8a5cd:

    # me "So it is."
    me "Nên nó là."

# game/places/wanderers/brothel.rpy:82
translate vietnamese wanderers_brothel_menu1_e446f0c3:

    # madam_hof "Are you here for some action?"
    madam_hof "chú em có ở đây để hành động không?"

# game/places/wanderers/brothel.rpy:83
translate vietnamese wanderers_brothel_menu1_58db956b:

    # me "I'm not sure.."
    me "tôi không chắc.."

# game/places/wanderers/brothel.rpy:87
translate vietnamese wanderers_brothel_menu1_bac46347:

    # madam_hof "Maybe we should start over, I'm Madam Hof."
    madam_hof "Có lẽ chúng ta nên bắt đầu lại, tôi là Madam Hof."

# game/places/wanderers/brothel.rpy:88
translate vietnamese wanderers_brothel_menu1_fb30c850:

    # me "[me]."
    me "[me]."

# game/places/wanderers/brothel.rpy:91
translate vietnamese wanderers_brothel_menu1_f7f0483e:

    # madam_hof "Nice to meet you, [me]. You look like someone that could become a regular." id wanderers_brothel_menu1_f7f0483e
    madam_hof "Rất vui được gặp chú em, [me]. chú em trông giống như một người có thể trở thành khách quen." id wanderers_brothel_menu1_f7f0483e

# game/places/wanderers/brothel.rpy:92
translate vietnamese wanderers_brothel_menu1_c3d85c42:

    # th "I'm not sure how I should take that.."
    th "Tôi không chắc mình nên làm như thế nào.."

# game/places/wanderers/brothel.rpy:95
translate vietnamese wanderers_brothel_menu1_ee714e66:

    # madam_hof "So what can I do for you, [me]?"
    madam_hof "Vậy tôi có thể làm gì cho chú em nào, [me]?"

# game/places/wanderers/brothel.rpy:103
translate vietnamese wanderers_brothel_menu1_2b07d2b5_1:

    # madam_hof "New here, aren't you boy?"
    madam_hof "Mới đến, phải không cậu bé?"

# game/places/wanderers/brothel.rpy:104
translate vietnamese wanderers_brothel_menu1_ac0e68b1_1:

    # me "Yeah, is it so obvious?"
    me "Không phải nó quá rõ ràng à?"

# game/places/wanderers/brothel.rpy:107
translate vietnamese wanderers_brothel_menu1_963b6575_1:

    # madam_hof "You look too honest to be local, and too clean."
    madam_hof "Trông chú em quá trung thực để trở thành người địa phương và quá sạch sẽ."

# game/places/wanderers/brothel.rpy:108
translate vietnamese wanderers_brothel_menu1_3820771a_1:

    # me "..."
    me "..."

# game/places/wanderers/brothel.rpy:109
translate vietnamese wanderers_brothel_menu1_c02b3a89_1:

    # me "So what do you do here?"
    me "Vậy chị làm gì ở đây?"

# game/places/wanderers/brothel.rpy:112
translate vietnamese wanderers_brothel_menu1_d3f62b62_1:

    # madam_hof "Oh, boy.. It's written everywhere."
    madam_hof "Ôi, cậu bé .. Nó được viết ở khắp mọi nơi."

# game/places/wanderers/brothel.rpy:113
translate vietnamese wanderers_brothel_menu1_5cc8a5cd_1:

    # me "So it is."
    me "Nên nó là."

# game/places/wanderers/brothel.rpy:116
translate vietnamese wanderers_brothel_menu1_e446f0c3_1:

    # madam_hof "Are you here for some action?"
    madam_hof "Bạn có ở đây để hành động không có ở đây để hành động không?"

# game/places/wanderers/brothel.rpy:117
translate vietnamese wanderers_brothel_menu1_58db956b_1:

    # me "I'm not sure.."
    me "tôi không chắc.."

# game/places/wanderers/brothel.rpy:121
translate vietnamese wanderers_brothel_menu1_1ce08715:

    # madam_hof "Maybe we should start over. I'm Madam Hof."
    madam_hof "Có lẽ chúng ta nên bắt đầu lại. Tôi là Madam Hof."

# game/places/wanderers/brothel.rpy:122
translate vietnamese wanderers_brothel_menu1_fb30c850_1:

    # me "[me]."
    me "[me]."

# game/places/wanderers/brothel.rpy:125
translate vietnamese wanderers_brothel_menu1_f7f0483e_1:

    # madam_hof "Nice to met you, [me]. You look like someone that could become a regular."
    madam_hof "Rất vui được gặp chú em, [me]. Bạn trông giống như một người có thể trở thành thường xuyên."

# game/places/wanderers/brothel.rpy:126
translate vietnamese wanderers_brothel_menu1_c3d85c42_1:

    # th "I'm not sure how I should take that.."
    th "Tôi không chắc mình nên làm như thế nào.."

# game/places/wanderers/brothel.rpy:129
translate vietnamese wanderers_brothel_menu1_ee714e66_1:

    # madam_hof "So what can I do for you, [me]?"
    madam_hof "Vậy tôi có thể làm gì cho chú em hả, [me]?"

# game/places/wanderers/brothel.rpy:185
translate vietnamese wanderers_brothel_photos_f5c3110e:

    # madam_hof "Of course, boy. Here, take a look."
    madam_hof "Tất nhiên rồi, cậu bé. Đây, hãy xem."

# game/places/wanderers/brothel.rpy:195
translate vietnamese wanderers_brothel_photos_fd5639b3:

    # madam_hof "Are you interested in any of my girls?"
    madam_hof "Chú em có quan tâm đến bất kỳ cô gái nào của chụy không?"

# game/places/wanderers/brothel.rpy:196
translate vietnamese wanderers_brothel_photos_8a65bc6b:

    # me "Perhaps."
    me "Có lẽ."

# game/places/wanderers/brothel.rpy:197
translate vietnamese wanderers_brothel_photos_bb02fcce:

    # madam_hof "If the one you like is not available, feel free to check another time."
    madam_hof "Nếu đứa Chú em thích không có sẵn, thì hãy đến lúc khác."

translate vietnamese strings:

    # game/places/wanderers/brothel.rpy:2
    old "Sukki"
    new "Sukki"

    # game/places/wanderers/brothel.rpy:2
    old "Amanda"
    new "Amanda"

    # game/places/wanderers/brothel.rpy:2
    old "Robin"
    new "Robin"

    # game/places/wanderers/brothel.rpy:2
    old "Foxy"
    new "Foxy"

    # game/places/wanderers/brothel.rpy:56
    old "Booth"
    new "Gian hàng"

    # game/places/wanderers/brothel.rpy:56
    old "Can I take a look at your girls before paying?"
    new "Tôi có thể nhìn các cô gái của Chị trước khi trả tiền không?"
