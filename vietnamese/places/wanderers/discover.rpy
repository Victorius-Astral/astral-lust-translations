# : Translation updated at 2021-12-24 07:09

# game/places/wanderers/discover.rpy:13
translate vietnamese discover_wanderers_65fc5510:

    # "Gunshots break the silence."
    "Tiếng súng rền vang phá vỡ sự im lặng." # TL

# game/places/wanderers/discover.rpy:14
translate vietnamese discover_wanderers_8198f605:

    # th "Someone's in trouble.."
    th "Ai đó đang gặp rắc rối..." # TL

# game/places/wanderers/discover.rpy:15
translate vietnamese discover_wanderers_ca2e464b:

    # th "Should I check it out?"
    th "Mình có nên kiểm tra nó không?" # TL

# game/places/wanderers/discover.rpy:22
translate vietnamese discover_wanderers_e2560b69:

    # th "That's not my business."
    th "Đó không phải việc của mình." # TL

# game/places/wanderers/discover.rpy:27
translate vietnamese discover_wanderers_part2_1e41129d:

    # "You soon find the source of the shots."
    "Bạn tìm thấy nguồn gốc của các phát bắn." # TL

# game/places/wanderers/discover.rpy:30
translate vietnamese discover_wanderers_part2_25b4b7c0:

    # th "Hmm.. Should I help him?"
    th "Hmm... Mình có nên giúp anh ta không?" # TL

# game/places/wanderers/discover.rpy:33
translate vietnamese discover_wanderers_part2_f4807aa2:

    # th "Seems I don't have a choice now.."
    th "Có vẻ như mình không có lựa chọn rồi..." # TL

# game/places/wanderers/discover.rpy:39
translate vietnamese discover_wanderers_win_acfe05be:

    # u_governor "Hah. You're not bad my friend."
    u_governor "Hah. không tệ đâu, bạn của tôi." # TL

# game/places/wanderers/discover.rpy:42
translate vietnamese discover_wanderers_win_109d9d71:

    # me "You are not too bad yourself."
    me "Ông cũng không tệ đâu." # TL

# game/places/wanderers/discover.rpy:43
translate vietnamese discover_wanderers_win_d839f1bd:

    # u_governor "Let's go."
    u_governor "Đi nào." # TL

# game/places/wanderers/discover.rpy:44
translate vietnamese discover_wanderers_win_2b5f54cc:

    # me "Where?"
    me "Đi đâu?" # TL

# game/places/wanderers/discover.rpy:45
translate vietnamese discover_wanderers_win_3219fc3c:

    # u_governor "Away, there is a nest nearby, and soon there will be dozens of them."
    u_governor "Đi khỏi đây, tổ của chúng ở gần đây đấy, chẳng mấy chốc sẽ có hàng chục con..." # TL

# game/places/wanderers/discover.rpy:46
translate vietnamese discover_wanderers_win_2529f3eb:

    # me "Oh, shit. Alright, after you!"
    me "Oh, shit. Được rồi, ngay sau ông!" # TL

# game/places/wanderers/discover.rpy:49
translate vietnamese discover_wanderers_win_e7938e38:

    # me "Do you know any safe place nearby?"
    me "ông có biết nơi nào an toàn gần đây không?" # TL

# game/places/wanderers/discover.rpy:50
translate vietnamese discover_wanderers_win_d5f8bdf6:

    # u_governor "Of course, it'll be night soon, and I've seen what happens to people that stay out there at night. Believe me, I don't want to be one of them."
    u_governor "Tất nhiên rồi, trời sẽ sớm tối, và tôi cũng thấy những gì đã xảy ra với những người ở ngoài đó vào ban đêm rồi. Tin tôi đi, tôi không muốn trở thành một trong số họ đâu." # TL

# game/places/wanderers/discover.rpy:53
translate vietnamese discover_wanderers_win_16fce20f:

    # me "How fa.."
    me "Còn bao x..." # TL

# game/places/wanderers/discover.rpy:56
translate vietnamese discover_wanderers_win_b68452fb:

    # "Tremendous roars echo from behind you."
    "Những tiếng gầm khủng khiếp vang lên từ phía sau bạn." # TL

# game/places/wanderers/discover.rpy:59
translate vietnamese discover_wanderers_win_c89f3dc6:

    # me "Shit! What the fuck is that?!"
    me "Chết tiệt! Cái đéo gì vậy?!" # TL

# game/places/wanderers/discover.rpy:60
translate vietnamese discover_wanderers_win_a721d6b4:

    # u_governor "We call it Abaddon! Don't worry, it won't pursue us for long. It also fears the night."
    u_governor "Bọn tôi gọi nó là Quỷ âm phủ! Đừng lo, nó sẽ không theo đuổi chúng ta lâu đâu. Nó cũng sợ ban đêm như chúng ta thôi." # TL

# game/places/wanderers/discover.rpy:63
translate vietnamese discover_wanderers_win_28eb48a0:

    # me "What the hell is it?"
    me "Cái quái gì thế này?" # TL

# game/places/wanderers/discover.rpy:64
translate vietnamese discover_wanderers_win_4cd32798:

    # u_governor "Time is tight. Don't talk, run!"
    u_governor "Thời gian rất eo hẹp. Đừng nói nữa, chạy đi!" # TL

# game/places/wanderers/discover.rpy:71
translate vietnamese discover_wanderers_win_a58e9d67:

    # "After a long time you finally stop in a small shanty town." id discover_wanderers_win_a58e9d67
    "Sau một thời gian dài, cuối cùng bạn cũng dừng lại ở một thị trấn nhỏ tồi tàn." id discover_wanderers_win_a58e9d67 # TL

# game/places/wanderers/discover.rpy:72
translate vietnamese discover_wanderers_win_a310548c:

    # me "Ahh... *Cough* My lungs.."
    me "Ahh... *Khụ* Phổi của mình..." # TL

# game/places/wanderers/discover.rpy:73
translate vietnamese discover_wanderers_win_8f076c8d:

    # th "Shit, I'm so tired.."
    th "Mẹ kiếp, mình mệt quá..." # TL

# game/places/wanderers/discover.rpy:74
translate vietnamese discover_wanderers_win_b5c22da3:

    # u_governor "Aaa.. It was a close call."
    u_governor "Aaa... Đó là một pha suýt soát đấy." # TL

# game/places/wanderers/discover.rpy:77
translate vietnamese discover_wanderers_win_df426f64:

    # me "It's time to talk, don't you think? Let's go inside."
    me "Đến lúc nói chuyện rồi, ông có nghĩ vậy không? Hãy vào trong thôi." # TL

# game/places/wanderers/discover.rpy:78
translate vietnamese discover_wanderers_win_618f7ce2:

    # u_governor "Don't worry, the fog can't enter into the hideout."
    u_governor "Đừng lo, sương mù không thể vào đây được đâu." # TL

# game/places/wanderers/discover.rpy:79
translate vietnamese discover_wanderers_win_314086b3:

    # me "What?"
    me "Hả?" # TL

# game/places/wanderers/discover.rpy:85
translate vietnamese discover_wanderers_win_ab5eea35:

    # u_governor "I also don't know the reason, but fog avoids this place. It avoids all places humans hide, but you have probably already noticed."
    u_governor "Tôi cũng không biết lý do, nhưng sương mù tránh xa nơi này. Nó tránh tất cả những nơi con người ẩn náu, nhưng bạn có thể đã nhận ra." # TL

# game/places/wanderers/discover.rpy:86
translate vietnamese discover_wanderers_win_effb048c:

    # me "But everything is in the open."
    me "Nhưng mọi thứ ở đây đều lộ thiên mà." # TL

# game/places/wanderers/discover.rpy:87
translate vietnamese discover_wanderers_win_4fb61e3a:

    # u_governor "It is."
    u_governor "Đúng vậy." # TL

# game/places/wanderers/discover.rpy:88
translate vietnamese discover_wanderers_win_b2f20a8e:

    # me "That's fucked.. This whole situation is fucked up."
    me "Cái tình huống này thật là rối rắm mà." # TL

# game/places/wanderers/discover.rpy:91
translate vietnamese discover_wanderers_win_09b8ddaf:

    # governor "It is indeed. Let me introduce myself, I'm the governor of this hideout."
    governor "Ừ. Hãy để tôi tự giới thiệu, tôi là thống đốc của nơi ẩn náu này." # TL

# game/places/wanderers/discover.rpy:92
translate vietnamese discover_wanderers_win_a265d218:

    # governor "While we wanderers are free spirits, this place is a safe haven one can use to trade and rest."
    governor "Trong khi những người du hành chúng ta là những linh hồn tự do, thì nơi này là nơi trú ẩn an toàn mà người ta có thể sử dụng để buôn bán và nghỉ ngơi." # TL

# game/places/wanderers/discover.rpy:95
translate vietnamese discover_wanderers_win_9204eebd:

    # me "Wanderers?"
    me "Người du mục?" # TL

# game/places/wanderers/discover.rpy:97
translate vietnamese discover_wanderers_win_ec126fe2:

    # governor "Yes, the wanderers consist of independent groups of people that help each other survive. You can think of us as a faction."
    governor "Đúng vậy, những người du mục bao gồm những nhóm người độc lập giúp nhau tồn tại. Bạn có thể coi chúng tôi như một phe." # TL

# game/places/wanderers/discover.rpy:98
translate vietnamese discover_wanderers_win_46d27802:

    # me "I see."
    me "À." # TL

# game/places/wanderers/discover.rpy:100
translate vietnamese discover_wanderers_win_f991a53c:

    # me "But if wanderers are independent groups of people, what's up with the hideout and governor thing?"
    me "Nhưng nếu những người du mục là những nhóm người độc lập, thì điều gì sẽ xảy ra với nơi ẩn náu và thống đốc?" # TL

# game/places/wanderers/discover.rpy:103
translate vietnamese discover_wanderers_win_dcdb7b98:

    # governor "Haha, this place is one of a kind."
    governor "Haha, nơi này là có một không hai." # TL

# game/places/wanderers/discover.rpy:106
translate vietnamese discover_wanderers_win_8027bf6c:

    # governor "I wanted a place like this to appear, so I built it from scratch. Later more people joined, and it grew to what it is now." id discover_wanderers_win_8027bf6c
    governor "Tôi muốn một nơi như thế này xuất hiện, vì vậy nên tôi đã xây dựng nó từ lúc còn hai bàn tay trắng. Sau đó, nhiều người tham gia hơn và nó đã phát triển như bây giờ." id discover_wanderers_win_8027bf6c # TL

# game/places/wanderers/discover.rpy:107
translate vietnamese discover_wanderers_win_5331fc6c:

    # governor "It's a free trade outpost and safehouse. It's not a needy group, but you can help out wherever you want. You're also free to leave anytime."
    governor "Đây là một tiền đồn thương mại tự do và cũng là một vùng an toàn. Đây không phải là một nhóm thiếu thốn, nhưng ông có thể giúp đỡ mọi lúc mọi nơi. Ông cũng có thể tự do rời đi bất cứ lúc nào." # TL

# game/places/wanderers/discover.rpy:108
translate vietnamese discover_wanderers_win_3f14e1ac:

    # me "I see. I think I understand now. So what should I call you?"
    me "Ồ. Hiểu rồi. Vậy tôi nên gọi ông là gì?" # TL

# game/places/wanderers/discover.rpy:111
translate vietnamese discover_wanderers_win_df1a4e9b:

    # governor "Call me [governor], no one's called me any different since The End began. I've already abandoned my old name."
    governor "Gọi tôi là [governor], mọi người luôn gọi tôi như vậy kể từ khi tận thế bắt đầu. Tôi đã bỏ tên cũ của mình lâu rồi." # TL

# game/places/wanderers/discover.rpy:112
translate vietnamese discover_wanderers_win_1de8d0c4:

    # me "Fine then, [governor]. I'm [me]."
    me "Được rồi, [governor]. tôi là [me]." # TL

# game/places/wanderers/discover.rpy:115
translate vietnamese discover_wanderers_win_2509e37b:

    # governor "It's nice to meet you, [me]."
    governor "Rất vui được gặp ông, [me]." # TL

# game/places/wanderers/discover.rpy:116
translate vietnamese discover_wanderers_win_87b3c51c:

    # governor "Also, thank you for your help. I would be hard pressed to return before night if not for you."
    governor "Mà, cảm ơn vì sự giúp đỡ của ông. Tôi sẽ khó trở về trước đêm nếu không có ông đấy." # TL

# game/places/wanderers/discover.rpy:117
translate vietnamese discover_wanderers_win_2797c9ff:

    # me "I do what I can."
    me "tôi làm những gì có thể thôi." # TL

# game/places/wanderers/discover.rpy:121
translate vietnamese discover_wanderers_win_4a6d623b:

    # governor "Take this."
    governor "Nhận lấy cái này đi." # TL

# game/places/wanderers/discover.rpy:122
translate vietnamese discover_wanderers_win_40e60644:

    # me "Are those.."
    me "Có phải chúng là..." # TL

# game/places/wanderers/discover.rpy:125
translate vietnamese discover_wanderers_win_d02b95ea:

    # governor "Bottle caps? Yes, they are."
    governor "Nắp chai? đúng, là nó đấy." # TL

# game/places/wanderers/discover.rpy:126
translate vietnamese discover_wanderers_win_23be24fb:

    # me "Thanks.."
    me "Cảm ơn..." # TL

# game/places/wanderers/discover.rpy:127
translate vietnamese discover_wanderers_win_cf3a6cd0:

    # th "What the hell am I supposed to do with them?"
    th "Mình phải làm cái quái gì với cái này đây?" # TL

# game/places/wanderers/discover.rpy:130
translate vietnamese discover_wanderers_win_9c3fc517:

    # governor "A pity you can't see your face!"
    governor "Rất tiếc là ông không thể nhìn thấy khuôn mặt của mình!" # TL

# game/places/wanderers/discover.rpy:131
translate vietnamese discover_wanderers_win_12190c3a:

    # me "Haha.. Sorry.."
    me "Haha... Xin lỗi..." # TL

# game/places/wanderers/discover.rpy:134
translate vietnamese discover_wanderers_win_c5d28f49:

    # governor "These are our local currency. They are accepted by all wanderers."
    governor "Đây là tiền nội tệ của chúng tôi. Nó được chấp nhận bởi tất cả những người lữ hành." # TL

# game/places/wanderers/discover.rpy:135
translate vietnamese discover_wanderers_win_225ecd31:

    # me "Oh, they will be useful then."
    me "Oh, chúng sẽ hữu ích đấy." # TL

# game/places/wanderers/discover.rpy:138
translate vietnamese discover_wanderers_win_acb81293:

    # governor "Come, I'll take you to a place you can stay at."
    governor "Đến đây, tôi sẽ đưa ông đến một nơi ông có thể ở." # TL

# game/places/wanderers/discover.rpy:139
translate vietnamese discover_wanderers_win_12cf7b69:

    # me "Thanks."
    me "Mơn." # TL

# game/places/wanderers/discover.rpy:145
translate vietnamese discover_wanderers_win_d611ebaa:

    # governor "It's not much, but it's better than nothing. The first room is yours."
    governor "Nó không nhiều, nhưng có còn hơn không. Căn phòng đầu tiên là của ông." # TL

# game/places/wanderers/discover.rpy:146
translate vietnamese discover_wanderers_win_a0a697c9:

    # me "Thank you."
    me "Cảm ơn ông." # TL

# game/places/wanderers/discover.rpy:149
translate vietnamese discover_wanderers_win_650ce5b6:

    # governor "You can stay here anytime. Normally one needs to pay for a night at the motel, but you saved my ass today. Let that stay between us."
    governor "Ông có thể ở đây bất cứ lúc nào. Thông thường người ta cần phải trả tiền cho một đêm ở nhà nghỉ, nhưng hôm nay ông đã cứu tôi. Nên hãy để điều đó ở lại giữa chúng ta." # TL

# game/places/wanderers/discover.rpy:150
translate vietnamese discover_wanderers_win_b5c73233:

    # me "Sure thing, [governor], and thank you, I appreciate your hospitality."
    me "Chắc chắn rồi, [governor], và cảm ơn ông, tôi đánh giá cao sự hiếu khách của ông." # TL

# game/places/wanderers/discover.rpy:153
translate vietnamese discover_wanderers_win_636d1176:

    # governor "It's the least I can do. Alright, my friend, I believe we both need to rest. We'll speak again soon."
    governor "Đó là điều tối thiểu mà tôi có thể làm được. Được rồi, bạn của tôi, tôi tin rằng cả hai chúng ta cần phải nghỉ ngơi. Chúng ta sẽ nói chuyện sau nha." # TL

# game/places/wanderers/discover.rpy:154
translate vietnamese discover_wanderers_win_0ec0d987:

    # me "Yeah, that was a very tiring evening. See you." id discover_wanderers_win_0ec0d987
    me "Yeah, đó là một buổi chiều khá mệt mỏi đấy. Gặp lại sau." id discover_wanderers_win_0ec0d987 # TL

# game/places/wanderers/discover.rpy:155
translate vietnamese discover_wanderers_win_6309f547:

    # governor "Sleep well, friend."
    governor "Ngủ ngon." # TL

# game/places/wanderers/discover.rpy:158
translate vietnamese discover_wanderers_win_e4e5e3cf:

    # th "Ahh.. That was really tiring day. This [governor] guy seems nice enough. I think we'll be able to get along just fine." id discover_wanderers_win_e4e5e3cf
    th "Ahh... Đó là một ngày thực sự mệt mỏi đấy. Anh chàng [governor] này có vẻ tốt. Có vẻ là sẽ hòa hợp với nhau đấy." id discover_wanderers_win_e4e5e3cf # TL
