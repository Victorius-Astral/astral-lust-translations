# : Translation updated at 2021-12-24 07:09

# game/events/dreams/deal_with_the_devil.rpy:23
translate vietnamese d_Deal_with_the_Devil_3_d94f51a6:

    # "You find yourself inside your room, surrounded by a roaring fire."
    "Bạn thấy mình đang ở trong phòng của bạn, xung quanh là một ngọn lửa đang bốc cháy dữ dội." # TL

# game/events/dreams/deal_with_the_devil.rpy:24
translate vietnamese d_Deal_with_the_Devil_3_18010e2b:

    # "Your bed is the only place not burning. On the sofa sits a dark silhouette with glowing red eyes."
    "Giường của bạn là nơi duy nhất không bị cháy. Trên ghế sofa là hình bóng đen với đôi mắt đỏ rực." # TL

# game/events/dreams/deal_with_the_devil.rpy:27
translate vietnamese d_Deal_with_the_Devil_3_248e66e0:

    # me "Who are you?"
    me "Ngươi là ai?" # TL

# game/events/dreams/deal_with_the_devil.rpy:29
translate vietnamese d_Deal_with_the_Devil_3_8219b62f:

    # devil "I have many names. Some call me God, some call me Devil."
    devil "Ta có nhiều tên. Có người gọi là Thần, có người gọi là quỷ." # TL

# game/events/dreams/deal_with_the_devil.rpy:30
translate vietnamese d_Deal_with_the_Devil_3_8c058f61:

    # "Its voice sounds like that of a young girl, almost playful and innocent. If you couldn't see it with your own eyes, you would think it was a child."
    "Giọng nói của nó giống như của một cô gái trẻ, kiểu vui tươi và ngây thơ. Nếu bạn không thể nhìn thấy nó tận mắt, bạn sẽ nghĩ rằng nó là một đứa trẻ." # TL

# game/events/dreams/deal_with_the_devil.rpy:32
translate vietnamese d_Deal_with_the_Devil_3_435f125f:

    # devil "I have an offer for you."
    devil "Ta có một đề nghị cho ngươi." # TL

# game/events/dreams/deal_with_the_devil.rpy:33
translate vietnamese d_Deal_with_the_Devil_3_86e225a7:

    # "You feel a flood of information entering your mind."
    "Bạn cảm thấy thông tin tràn ngập vào tâm trí mình." # TL

# game/events/dreams/deal_with_the_devil.rpy:34
translate vietnamese d_Deal_with_the_Devil_3_1d1111c9:

    # devil "So now you know my proposal, are you interested?"
    devil "Bây giờ ngươi biết đề xuất của ta rồi. Vậy, ngươi có quan tâm không?" # TL

# game/events/dreams/deal_with_the_devil.rpy:53
translate vietnamese d_Deal_with_the_Devil_3_menu1_036dfd22:

    # devil "Done."
    devil "Xong." # TL

# game/events/dreams/deal_with_the_devil.rpy:57
translate vietnamese d_Deal_with_the_Devil_3_menu1_36d49e85:

    # devil "You don't have the required price."
    devil "Ngươi không có cái giá mà ta yêu cầu." # TL

# game/events/dreams/deal_with_the_devil.rpy:64
translate vietnamese d_Deal_with_the_Devil_3_menu1_036dfd22_1:

    # devil "Done."
    devil "Xong." # TL

# game/events/dreams/deal_with_the_devil.rpy:68
translate vietnamese d_Deal_with_the_Devil_3_menu1_36d49e85_1:

    # devil "You don't have the required price."
    devil "Ngươi không có cái giá mà ta yêu cầu." # TL

# game/events/dreams/deal_with_the_devil.rpy:75
translate vietnamese d_Deal_with_the_Devil_3_menu1_036dfd22_2:

    # devil "Done."
    devil "Xong." # TL

# game/events/dreams/deal_with_the_devil.rpy:82
translate vietnamese d_Deal_with_the_Devil_3_menu1_036dfd22_3:

    # devil "Done."
    devil "Xong." # TL

# game/events/dreams/deal_with_the_devil.rpy:86
translate vietnamese d_Deal_with_the_Devil_3_menu1_3f9ebed0:

    # devil "I don't want to involve Myself into these games. But I can help you a little."
    devil "Ta không muốn bản thân dính líu vào mấy trò chơi này. Nhưng ta có thể giúp một chút." # TL

# game/events/dreams/deal_with_the_devil.rpy:87
translate vietnamese d_Deal_with_the_Devil_3_menu1_2fc0e6f2:

    # me "How?"
    me "Như thế nào?" # TL

# game/events/dreams/deal_with_the_devil.rpy:88
translate vietnamese d_Deal_with_the_Devil_3_menu1_fd1dac3b:

    # devil "I'll weaken influence someone has on you."
    devil "Ta sẽ làm suy yếu sự ảnh hưởng của ai đó đặt lên ngươi." # TL

# game/events/dreams/deal_with_the_devil.rpy:89
translate vietnamese d_Deal_with_the_Devil_3_menu1_273f3d1f:

    # me "Who?!"
    me "Ai?!" # TL

# game/events/dreams/deal_with_the_devil.rpy:90
translate vietnamese d_Deal_with_the_Devil_3_menu1_dc4cc646:

    # devil "You will know when you need to."
    devil "Ngươi sẽ biết khi nào ngươi cần." # TL

# game/events/dreams/deal_with_the_devil.rpy:91
translate vietnamese d_Deal_with_the_Devil_3_menu1_d7945924:

    # th "What the hell, someone is influencing me and I don't even know?"
    th "Cái quái gì vậy, ai đó đang tác động đến mình mà thậm chí mình cũng không biết sao?" # TL

# game/events/dreams/deal_with_the_devil.rpy:92
translate vietnamese d_Deal_with_the_Devil_3_menu1_65f639db:

    # me "What is the price?"
    me "Cái giá là gì?" # TL

# game/events/dreams/deal_with_the_devil.rpy:93
translate vietnamese d_Deal_with_the_Devil_3_menu1_ee5e4510:

    # devil "I'll take most of your sanity, and I'll take most of your abilities."
    devil "Ta sẽ lấy lấy đi gần hết sự tỉnh táo của ngươi, và Ta sẽ lấy đi hầu hết các khả năng của ngươi." # TL

# game/events/dreams/deal_with_the_devil.rpy:94
translate vietnamese d_Deal_with_the_Devil_3_menu1_3820771a:

    # me "..."
    me "..."

# game/events/dreams/deal_with_the_devil.rpy:103
translate vietnamese d_Deal_with_the_Devil_3_menu1_9539ab5a:

    # devil "This will give you more time. Check yourself often, as one day He might replace you if you're not careful."
    devil "Điều này sẽ cung cấp cho ngươi nhiều thời gian hơn. Hãy kiểm tra bản thân thường xuyên, vì một ngày nào đó, Hắn có thể thay thế ngươi nếu ngươi không cẩn thận." # TL

# game/events/dreams/deal_with_the_devil.rpy:104
translate vietnamese d_Deal_with_the_Devil_3_menu1_b5966159:

    # me "What do you mean?!"
    me "Ý ngươi là gì?!" # TL

# game/events/dreams/deal_with_the_devil.rpy:115
translate vietnamese d_Deal_with_the_Devil_3_menu1_4d32fd8e:

    # devil "So be it."
    devil "Như vậy đi." # TL

translate vietnamese strings:

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "Make me lucky [[Lose 40 random attribute points. Gain +2 luck, max 5 purchases] {#visible}"
    new "Làm cho tôi may mắn [[Mất 40 điểm thuộc tính ngẫu nhiên. Nhận được +2 may mắn, tối đa 5] {#visible}" # TL

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "I want three powerful abilities [[Lose 20 random attribute points. Choose a card from three high rarity choices.]"
    new "Tôi muốn có ba khả năng mạnh mẽ [[Mất 20 điểm thuộc tính ngẫu nhiên. Chọn một thẻ từ ba lựa chọn độ hiếm cao.]" # TL

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "Remove my corruption [[-50% Sanity, -5% Corruption]"
    new "Xóa sự xa đọa của tôi [[-50% Tỉnh táo, -5% xa đọa]" # TL

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "Restore my clarity [[+100% Sanity, +5% Corruption]"
    new "Khôi phục sự trong sáng của tôi [[+100% Tỉnh táo, +5% xa đọa]" # TL

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "I want to be a master of my own destiny {#visible}"
    new "Tôi muốn trở thành người làm chủ vận mệnh của chính mình {#visible}" # TL

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "Kill me [[Die]"
    new "Giết ta đi [[Chết]" # TL

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "I want nothing from you"
    new "Ta không muốn gì từ ngươi" # TL

    # game/events/dreams/deal_with_the_devil.rpy:74
    old "Sanity -50%\nCorruption -5%"
    new "-50 % Tỉnh táo, -5% sa đọa" # TL

    # game/events/dreams/deal_with_the_devil.rpy:81
    old "Sanity +100%\nCorruption +5%"
    new "+ 100 % Tỉnh táo, +5% sa đọa" # TL

    # game/events/dreams/deal_with_the_devil.rpy:95
    old "[[Sanity will become 1%, you'll lose all your cards. Eldritch influence will weaken by 4 points. Influence: [author_influence]/10]"
    new "[[Độ tỉnh táo sẽ trở thành 1%, bạn sẽ mất tất cả các thẻ của mình. Ảnh hưởng của Eldritch sẽ yếu đi 4 điểm. Ảnh hưởng: [author_influence]/10]" # TL

    # game/events/dreams/deal_with_the_devil.rpy:95
    old "Deal"
    new "Thỏa thuận" # TL

    # game/events/dreams/deal_with_the_devil.rpy:95
    old "I won't take your offer"
    new "Ta sẽ không nhận lời đề nghị của ngươi" # TL

# : Translation updated at 2022-02-06 08:10

translate vietnamese strings:

    # game/events/dreams/deal_with_the_devil.rpy:95
    old "[[Sanity will become 1%, you'll lose all your cards. Eldritch influence will weaken by 4 points. Influence: [player.author_influence]/10]"
    new "[[Độ tỉnh táo sẽ còn 1%, bạn sẽ mất tất cả các thẻ của mình. Ảnh hưởng của Eldritch sẽ yếu đi 4 điểm. Ảnh hưởng: [player.author_influence]/10]" # TL
# : Translation updated at 2022-04-17 12:12

translate vietnamese strings:

    # game/events/dreams/deal_with_the_devil.rpy:32
    old "Remove my corruption [[-50%% Sanity, -5%% Corruption]"
    new "Xóa bỏ sự xa đọa của tôi [[-50%% Tỉnh táo, -5%% Sa đọa]" # TL

    # game/events/dreams/deal_with_the_devil.rpy:32
    old "Restore my clarity [[+100%% Sanity, +5%% Corruption]"
    new "Khôi phục sự trong sáng của tôi [[+100%% Tỉnh táo, +5%% Sa đọa]" # TL

    # game/events/dreams/deal_with_the_devil.rpy:91
    old "[[Sanity will become 1%%, you'll lose all your cards. Eldritch influence will weaken by 4 points. Influence: [player.author_influence]/10]"
    new "[[Độ tỉnh táo sẽ chỉ còn 1%%, bạn sẽ mất tất cả các thẻ của mình. Ảnh hưởng của Eldritch sẽ yếu đi 4 điểm. Ảnh hưởng: [author_influence]/10]" # TL
# : Translation updated at 2022-09-15 16:10

# game/events/dreams/deal_with_the_devil.rpy:110
translate vietnamese d_Deal_with_the_Devil_3_menu1_75df7159:

    # devil "{evil}Hahaha!{/evil}"
    devil "{evil}Hahaha!{/evil}" # TL # Victorius - AI

# game/events/dreams/deal_with_the_devil.rpy:111
translate vietnamese d_Deal_with_the_Devil_3_menu1_44eef04f:

    # devil "{evil}I shall grant you your will.{/evil}"
    devil "{evil}Ta sẽ ban cho ngươi thứ mà ngươi muốn.{/evil}" # TL

# game/events/dreams/deal_with_the_devil.rpy:118
translate vietnamese d_Deal_with_the_Devil_3_menu1_4af8bc15:

    # devil "{evil}Decadent Monarch, Scorching Nightmare, Father of Desires{/evil}"
    devil "{evil}Quân vương suy đồi, Cơn ác mộng bỏng rát, Người cha của dục vọng{/evil}" # TL

# game/events/dreams/deal_with_the_devil.rpy:119
translate vietnamese d_Deal_with_the_Devil_3_menu1_67ed8805:

    # "Its name makes your head to throb in pain as illusory blood flows down your ears."
    "Cái tên của nó khiến đầu bạn nhói lên như thể dòng máu hư ảo chảy xuống tai." # TL

# game/events/dreams/deal_with_the_devil.rpy:120
translate vietnamese d_Deal_with_the_Devil_3_menu1_6aba83f5:

    # me "Argh!"
    me "Argh!" # TL # Victorius - AI

# game/events/dreams/deal_with_the_devil.rpy:121
translate vietnamese d_Deal_with_the_Devil_3_menu1_0110eb39:

    # devil "Haha, let this pain be the price."
    devil "Haha, hãy để nỗi đau này là cái giá đi." # TL

# game/events/dreams/deal_with_the_devil.rpy:122
translate vietnamese d_Deal_with_the_Devil_3_menu1_4bc02b39:

    # me "That's all?"
    me "Thế thôi á?" # TL

# game/events/dreams/deal_with_the_devil.rpy:123
translate vietnamese d_Deal_with_the_Devil_3_menu1_9181b3a9:

    # devil "If your sacrifices please me, you'll receive my blessings. If they insult me-"
    devil "Nếu đồ hiến tế của ngươi làm hài lòng ta, ngươi sẽ nhận được phước lành của ta. Nếu chúng xúc phạm ta-" # TL

# game/events/dreams/deal_with_the_devil.rpy:124
translate vietnamese d_Deal_with_the_Devil_3_menu1_c465d04f:

    # devil "Hahaha!"
    devil "Hahaha!" # TL # Victorius - AI

# game/events/dreams/deal_with_the_devil.rpy:125
translate vietnamese d_Deal_with_the_Devil_3_menu1_3820771a_1:

    # me "..."
    me "..."

# game/events/dreams/deal_with_the_devil.rpy:130
translate vietnamese d_Deal_with_the_Devil_3_menu1_d0e2f6d7:

    # devil "Very well."
    devil "Tốt lắm." # TL

# game/events/dreams/deal_with_the_devil.rpy:131
translate vietnamese d_Deal_with_the_Devil_3_menu1_d424dc21:

    # devil "{evil}We shall met again. Do you agree?{/evil}"
    devil "{evil}Chúng ta sẽ gặp lại nhau. Ngươi có đồng ý chứ?{/evil}" # TL

# game/events/dreams/deal_with_the_devil.rpy:132
translate vietnamese d_Deal_with_the_Devil_3_menu1_17dbf313:

    # "Its tone suddenly changed into a monstorous and evil raving, making your soul and mind shiver in horror."
    "Giọng điệu của nó đột nhiên thay đổi thành một thứ quái dị và ác độc, khiến tâm hồn và tâm trí bạn rùng mình kinh hãi." # TL

# game/events/dreams/deal_with_the_devil.rpy:133
translate vietnamese d_Deal_with_the_Devil_3_menu1_0e4aef8c:

    # th "There is something wrong with what 'It' said..."
    th "Có điều gì đó không ổn với những gì 'Nó' nói..." # TL

# game/events/dreams/deal_with_the_devil.rpy:134
translate vietnamese d_Deal_with_the_Devil_3_menu1_a2f309d6:

    # me "Yes..."
    me "Có..." # TL

translate vietnamese strings:

    # game/events/dreams/deal_with_the_devil.rpy:34
    old "I wish to know your honorary name, to seek your blessings in the rituals"
    new "Ta muốn biết tên danh dự của ngươi, để tìm kiếm phước lành của ngươi trong các nghi lễ" # TL

    # game/events/dreams/deal_with_the_devil.rpy:34
    old "Can we make the deal another time?"
    new "Chúng ta có thể thực hiện giao kèo vào lần khác không?" # TL
