# : Translation updated at 2021-12-24 07:09

# game/events/endings/autor.rpy:14
translate vietnamese end_author_c1b575f0:

    # me "Who am I? Where am I?"
    me "Tôi là ai? Đây là đâu??" # TL

# game/events/endings/autor.rpy:15
translate vietnamese end_author_ac5d27e2:

    # "You're Author. You're in your cathedral."
    "Ngươi là Tác giả. Ngươi đang ở trong thánh đường của mình." # TL

# game/events/endings/autor.rpy:16
translate vietnamese end_author_3812e5ee:

    # me "I am Author.."
    me "Tôi là tác giả..." # TL

# game/events/endings/autor.rpy:19
translate vietnamese end_author_2de99c9a:

    # me "Wait, I'm [me]!"
    me "Chờ đã, tôi là [me]!" # TL

# game/events/endings/autor.rpy:20
translate vietnamese end_author_f6f37c6b:

    # "Forget this name, you had never heard of it before."
    "Quên cái tên này đi, ngươi chưa bao giờ nghe nói về nó trước đây." # TL

# game/events/endings/autor.rpy:23
translate vietnamese end_author_3820771a:

    # me "..."
    me "..."

# game/events/endings/autor.rpy:24
translate vietnamese end_author_e3792e66:

    # "You need to prepare for apotheosis ritual."
    "Ngươi cần chuẩn bị cho nghi lễ phong thần." # TL

# game/events/endings/autor.rpy:25
translate vietnamese end_author_fd9be525:

    # me "I need to prepare for apotheosis ritual."
    me "Tôi cần chuẩn bị cho nghi lễ phong thần." # TL

# game/events/endings/autor.rpy:28
translate vietnamese end_author_873ecf41:

    # th "Shit!"
    th "Địt!" # TL

# game/events/endings/autor.rpy:37
translate vietnamese end_author_7aa9c3e9:

    # aharon "Make a wish."
    aharon "Ước đi." # TL

# game/events/endings/autor.rpy:38
translate vietnamese end_author_28d8bef0:

    # me "I wish to be saved!"
    me "Tôi ước sẽ được cứu!" # TL

# game/events/endings/autor.rpy:41
translate vietnamese end_author_f6c49435:

    # "Just as you made your wish, blood covered your vision."
    "Như bạn đã ước, máu đã che mất tầm nhìn của bạn." # TL

# game/events/endings/autor.rpy:45
translate vietnamese end_author_79e4ab7c:

    # aharon "Your wish shall be granted."
    aharon "Điều ước của ngươi sẽ được thực hiện." # TL

# game/events/endings/autor.rpy:55
translate vietnamese end_author_wait_fcdb0505:

    # "Wait, time will solve all."
    "Chờ đi, thời gian sẽ giải quyết tất cả." # TL

# game/events/endings/autor.rpy:56
translate vietnamese end_author_wait_3820771a:

    # me "..."
    me "..."

# game/events/endings/autor.rpy:58
translate vietnamese end_author_wait_b3e76ead:

    # "I'll take care of everything for you, you can rest now."
    "Ta sẽ lo mọi thứ cho, Ngươi có thể nghỉ ngơi ngay bây giờ." # TL

# game/events/endings/autor.rpy:61
translate vietnamese end_author_wait_cc7e75dd:

    # me "I'm so tired.."
    me "tôi mệt quá..." # TL

# game/events/endings/autor.rpy:64
translate vietnamese end_author_wait_8994d7b6:

    # "Yes, you can sleep now.."
    "Được, Ngươi có thể ngủ ngay bây giờ..." # TL

# game/events/endings/autor.rpy:67
translate vietnamese end_author_wait_efbcdf46:

    # me "Yes, I feel tired."
    me "Vâng, tôi cảm thấy mệt mỏi." # TL

# game/events/endings/autor.rpy:70
translate vietnamese end_author_wait_aec79327:

    # th "So tired.."
    th "quá mệt mỏi..." # TL

translate vietnamese strings:

    # game/events/endings/autor.rpy:11
    old "Author's influence on you unknowingly reached the peak!"
    new "Ảnh hưởng của tác giả lên bạn vô tình đạt đến đỉnh điểm!" # TL

    # game/events/endings/autor.rpy:30
    old "You should wait."
    new "Bạn nên đợi." # TL

    # game/events/endings/autor.rpy:30
    old "Aharon! [[Aharon]"
    new "Aharon! [[Aharon]" # TL # Victorius - AI

    # game/events/endings/autor.rpy:36
    old "Card 'Aharon' destroyed!"
    new "Thẻ 'Aharon' bị phá hủy!" # TL
# : Translation updated at 2022-02-06 08:10

# game/events/endings/autor.rpy:20
translate vietnamese end_author_31573d51:

    # "Forget this name. You had never heard of it before."
    "Quên cái tên này đi, ngươi chưa bao giờ nghe nói về nó trước đây." # TL
