# : Translation updated at 2022-02-18 12:36

translate vietnamese strings:

    # game/events/endings/credits.rpy:3
    old "You perished.\n\n{small}What remains of you is just some insignificant remnants in the endless river of time.{/small}"
    new "Bạn đã chết.\n\n{small}Những gì còn lại của bạn chỉ là những tàn tích không đáng kể trong dòng sông thời gian vô tận.{/small}" # TL

    # game/events/endings/credits.rpy:3
    old "Thank you for playing Astral Lust!"
    new "Cảm ơn bạn đã chơi Astral Lust!" # TL

    # game/events/endings/credits.rpy:57
    old "Prefix"
    new "Tiền tố" # TL # Victorius - AI

    # game/events/endings/credits.rpy:57
    old "Suffix"
    new "Hậu tố" # TL # Victorius - AI
