# : Translation updated at 2021-12-24 07:09

# game/characters/alice/ev_14.rpy:12
translate vietnamese alice_14_c61c6c36:

    # alice "I'm in, but I think we should try something new."
    alice "Em sẽ tham gia, nhưng em nghĩ chúng ta nên thử một cái gì đó mới."

# game/characters/alice/ev_14.rpy:13
translate vietnamese alice_14_a55068a2:

    # me "What do you mean exactly?"
    me "Chính xác là em muốn gì?"

# game/characters/alice/ev_14.rpy:18
translate vietnamese alice_14_081b15bf:

    # alice "Do you really think I don't know about your dungeon thing?"
    alice "Anh thực sự nghĩ rằng em không biết về thứ trong Ngục tối của anh sao?"

# game/characters/alice/ev_14.rpy:19
translate vietnamese alice_14_3820771a:

    # me "..."
    me "..."

# game/characters/alice/ev_14.rpy:25
translate vietnamese alice_14_11d28569:

    # alice "You even locked someone there.. Not sure if I want to know what you do with your prisoners."
    alice "Anh thậm chí còn nhốt ai đó ở đấy cơ... em còn không chắc là mình có muốn biết anh làm gì với tù nhân của anh không nữa."

# game/characters/alice/ev_14.rpy:27
translate vietnamese alice_14_ad31009f:

    # alice "And really, you don't have better ways to use your materials?"
    alice "Và thật luôn, Anh không có cách nào tốt hơn để sử dụng tài nguyên của mình à?"

# game/characters/alice/ev_14.rpy:28
translate vietnamese alice_14_402f0fee:

    # alice "You've even built a playroom. A fucking playroom, [me]."
    alice "Anh thậm chí đã xây hẳn một cái phòng tình yêu. Một cái phòng tình yêu chết tiệt đấy [me]."

# game/characters/alice/ev_14.rpy:33
translate vietnamese alice_14_754668c5:

    # alice "And the worst part? You didn't invite ME!"
    alice "Và phần tồi tệ nhất á? Anh còn không thèm mời EM!"

# game/characters/alice/ev_14.rpy:34
translate vietnamese alice_14_0873721e:

    # alice "Me, your fucking girlfriend, [me]!"
    alice "Em, bạn gái của anh đấy [me]!"

# game/characters/alice/ev_14.rpy:39
translate vietnamese alice_14_4f7233d1:

    # alice "Don't you know I like to play with you?"
    alice "Anh không biết em muốn chơi với anh thế nào hả?"

# game/characters/alice/ev_14.rpy:40
translate vietnamese alice_14_9e42975d:

    # me "Uh.. Wow.."
    me "Uh... Wow..."

# game/characters/alice/ev_14.rpy:45
translate vietnamese alice_14_a289625b:

    # alice "So are you going to finally take me there?"
    alice "Thế anh sẽ đưa em đến đó chứ?"

# game/characters/alice/ev_14.rpy:57
translate vietnamese alice_14_40fc69f3:

    # alice "Hmph! Whatever, go have sex with your hand, instead."
    alice "Hmph! Sao cũng được, đi làm bạn với tay phải của mình thay vì em đi."

# game/characters/alice/ev_14.rpy:65
translate vietnamese alice_14_part2_d180988e:

    # alice "Great."
    alice "Tuyệt quá."

# game/characters/alice/ev_14.rpy:72
translate vietnamese alice_14_part2_ec4e92c2:

    # alice "So.."
    alice "Thế..."

# game/characters/alice/ev_14.rpy:73
translate vietnamese alice_14_part2_f660d172:

    # me "So?"
    me "Thế?"

# game/characters/alice/ev_14.rpy:74
translate vietnamese alice_14_part2_f85609cd:

    # alice "What are we going to do?"
    alice "Chúng ta sẽ làm gì?"

# game/characters/alice/ev_14.rpy:75
translate vietnamese alice_14_part2_237582ec:

    # me "Take a guess."
    me "Đoán xem nào."

# game/characters/alice/ev_14.rpy:80
translate vietnamese alice_14_part2_8d7fd32f:

    # alice "I would like to do something, we didn't do before."
    alice "Em muốn làm điều gì đó mà chúng ta đã chưa từng làm trước đây."

# game/characters/alice/ev_14.rpy:96
translate vietnamese alice_14_part2_da61283f:

    # alice "Something that will make me lose myself and scream your name. Something that will throw me out of my feet."
    alice "Điều gì đó sẽ khiến em đánh mất bản thân và hét lên tên của anh chẳng hạn."

# game/characters/alice/ev_14.rpy:97
translate vietnamese alice_14_part2_a9d7bf98:

    # me "I think I got an idea.."
    me "Anh nghĩ là anh có một ý này."

# game/characters/alice/ev_14.rpy:102
translate vietnamese alice_14_part2_c7e67419:

    # alice "What is it?"
    alice "Và đấy là gì?"

# game/characters/alice/ev_14.rpy:103
translate vietnamese alice_14_part2_cadd1e0c:

    # me "Hehe, that's a surprise.."
    me "Hêhê, đó sẽ là một sự bất ngờ..."

# game/characters/alice/ev_14.rpy:108
translate vietnamese alice_14_part2_9e40943f:

    # alice "I love surprises."
    alice "Em thích những điều bất ngờ."

# game/characters/alice/ev_14.rpy:109
translate vietnamese alice_14_part2_1e9b6953:

    # me "Then put some trust in me, and close your eyes, don't open them before I tell you to."
    me "thế thì, hãy đặt niềm tin vào Anh, và nhắm mắt lại, đừng mở ra trước khi anh nói với em."

# game/characters/alice/ev_14.rpy:114
translate vietnamese alice_14_part2_4b95af6f:

    # alice "Alright, I trust you, [me]. Do whatever you want with me."
    alice "Được rồi, em tin anh đấy. Làm bất cứ điều gì anh muốn với em đi."

# game/characters/alice/ev_14.rpy:115
translate vietnamese alice_14_part2_3352070d:

    # me "Haha, don't tempt me."
    me "Haha, đừng dụ dỗ anh."

# game/characters/alice/ev_14.rpy:116
translate vietnamese alice_14_part2_f38d9210:

    # me "Let's start with undressing you..."
    me "Hãy bắt đầu với việc cởi quần áo cho em nào..."

# game/characters/alice/ev_14.rpy:120
translate vietnamese alice_14_part2_d009a09d:

    # alice "Cold.. What is it?"
    alice "Lạnh quá... Cái gì vậy?"

# game/characters/alice/ev_14.rpy:121
translate vietnamese alice_14_part2_5a013792:

    # me "Patience."
    me "Kiên nhẫn nào."

# game/characters/alice/ev_14.rpy:122
translate vietnamese alice_14_part2_64d2cba3:

    # alice "And why did you bind my hands?"
    alice "Và tại sao anh lại trói tay em?"

# game/characters/alice/ev_14.rpy:123
translate vietnamese alice_14_part2_f96fadd5:

    # me "Are you scared?"
    me "Em sợ hả?"

# game/characters/alice/ev_14.rpy:124
translate vietnamese alice_14_part2_292612fa:

    # alice "A little, it feels weird."
    alice "Một chút, em cảm thấy khá kỳ lạ."

# game/characters/alice/ev_14.rpy:125
translate vietnamese alice_14_part2_48ddd1ae:

    # me "You wanted something new."
    me "Em muốn một cái gì đó mới mẻ đúng không?"

# game/characters/alice/ev_14.rpy:126
translate vietnamese alice_14_part2_82bdeeda:

    # me "Alright, open your eyes."
    me "Được rồi, mở mắt ra đi."

# game/characters/alice/ev_14.rpy:130
translate vietnamese alice_14_part2_40325204:

    # alice "Oh? What is it?"
    alice "Ồ? Đây là cái gì vậy?"

# game/characters/alice/ev_14.rpy:131
translate vietnamese alice_14_part2_a6b77de4:

    # me "It's called wooden horse."
    me "Nó được gọi là ngựa gỗ."

# game/characters/alice/ev_14.rpy:135
translate vietnamese alice_14_part2_a915c093:

    # alice "And, ummm.. What's its use?"
    alice "Và, ummm... Công dụng của nó là gì?"

# game/characters/alice/ev_14.rpy:136
translate vietnamese alice_14_part2_9ca4bd2b:

    # me "You're light, so it might not be obvious yet, but worry not, you'll figure it out soon enough."
    me "em khá là nhẹ nên có thể chưa thấy gì hết, nhưng đừng lo, em sẽ sớm nhận ra thôi."

# game/characters/alice/ev_14.rpy:137
translate vietnamese alice_14_part2_2fe6da5c:

    # alice "Oh? My weight matters?"
    alice "Oh? Cân nặng của em cũng quan trọng á?"

# game/characters/alice/ev_14.rpy:141
translate vietnamese alice_14_part2_0f4b199c:

    # alice "I think I've figured it out."
    alice "Em nghĩ là mình đã hiểu rồi."

# game/characters/alice/ev_14.rpy:142
translate vietnamese alice_14_part2_1dbe2ddc:

    # me "Good."
    me "Tốt."

# game/characters/alice/ev_14.rpy:146
translate vietnamese alice_14_part2_28d63b25:

    # alice "If I'm to be honest, being bound by you like this.. is hot."
    alice "Để mà nói thật thì, bị trói bởi anh như thế này cũng... khá là hot đấy."

# game/characters/alice/ev_14.rpy:147
translate vietnamese alice_14_part2_a68f7064:

    # me "Believe me, the hot part is only beginning. Time for you to move your hips."
    me "Tin anh đi, phần nóng bỏng chỉ mới bắt đầu thôi. Đã đến lúc em phải di chuyển hông rồi."

# game/characters/alice/ev_14.rpy:148
translate vietnamese alice_14_part2_24b7b2f8:

    # alice "Mm~"
    alice "Mm~"

translate vietnamese strings:

    # game/characters/alice/ev_14.rpy:47
    old "How could I reject a lady?"
    new "Làm sao anh có thể từ chối một Quý cô được?"

    # game/characters/alice/ev_14.rpy:47
    old "Nah"
    new "Không"
