# TODO: Translation updated at 2021-12-24 07:09

# game/characters/alice/ev_7.rpy:10
translate vietnamese alice_7_cf77ba87:

    # me "What's up?"
    me "Có chuyện gì vậy?"

# game/characters/alice/ev_7.rpy:15
translate vietnamese alice_7_77040e8e:

    # alice "I'm fine, just bored. What about you? Anything interesting going on?"
    alice "Em không sao, chỉ là hơi chán thôi. Còn anh thì sao? Có gì thú vị hả?"

# game/characters/alice/ev_7.rpy:21
translate vietnamese alice_7_459f9370:

    # alice "Except the thing with Grace."
    alice "Ngoại trừ chuyện với Grace."

# game/characters/alice/ev_7.rpy:27
translate vietnamese alice_7_37214622:

    # "You tell her about your journey."
    "Bạn kể cho cô ấy nghe về cuộc hành trình của mình."

# game/characters/alice/ev_7.rpy:28
translate vietnamese alice_7_8a7cdd22:

    # "After five minutes."
    "Sau năm phút."

# game/characters/alice/ev_7.rpy:31
translate vietnamese alice_7_52e67404:

    # alice "Boooring."
    alice "Nhàm chán."

# game/characters/alice/ev_7.rpy:32
translate vietnamese alice_7_e5276807:

    # alice "I don't want to hear about you doing fun things, I want to do them myself."
    alice "Em không muốn nghe về việc anh làm mấy cái đấy, em muốn tự mình làm chúng."

# game/characters/alice/ev_7.rpy:33
translate vietnamese alice_7_8177c361:

    # me "You can go out with me."
    me "Em có thể ra ngoài với anh."

# game/characters/alice/ev_7.rpy:37
translate vietnamese alice_7_3de9f756:

    # alice "To fight with monsters? No thanks."
    alice "Để chiến đấu với quái vật á? Không cám ơn."

# game/characters/alice/ev_7.rpy:40
translate vietnamese alice_7_9f6393d7:

    # alice "I've got only one life, unlike you."
    alice "Em chỉ có một mạng thôi, không giống như anh."

# game/characters/alice/ev_7.rpy:42
translate vietnamese alice_7_33e1c77b:

    # alice "I've got only one life."
    alice "Em chỉ có một cái mạng thôi."

# game/characters/alice/ev_7.rpy:50
translate vietnamese alice_7_542ad362:

    # alice "So I'm not the only one, huh?"
    alice "Thế thì, em không phải là người duy nhất, huh?"

# game/characters/alice/ev_7.rpy:58
translate vietnamese alice_7_1_fdc44ce5:

    # alice "You know what? I've got an idea. Let's play a game."
    alice "Anh biết gì không? Em có ý này nè. Hãy chơi 1 trò chơi đi."

# game/characters/alice/ev_7.rpy:59
translate vietnamese alice_7_1_c2739e5a:

    # me "I'm all ears."
    me "Anh đang nghe đây."

# game/characters/alice/ev_7.rpy:64
translate vietnamese alice_7_1_4e64e798:

    # alice "I'll ask you a question, and if you answer correctly I'll give you something."
    alice "Em sẽ hỏi anh một câu hỏi và nếu anh trả lời đúng, em sẽ cho anh một thứ."

# game/characters/alice/ev_7.rpy:65
translate vietnamese alice_7_1_8a8496e8:

    # me "What if I'm wrong?"
    me "Nếu anh sai thì sao?"

# game/characters/alice/ev_7.rpy:70
translate vietnamese alice_7_1_de3ca9d7:

    # alice "Naturally you would have to give me something."
    alice "Đương nhiên anh sẽ phải cho em một cái gì đó rồi."

# game/characters/alice/ev_7.rpy:71
translate vietnamese alice_7_1_024fd80c:

    # th "These questions are going to be hard.."
    th "Những câu hỏi này sẽ khó đấy..."

# game/characters/alice/ev_7.rpy:73
translate vietnamese alice_7_1_f5489ffd:

    # alice "Even though it's not very fun at least I'll get something out of it."
    alice "Mặc dù nó không vui lắm nhưng ít nhất em sẽ lấy được thứ gì đó từ nó."

# game/characters/alice/ev_7.rpy:74
translate vietnamese alice_7_1_451dd1cd:

    # th "Very hard."
    th "Rất khó."

# game/characters/alice/ev_7.rpy:76
translate vietnamese alice_7_1_f6971219:

    # alice "So, are you in?"
    alice "Vậy anh có chơi không?"

# game/characters/alice/ev_7.rpy:77
translate vietnamese alice_7_1_1420afa3:

    # me "If I answer wrong, are you going to ask the same question twice?"
    me "Nếu Anh trả lời sai, em có hỏi cùng một câu hỏi hai lần không?"

# game/characters/alice/ev_7.rpy:78
translate vietnamese alice_7_1_1d67529c:

    # alice "Nope. Well, let's say I'll ask you eight different questions. If you answer incorrectly, I'll ask the next one."
    alice "Không. Giả sử Em sẽ hỏi Anh tám câu hỏi khác nhau. Nếu anh trả lời sai, em sẽ hỏi câu tiếp theo."

# game/characters/alice/ev_7.rpy:79
translate vietnamese alice_7_1_b0e73989:

    # me "How much do you have on you?"
    me "em có bao nhiêu thứ trên người?"

# game/characters/alice/ev_7.rpy:80
translate vietnamese alice_7_1_2d2d65af:

    # alice "I have only two items on me."
    alice "Em chỉ có hai món đồ trên người thôi."

# game/characters/alice/ev_7.rpy:81
translate vietnamese alice_7_1_3cc01e00:

    # me "What if you run out of stuff?"
    me "Điều gì sẽ xảy ra nếu em hết đồ để chơi?"

# game/characters/alice/ev_7.rpy:82
translate vietnamese alice_7_1_df3767c2:

    # alice "Pff~ You wish."
    alice "Pff~ Mơ đi."

# game/characters/alice/ev_7.rpy:83
translate vietnamese alice_7_1_51ba2fbd:

    # th "Seems I'll win if I answer three questions."
    th "Có vẻ như mình sẽ thắng nếu mình trả lời được ba câu hỏi."

# game/characters/alice/ev_7.rpy:89
translate vietnamese al_7_me_74a950c2:

    # alice "Good!"
    alice "Tốt!"

# game/characters/alice/ev_7.rpy:95
translate vietnamese al_7_me_b82fe66a:

    # alice "Don't be a pussy."
    alice "Đừng có làm một thằng đàn bà nha."

# game/characters/alice/ev_7.rpy:96
translate vietnamese al_7_me_1d288b67:

    # me "You're a pussy."
    me "Đấy là em mà."

# game/characters/alice/ev_7.rpy:99
translate vietnamese al_7_me_35b037b1:

    # alice "I am. A very cute one at that."
    alice "Vì em rất dễ thương."

# game/characters/alice/ev_7.rpy:100
translate vietnamese al_7_me_227724ba:

    # me "Can't deny."
    me "Haha."

# game/characters/alice/ev_7.rpy:103
translate vietnamese al_7_me_23e68f81:

    # alice "Find me again when you change your mind."
    alice "Tìm em khi anh thay đổi quyết định."

# game/characters/alice/ev_7.rpy:104
translate vietnamese al_7_me_fca811a5:

    # me "Sure."
    me "Được rồi."

# game/characters/alice/ev_7.rpy:112
translate vietnamese al_7_me_80adbec5:

    # alice "You wish!"
    alice "Cứ mơ đi!"

# game/characters/alice/ev_7.rpy:115
translate vietnamese al_7_me_cfe76ef9:

    # alice "You can give me one, though."
    alice "Anh có thể cho em luôn cũng được mà."

# game/characters/alice/ev_7.rpy:121
translate vietnamese al_7_me_346b45ff:

    # alice "Mmmn~"
    alice "Mmmn~"

# game/characters/alice/ev_7.rpy:122
translate vietnamese al_7_me_2ee74ceb:

    # "You spend a moment enjoying each other's lips."
    "Bạn dành một chút thời gian để tận hưởng đôi môi của nhau."

# game/characters/alice/ev_7.rpy:126
translate vietnamese al_7_me_ed012367:

    # alice "Tasty~"
    alice "Ngon~"

# game/characters/alice/ev_7.rpy:129
translate vietnamese al_7_me_d139861c:

    # alice "Are you playing with me?"
    alice "Anh đang chơi đùa với em hả?"

# game/characters/alice/ev_7.rpy:136
translate vietnamese al_7_me_76117695:

    # alice "Ahahaha~ So you're a gigolo now?"
    alice "Ahahaha~ Vậy bây giờ anh là trai bao hả?"

# game/characters/alice/ev_7.rpy:137
translate vietnamese al_7_me_3820771a:

    # me "..."
    me "..."

# game/characters/alice/ev_7.rpy:156
translate vietnamese al_7_questions_78b65674:

    # alice "Come back when you have something to give me."
    alice "Hãy quay lại khi anh có thứ gì đó để đưa cho em."

# game/characters/alice/ev_7.rpy:197
translate vietnamese al_7_questions_00f07480:

    # alice "What goes up but never ever comes down? To be clear, I don't mean your dick."
    alice "Cái gì đi lên nhưng không bao giờ đi xuống? Nói rõ hơn, ý em không phải là thằng nhỏ của anh đâu."

# game/characters/alice/ev_7.rpy:198
translate vietnamese al_7_questions_40d9d377:

    # me "Ha. Ha. Ha. Very funny.."
    me "Ha. Ha. Ha. Hài hước lắm..."

# game/characters/alice/ev_7.rpy:209
translate vietnamese al_7_questions_8da92649:

    # alice "What is always coming, but never arrives?"
    alice "Có bao nhiêu chữ C trong câu sau đây: “ Cơm, canh, cháo gì tớ cũng thích ăn?"

# game/characters/alice/ev_7.rpy:220
translate vietnamese al_7_questions_14137979:

    # alice "What kind of tree can you carry in your hand?"
    alice "Đỉnh Fansipan cao bao nhiêu m?"

# game/characters/alice/ev_7.rpy:231
translate vietnamese al_7_questions_6671e4cd:

    # alice "Some months have 31 days, others have 30 days. How many have 28 days?"
    alice "Một vài tháng có 31 ngày, những tháng khác có 30 ngày. Có bao nhiêu tháng có 28 ngày?"

# game/characters/alice/ev_7.rpy:242
translate vietnamese al_7_questions_d5b6cc22:

    # alice "Which room has no walls?"
    alice "Nhà Nam có 4 anh chị em, 3 đứa lớn tên là Xuân, Hạ, Thu. Vậy người em út tên gì?"

# game/characters/alice/ev_7.rpy:253
translate vietnamese al_7_questions_d2e18c69:

    # alice "I will go easy on you, you're not good at this."
    alice "Em sẽ dễ dàng hơn với anh, anh không giỏi trong việc này."

# game/characters/alice/ev_7.rpy:254
translate vietnamese al_7_questions_2eb0c5cf:

    # alice "Solve: 2 + 2 * 2"
    alice "Giải: 2 + 2 * 2"

# game/characters/alice/ev_7.rpy:265
translate vietnamese al_7_questions_a80bbf8f:

    # alice "In what year did World War II begin?"
    alice "Chiến tranh thế giới thứ hai bắt đầu vào năm nào?"

# game/characters/alice/ev_7.rpy:276
translate vietnamese al_7_questions_2a9acec4:

    # alice "Solve: 1 + 1, it isn't three."
    alice "Giải: 1 + 1, nó không phải là ba."

# game/characters/alice/ev_7.rpy:277
translate vietnamese al_7_questions_a1e9073c:

    # me "Really?"
    me "Thật luôn?"

# game/characters/alice/ev_7.rpy:278
translate vietnamese al_7_questions_e5aef61a:

    # alice "Yeah, I started to pity you a while ago."
    alice "Yeah, em đã bắt đầu cảm thấy thương hại anh rồi đấy."

# game/characters/alice/ev_7.rpy:279
translate vietnamese al_7_questions_3820771a:

    # me "..."
    me "..."

# game/characters/alice/ev_7.rpy:294
translate vietnamese al_7_questions_7488bf42:

    # alice "I told you it's not the answer.."
    alice "Em đã nói với anh đó không phải là câu trả lời..."

# game/characters/alice/ev_7.rpy:299
translate vietnamese al_7_questions_d601de8b:

    # alice "You are hopeless."
    alice "Anh đúng là vô vọng rồi."

# game/characters/alice/ev_7.rpy:302
translate vietnamese al_7_questions_3b5a6acf:

    # alice "Repeat: Help me."
    alice "Lặp lại theo em: hentaiz.net."

# game/characters/alice/ev_7.rpy:303
translate vietnamese al_7_questions_3820771a_1:

    # me "..."
    me "..."

# game/characters/alice/ev_7.rpy:304
translate vietnamese al_7_questions_74f05a4e:

    # alice "..."
    alice "..."

# game/characters/alice/ev_7.rpy:305
translate vietnamese al_7_questions_3820771a_2:

    # me "..."
    me "..."

# game/characters/alice/ev_7.rpy:306
translate vietnamese al_7_questions_74f05a4e_1:

    # alice "..."
    alice "..."

# game/characters/alice/ev_7.rpy:322
translate vietnamese al_7_questions_win_2c72d8e0:

    # alice "That's right."
    alice "Đúng rồi."

# game/characters/alice/ev_7.rpy:332
translate vietnamese al_7_questions_win_5a38e81b:

    # me "Ohhh, what's up with this face? Are you alright?"
    me "Ỏoo, sao lại chưng cái bộ mặt này ra vậy? Em ổn chứ?"

# game/characters/alice/ev_7.rpy:333
translate vietnamese al_7_questions_win_74f05a4e:

    # alice "..."
    alice "..."

# game/characters/alice/ev_7.rpy:334
translate vietnamese al_7_questions_win_9cfe3d82:

    # alice "Take what you want."
    alice "Lấy những gì anh muốn đi."

# game/characters/alice/ev_7.rpy:338
translate vietnamese al_7_questions_win_c4d4e13a:

    # alice "Next time it will be you paying me."
    alice "Lần sau sẽ là anh trả cho cho em."

# game/characters/alice/ev_7.rpy:344
translate vietnamese al_7_questions_fail_1fd41872:

    # alice "Wroooong~"
    alice "Saiiiiiii~"

# game/characters/alice/ev_7.rpy:345
translate vietnamese al_7_questions_fail_c168eeb4:

    # me "What? What is the answer?"
    me "Gì? Câu trả lời là gì?"

# game/characters/alice/ev_7.rpy:348
translate vietnamese al_7_questions_fail_f2a02951:

    # alice "Not telling you. Pay up."
    alice "Không nói với anh đâu. moi đồ ra đi."

# game/characters/alice/ev_7.rpy:350
translate vietnamese al_7_questions_fail_6ae81f13:

    # alice "Lose like this some more and I'll go easy on you."
    alice "Thua như thế này vài ván nữa và em sẽ dễ dàng hơn với anh."

# game/characters/alice/ev_7.rpy:351
translate vietnamese al_7_questions_fail_23be24fb:

    # me "Thanks.."
    me "Cảm ơn..."

# game/characters/alice/ev_7.rpy:358
translate vietnamese al_7_questions_reward_1be97641:

    # me "What's up with this face?"
    me "Có chuyện gì với bản mặt này vậy?"

# game/characters/alice/ev_7.rpy:359
translate vietnamese al_7_questions_reward_ca698b98:

    # alice "I kind of.. don't have anything else to give you."
    alice "em... không có bất cứ thứ gì khác để đưa cho anh rồi."

# game/characters/alice/ev_7.rpy:360
translate vietnamese al_7_questions_reward_989dc348:

    # me "Hahaha, you broke already?"
    me "Hahaha, em chưa gì đã phá sản rồi à?"

# game/characters/alice/ev_7.rpy:363
translate vietnamese al_7_questions_reward_74f05a4e:

    # alice "..."
    alice "..."

# game/characters/alice/ev_7.rpy:364
translate vietnamese al_7_questions_reward_ffc53b55:

    # me "Pay up."
    me "Nào."

# game/characters/alice/ev_7.rpy:365
translate vietnamese al_7_questions_reward_74f05a4e_1:

    # alice "..."
    alice "..."

# game/characters/alice/ev_7.rpy:370
translate vietnamese al_7_questions_reward_6ee5cf18:

    # alice "Let's go to my room."
    alice "Hãy về phòng của em đi."

# game/characters/alice/ev_7.rpy:371
translate vietnamese al_7_questions_reward_8dd7b859:

    # me "Oh, paying in nature are you?"
    me "Oh, trả bằng vốn tự có hả em?"

# game/characters/alice/ev_7.rpy:376
translate vietnamese al_7_questions_reward_53aafa34:

    # alice "Do you want your reward or not?"
    alice "Anh có muốn phần thưởng của mình hay không?"

# game/characters/alice/ev_7.rpy:377
translate vietnamese al_7_questions_reward_eb294d75:

    # me "Of course I want it."
    me "Muốn chứ muốn chứ."

# game/characters/alice/ev_7.rpy:382
translate vietnamese al_7_questions_reward_0d64bbfe:

    # "You have arrived at her room."
    "Bạn đã đến phòng của cô ấy."

# game/characters/alice/ev_7.rpy:386
translate vietnamese al_7_questions_reward_dcaec663:

    # alice "Undress."
    alice "Cởi đồ đi."

# game/characters/alice/ev_7.rpy:388
translate vietnamese al_7_questions_reward_58fcd08c:

    # "You do as you're told."
    "Bạn làm như cô ấy đã nói."

# game/characters/alice/ev_7.rpy:389
translate vietnamese al_7_questions_reward_98de0dd2:

    # me "What is it going to be this time? A blowjob again?"
    me "Lần này sẽ là gì? Blowjob nữa à?"

# game/characters/alice/ev_7.rpy:393
translate vietnamese al_7_questions_reward_6a58dab5:

    # alice "No, I thought of something else. Lie down."
    alice "Không, em đã nghĩ đến một thứ khác. Nằm xuống."

# game/characters/alice/ev_7.rpy:409
translate vietnamese al_7_questions_reward_dcaec663_1:

    # alice "Undress."
    alice "Cởi quần áo."

# game/characters/alice/ev_7.rpy:411
translate vietnamese al_7_questions_reward_58fcd08c_1:

    # "You do as you're told."
    "Bạn làm như cô ấy đã nói."

# game/characters/alice/ev_7.rpy:412
translate vietnamese al_7_questions_reward_98de0dd2_1:

    # me "What is it going to be this time? A blowjob again?"
    me "Lần này sẽ là gì? Blowjob nữa à?"

# game/characters/alice/ev_7.rpy:416
translate vietnamese al_7_questions_reward_6a58dab5_1:

    # alice "No, I thought of something else. Lie down."
    alice "Không, em đã nghĩ ra một thứ khác rồi. Nằm xuống."

# game/characters/alice/ev_7.rpy:433
translate vietnamese al_7_questions_ask_c13e1162:

    # alice "Do you want to play again?"
    alice "Anh có muốn chơi lại không?"

# game/characters/alice/ev_7.rpy:441
translate vietnamese al_7_questions_ask_2cb774de:

    # alice "Lost faith in yourself already?"
    alice "Chưa gì đã mất niềm tin vào bản thân rồi à?"

# game/characters/alice/ev_7.rpy:442
translate vietnamese al_7_questions_ask_39888bac:

    # me "No, I'm just giving you time to prepare some rewards."
    me "Không, anh chỉ cho em thời gian để chuẩn bị phần thưởng thôi."

# game/characters/alice/ev_7.rpy:443
translate vietnamese al_7_questions_ask_6712f3de:

    # alice "Sure~"
    alice "Phải rồi~"

# game/characters/alice/ev_7.rpy:444
translate vietnamese al_7_questions_ask_69e73332:

    # "You leave her."
    "Bạn bỏ cô ấy lại."

translate vietnamese strings:

    # game/characters/alice/ev_7.rpy:23
    old "Tell her stories of your adventures"
    new "Kể cho cô ấy nghe những câu chuyện về cuộc phiêu lưu của bạn"

    # game/characters/alice/ev_7.rpy:23
    old "Nothing interesting"
    new "Không có gì thú vị"

    # game/characters/alice/ev_7.rpy:85
    old "Let's try"
    new "Hãy thử nào"

    # game/characters/alice/ev_7.rpy:85
    old "Maybe some other time"
    new "Có lẽ lúc khác"

    # game/characters/alice/ev_7.rpy:85
    old "Can I pay with a kiss"
    new "Em có thể trả bằng một nụ hôn không"

    # game/characters/alice/ev_7.rpy:116
    old "Kiss her"
    new "Hôn"

    # game/characters/alice/ev_7.rpy:116
    old "Not for free"
    new "Không miễn phí đâu"
# TODO: Translation updated at 2022-02-06 08:10

translate vietnamese strings:

    # game/characters/alice/ev_7.rpy:85
    old "Can I pay with a kiss?"
    new "Anh có thể trả bằng một nụ hôn không?"

    # game/characters/alice/ev_7.rpy:204
    old "age"
    new "tuoi"

    # game/characters/alice/ev_7.rpy:215
    old "tomorrow"
    new "1"

    # game/characters/alice/ev_7.rpy:226
    old "palm"
    new "3141m"

    # game/characters/alice/ev_7.rpy:237
    old "all"
    new "12"

    # game/characters/alice/ev_7.rpy:260
    old "six"
    new "6"

    # game/characters/alice/ev_7.rpy:271
    old "1939"
    new "1939"

    # game/characters/alice/ev_7.rpy:285
    old "two"
    new "2"

    # game/characters/alice/ev_7.rpy:289
    old "three"
    new "3"

    # game/characters/alice/ev_7.rpy:312
    old "help me"
    new "hentaiz.net"

# TODO: Translation updated at 2022-02-06 09:10

translate vietnamese strings:

    # game/characters/alice/ev_7.rpy:281
    old "Solve: 1 + 1"
    new "Giải: 1 + 1"

    # game/characters/alice/ev_7.rpy:308
    old "Repeat: Help me."
    new "Nói: hentaiz.net."

# TODO: Translation updated at 2022-02-06 09:15

translate vietnamese strings:

    # game/characters/alice/ev_7.rpy:200
    old "What goes up but never ever comes down?"
    new "Cái gì đi lên nhưng không bao giờ đi xuống?(viết không dấu)"

    # game/characters/alice/ev_7.rpy:211
    old "What is always coming, but never arrives?"
    new "Có bao nhiêu chữ C trong câu sau đây: Cơm, canh, cháo gì tớ cũng thích ăn?"

    # game/characters/alice/ev_7.rpy:222
    old "What kind of tree can you carry in your hand?"
    new "Đỉnh Fansipan cao bao nhiêu m?"

    # game/characters/alice/ev_7.rpy:233
    old "Some months have 31 days, others have 30 days. How many have 28 days?"
    new "Một năm sẽ vài tháng có 31 ngày, những tháng khác có 30 ngày. Có bao nhiêu tháng có 28 ngày?"

    # game/characters/alice/ev_7.rpy:244
    old "Which room has no walls?"
    new "Nhà Nam có 4 anh chị em, 3 đứa lớn tên là Xuân, Hạ, Thu. Vậy người em út tên gì?"

    # game/characters/alice/ev_7.rpy:256
    old "Solve: 2 + 2 * 2"
    new "Gải: 2 + 2 * 2"

    # game/characters/alice/ev_7.rpy:267
    old "What year had World War II began?"
    new "Chiến tranh thế giới thứ hai bắt đầu vào năm nào?"

# TODO: Translation updated at 2022-02-06 09:30

translate vietnamese strings:

    # game/characters/alice/ev_7.rpy:248
    old "mushroom"
    new "Nấm"
