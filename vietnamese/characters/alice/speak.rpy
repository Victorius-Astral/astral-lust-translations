# : Translation updated at 2021-12-24 07:09

# game/characters/alice/speak.rpy:9
translate vietnamese speak_alice_56062ced:

    # alice "Hey."
    alice "Chào."

# game/characters/alice/speak.rpy:14
translate vietnamese speak_alice_ab868826:

    # alice "Hey.."
    alice "Chào..."

# game/characters/alice/speak.rpy:19
translate vietnamese speak_alice_6688f373:

    # alice "Heya~"
    alice "Chào anh~"

# game/characters/alice/speak.rpy:41
translate vietnamese speak_alice_menu1_2efa8c0a:

    # alice "I'm feeling sick. I think I caught something when I was out scavenging."
    alice "Em đang thấy buồn nôn quá. Em nghĩ rằng mình đã bị bệnh khi em đang đi ra ngoài kiếm đồ rồi."

# game/characters/alice/speak.rpy:42
translate vietnamese speak_alice_menu1_a9f53024:

    # alice "I feel like I'm dying.."
    alice "Em cảm thấy như là chết rồi..."

# game/characters/alice/speak.rpy:46
translate vietnamese speak_alice_menuev9_e9077839:

    # me "Take this."
    me "Dùng cái này đi."

# game/characters/alice/speak.rpy:47
translate vietnamese speak_alice_menuev9_3049d90d:

    # alice "Thank you. It should help."
    alice "Cảm ơn anh. Nó sẽ giúp em đỡ hơn đấy."

# game/characters/alice/speak.rpy:50
translate vietnamese speak_alice_menuev9_aa68854e:

    # th "I should check on her in a few days to see if she's feeling better."
    th "Mình nên kiểm tra cô ấy sau một vài ngày để xem cô ấy có cảm thấy tốt hơn không."

# game/characters/alice/speak.rpy:55
translate vietnamese speak_alice_menuev9_f4e5d282:

    # "You don't have any antibiotics."
    "Bạn không có bất kỳ loại thuốc kháng sinh nào."

# game/characters/alice/speak.rpy:65
translate vietnamese speak_alice_menuev9_71e87485:

    # "You've used your powers to heal her."
    "Bạn đã sử dụng sức mạnh của mình để chữa lành cho cô ấy."

# game/characters/alice/speak.rpy:66
translate vietnamese speak_alice_menuev9_5d29181d:

    # alice "Weird. I feel better.. What's happening?"
    alice "Kỳ lạ. Em cảm thấy tốt hơn rồi... Chuyện gì vừa xảy ra vậy?"

# game/characters/alice/speak.rpy:67
translate vietnamese speak_alice_menuev9_cb647bc0:

    # me "It's something I've found outside."
    me "Đó là thứ anh đã tìm thấy bên ngoài."

# game/characters/alice/speak.rpy:68
translate vietnamese speak_alice_menuev9_e49d366a:

    # alice "Oh, useful."
    alice "Ồ, hữu ích đấy."

# game/characters/alice/speak.rpy:69
translate vietnamese speak_alice_menuev9_bfd41d70:

    # me "Yeah, how are you feeling?"
    me "Yeah, em cảm thấy thế nào?"

# game/characters/alice/speak.rpy:73
translate vietnamese speak_alice_menuev9_e0f3c572:

    # alice "Better, it'll probably take a while to recover fully. Thank you."
    alice "Tốt hơn, có lẽ sẽ mất một khoảng thời gian để phục hồi hoàn toàn. Cảm ơn anh."

# game/characters/alice/speak.rpy:74
translate vietnamese speak_alice_menuev9_a62c2570:

    # th "I should check on her tomorrow if she's alright."
    th "Mình nên kiểm tra em ấy vào ngày mai nếu em ấy ổn."

# game/characters/alice/speak.rpy:78
translate vietnamese speak_alice_menuev9_15e07f7a:

    # "You don't have any usable ability. She can be healed with Healing, Greater Healing and Antidote."
    "Bạn không có bất kỳ khả năng sử dụng được. Cô ấy có thể được chữa lành bằng Hồi máu, Hồi phục trung cấp và Kháng thể."

# game/characters/alice/speak.rpy:82
translate vietnamese speak_alice_menuev9_07df25ec:

    # alice "Thank you, I wish that your wishes had any power."
    alice "Cảm ơn anh, em ước là điều ước của anh có sức mạnh."

# game/characters/alice/speak.rpy:83
translate vietnamese speak_alice_menuev9_cdf96723:

    # th "Maybe I can find antibiotics for her somewhere."
    th "Có lẽ mình có thể tìm thấy thuốc kháng sinh cho em ấy ở đâu đó."

# game/characters/alice/speak.rpy:92
translate vietnamese speak_alice_menuev9_e5f2fbf0:

    # alice "Ready to lose?"
    alice "Sẵn sàng để mất chưa?"

# game/characters/alice/speak.rpy:93
translate vietnamese speak_alice_menuev9_c895d41c:

    # me "Never."
    me "Không bao giờ."

# game/characters/alice/speak.rpy:102
translate vietnamese speak_alice_menuev9_985b54f5:

    # alice "Actually I was about to watch something too, how about we watch something together?"
    alice "Thực ra em cũng định xem một cái gì đó, vậy chúng ta cùng xem với nhau được không?"

# game/characters/alice/speak.rpy:103
translate vietnamese speak_alice_menuev9_e23b4942:

    # me "Great, what do you want to watch?"
    me "Tuyệt vời, em muốn xem gì?"

# game/characters/alice/speak.rpy:104
translate vietnamese speak_alice_menuev9_df6e6d73:

    # alice "I don't have anything in mind, what about you?"
    alice "Em không nghĩ ra được gì, còn anh thì sao?"

# game/characters/alice/speak.rpy:105
translate vietnamese speak_alice_menuev9_3325d9bf:

    # me "Nah."
    me "Nah."

# game/characters/alice/speak.rpy:108
translate vietnamese speak_alice_menuev9_839ccf84:

    # alice "Then let's watch the first thing we turn on."
    alice "Vậy thì, hãy xem cái đầu tiên chúng ta bật lên vậy."

# game/characters/alice/speak.rpy:109
translate vietnamese speak_alice_menuev9_d59a6b9a:

    # me "Great."
    me "Tuyệt."

# game/characters/alice/speak.rpy:111
translate vietnamese speak_alice_menuev9_52e8344d:

    # "You two sit on the sofa and turn on the tv."
    "Hai bạn ngồi trên ghế sofa và bật tv."

# game/characters/alice/speak.rpy:114
translate vietnamese speak_alice_menuev9_3cda5fb6:

    # "You watch some drama series for a few hours, until [alice] is about to cry."
    "Bạn xem một số bộ phim drama hàn xẻng trong vài giờ, cho đến khi [alice] sắp khóc."

# game/characters/alice/speak.rpy:115
translate vietnamese speak_alice_menuev9_6ba1d500:

    # me "It's just a show."
    me "Nó chỉ là một bộ phim thôi mà."

# game/characters/alice/speak.rpy:117
translate vietnamese speak_alice_menuev9_40b2f332:

    # alice "Shut up. How can you be unmoved by this? It's so sad."
    alice "Im đi. Tại sao mà anh không bị lay động bởi điều này chứ? Nó cảm động thế cơ mà."

# game/characters/alice/speak.rpy:118
translate vietnamese speak_alice_menuev9_1ab180cd:

    # me "If you want to see something sad then look out the window."
    me "Nếu em muốn thấy điều gì đó buồn thì hãy nhìn ra cửa sổ."

# game/characters/alice/speak.rpy:120
translate vietnamese speak_alice_menuev9_c6f736dd:

    # alice "Can you not destroy the mood? It felt like before The End for a while."
    alice "Anh có thể đừng phá hủy tâm trạng của em được không?"

# game/characters/alice/speak.rpy:121
translate vietnamese speak_alice_menuev9_a8bfbc59:

    # me "Sorry for bringing you back."
    me "Xin lỗi."

# game/characters/alice/speak.rpy:123
translate vietnamese speak_alice_menuev9_231e4f45:

    # alice "To think I wanted to reward you for being a good boy."
    alice "Em nghĩ rằng em sẽ thưởng cho anh vì đã làm một cậu bé ngoan."

# game/characters/alice/speak.rpy:124
translate vietnamese speak_alice_menuev9_5d6b5979:

    # me "Wait, what? I'm very go.."
    me "Chờ đã, cái gì? Anh rất ng..."

# game/characters/alice/speak.rpy:126
translate vietnamese speak_alice_menuev9_bce84c30:

    # alice "Bye bye~"
    alice "Bye bye~"

# game/characters/alice/speak.rpy:127
translate vietnamese speak_alice_menuev9_4bd8801e:

    # "Left with no other options you leave her alone."
    "Rời đi mà không có lựa chọn nào khác, bạn để cô ấy một mình."

# game/characters/alice/speak.rpy:128
translate vietnamese speak_alice_menuev9_3ab04ce7:

    # th "Ahh.. She is teasing me again.."
    th "Ahh... Em ấy lại trêu mình rồi..."

# game/characters/alice/speak.rpy:139
translate vietnamese speak_alice_menuev9_ab32099c:

    # alice "Boooriiinggg.."
    alice "Nhàm chánnn..."

# game/characters/alice/speak.rpy:140
translate vietnamese speak_alice_menuev9_6f372945:

    # me ".. You're boring.."
    me "... Em mới chán đấy..."

# game/characters/alice/speak.rpy:149
translate vietnamese speak_alice_menuev9_cde3d046:

    # alice "Of course, daddy~"
    alice "Tất nhiên rồi~"

# game/characters/alice/speak.rpy:150
translate vietnamese speak_alice_menuev9_7952d572:

    # alice "How do you want your little girl to wake you up?"
    alice "Anh muốn em đánh thức như thế nào??"

# game/characters/alice/speak.rpy:154
translate vietnamese speak_alice_menuev9_82b1d477:

    # alice "sure thing, daddy~"
    alice "Chắc chắn rồi~"

# game/characters/alice/speak.rpy:161
translate vietnamese speak_alice_menuev9_82b1d477_1:

    # alice "sure thing, daddy~"
    alice "Được thôi~"

# game/characters/alice/speak.rpy:168
translate vietnamese speak_alice_menuev9_4988c52a:

    # alice "Sure, for who?"
    alice "Được rồi, cho ai?"

# game/characters/alice/speak.rpy:173
translate vietnamese speak_alice_outfits_e98034ac:

    # alice "So you are going to get me to make a gift for myself? How.. original.."
    alice "Vậy anh định để em làm một món quà cho chính mình á...?"

# game/characters/alice/speak.rpy:174
translate vietnamese speak_alice_outfits_3820771a:

    # me "..."
    me "..."

# game/characters/alice/speak.rpy:177
translate vietnamese speak_alice_outfits_0e8e8f19:

    # alice "So what do you have in mind?"
    alice "Vậy anh có ý tưởng gì?"

# game/characters/alice/speak.rpy:182
translate vietnamese speak_alice_outfits_169d617c:

    # alice "It's simple. I would need only 4 Cloth to make it. Want me to do this?"
    alice "Đơn giản thôi. em sẽ chỉ cần 4 Vải để làm nó. Muốn em làm cái này chứ?"

# game/characters/alice/speak.rpy:186
translate vietnamese speak_alice_outfits_a838aa40:

    # alice "Sure I'll start immediately."
    alice "Em sẽ bắt đầu ngay lập tức."

# game/characters/alice/speak.rpy:196
translate vietnamese speak_alice_outfits_89407595:

    # alice "First get the stuff needed."
    alice "Trước tiên hãy lấy những thứ cần thiết."

# game/characters/alice/speak.rpy:202
translate vietnamese speak_alice_outfits_07ab9ca6:

    # alice "If you wish so.."
    alice "Nếu anh muốn như vậy..."

# game/characters/alice/speak.rpy:208
translate vietnamese speak_alice_outfits_192bde7b:

    # alice "Someone's going to get naughty, right daddy?. I would need 15 Cloth to make it. Want me to do this?"
    alice "Ai đó sắp trở nên hư hỏng này, phải không? Em sẽ cần 15 Vải để làm nó."

# game/characters/alice/speak.rpy:214
translate vietnamese speak_alice_outfits_a838aa40_1:

    # alice "Sure I'll start immediately."
    alice "Chắc chắn rồi em sẽ bắt đầu ngay lập tức."

# game/characters/alice/speak.rpy:224
translate vietnamese speak_alice_outfits_89407595_1:

    # alice "First get the stuff needed."
    alice "Trước tiên anh hãy lấy những thứ cần thiết đã."

# game/characters/alice/speak.rpy:230
translate vietnamese speak_alice_outfits_07ab9ca6_1:

    # alice "If you wish so.."
    alice "Nếu anh muốn vậy..."

# game/characters/alice/speak.rpy:236
translate vietnamese speak_alice_outfits_ae978bb1:

    # alice "Grace isn't enough for you? You beast~ I would need 12 Cloth to make it. Want me to do this?"
    alice "Grace không đủ cho anh à? đồ quái thú~ Em sẽ cần 12 Vải để làm nó. Muốn Em làm cái này không?"

# game/characters/alice/speak.rpy:242
translate vietnamese speak_alice_outfits_a838aa40_2:

    # alice "Sure I'll start immediately."
    alice "Chắc chắn rồi em sẽ bắt đầu đây."

# game/characters/alice/speak.rpy:252
translate vietnamese speak_alice_outfits_89407595_2:

    # alice "First get the stuff needed."
    alice "Trước tiên hãy lấy những thứ cần thiết."

# game/characters/alice/speak.rpy:258
translate vietnamese speak_alice_outfits_07ab9ca6_2:

    # alice "If you wish so.."
    alice "Nếu anh muốn vậy..."

# game/characters/alice/speak.rpy:269
translate vietnamese speak_alice_outfits_aae4df87:

    # alice "Why do you want me to make something like this?"
    alice "Tại sao anh muốn em làm một thứ như thế này?"

# game/characters/alice/speak.rpy:270
translate vietnamese speak_alice_outfits_a8870946:

    # me "Just tell me if you can do it."
    me "Chỉ cần nói với anh em có thể làm nó không."

# game/characters/alice/speak.rpy:274
translate vietnamese speak_alice_outfits_cfa939e4:

    # alice "Of course I can. Gimme Maddened Heart, and three vials of Abyssal Blood."
    alice "Tất nhiên tôi có thể. Đưa em Trái tim điên cuồng, và ba lọ Máu vực thẳm."

# game/characters/alice/speak.rpy:281
translate vietnamese speak_alice_outfits_2ac692e4:

    # alice "Sure, I'll start immediately."
    alice "Chắc chắn rồi em sẽ bắt ngay đầu đây."

# game/characters/alice/speak.rpy:291
translate vietnamese speak_alice_outfits_89407595_3:

    # alice "First get the stuff needed."
    alice "Trước tiên hãy lấy những thứ cần thiết."

# game/characters/alice/speak.rpy:297
translate vietnamese speak_alice_outfits_07ab9ca6_3:

    # alice "If you wish so.."
    alice "Nếu anh muốn vậy..."

# game/characters/alice/speak.rpy:315
translate vietnamese speak_alice_outfits_b211a70c:

    # alice "Daddy wants to pound [alice]'s little pussy again?"
    alice "Anh lại muốn dập nát cái lồn nhỏ của [alice] nữa hả?"

# game/characters/alice/speak.rpy:316
translate vietnamese speak_alice_outfits_2d7dbb9a:

    # alice "Should I dress up for you?"
    alice "Em có nên lên đồ cho anh không?"

# game/characters/alice/speak.rpy:339
translate vietnamese speak_alice_outfits_1b3457b5:

    # alice "You know I could use you for something.."
    alice "Anh biết đấy em có thể sử dụng anh cho một cái gì đó..."

# game/characters/alice/speak.rpy:340
translate vietnamese speak_alice_outfits_e20c2277:

    # me "Oh? What is it?"
    me "Oh? Và đó là cái gì?"

# game/characters/alice/speak.rpy:343
translate vietnamese speak_alice_outfits_a5d3abf8:

    # alice "I could use some cleaning down there~"
    alice "Em có thể sử dụng anh để dọn dẹp dưới này~"

# game/characters/alice/speak.rpy:344
translate vietnamese speak_alice_outfits_376747c4:

    # me "Haha, got it."
    me "Haha, rõ rồi."

# game/characters/alice/speak.rpy:345
translate vietnamese speak_alice_outfits_8bd0d7e1:

    # alice "Should I change?"
    alice "Em có nên thay đồ không?"

# game/characters/alice/speak.rpy:351
translate vietnamese speak_alice_outfits_cb8a02c6:

    # alice "Ohh.. This little girl is going to be played by you again mister."
    alice "Ohh... Cô gái nhỏ này sẽ lại được chơi bởi ngài đấy, thưa ngài."

# game/characters/alice/speak.rpy:361
translate vietnamese speak_alice_outfits_b971ae2f:

    # me "Wanna reward me once more?"
    me "Muốn thưởng cho anh một lần nữa không?"

# game/characters/alice/speak.rpy:364
translate vietnamese speak_alice_outfits_81e18fa7:

    # alice "You can just ask directly you know?"
    alice "Anh chỉ cần hỏi trực tiếp thôi mà?"

# game/characters/alice/speak.rpy:365
translate vietnamese speak_alice_outfits_c1769394:

    # me "Then will you blow me?"
    me "Vậy thì em sẽ buscu cho anh chứ?"

# game/characters/alice/speak.rpy:368
translate vietnamese speak_alice_outfits_7b75f538:

    # alice "W-What are you talking about mister? I'm only a little girl.."
    alice "Ô-Ông đang nói gì vậy? Tôi chỉ là một cô bé..."

# game/characters/alice/speak.rpy:369
translate vietnamese speak_alice_outfits_3820771a_1:

    # me "..."
    me "..."

# game/characters/alice/speak.rpy:370
translate vietnamese speak_alice_outfits_388c714f:

    # me "Come to daddy."
    me "Đến với daddy nào."

# game/characters/alice/speak.rpy:373
translate vietnamese speak_alice_outfits_0b60a0c3:

    # alice "Hehe~"
    alice "Hehe~"

# game/characters/alice/speak.rpy:374
translate vietnamese speak_alice_outfits_e2b90efc:

    # alice "Lay down, daddy~"
    alice "Nằm xuống đi, daddy~"

# game/characters/alice/speak.rpy:375
translate vietnamese speak_alice_outfits_fec91136:

    # "You do as told and lie down waiting for her to join."
    "Bạn làm như đã được nói và nằm xuống đợi cô ấy tham gia."

# game/characters/alice/speak.rpy:376
translate vietnamese speak_alice_outfits_a69a8806:

    # alice "What outfit do you want this little girl to wear, daddy?"
    alice "Anh muốn cô gái nhỏ này mặc trang phục gì đây?"

# game/characters/alice/speak.rpy:382
translate vietnamese speak_alice_outfits_f4b76968:

    # alice "Hehe~ You pervert~"
    alice "Hehe~ Đồ biến thái~"

# game/characters/alice/speak.rpy:395
translate vietnamese speak_alice_outfits_f6adff98:

    # me "Can you give me a footjob?"
    me "Em có thể dùng chân được không?"

# game/characters/alice/speak.rpy:400
translate vietnamese speak_alice_outfits_4bcaf3b9:

    # alice "You're becoming more and more daring."
    alice "anh ngày càng trở nên táo bạo hơn đấy."

# game/characters/alice/speak.rpy:401
translate vietnamese speak_alice_outfits_f660d172:

    # me "So?"
    me "Thì?"

# game/characters/alice/speak.rpy:402
translate vietnamese speak_alice_outfits_c313e6b5:

    # alice "And impatient."
    alice "Và thiếu kiên nhẫn."

# game/characters/alice/speak.rpy:403
translate vietnamese speak_alice_outfits_f660d172_1:

    # me "So?"
    me "Thì?"

# game/characters/alice/speak.rpy:404
translate vietnamese speak_alice_outfits_74f05a4e:

    # alice "..."
    alice "..."

# game/characters/alice/speak.rpy:407
translate vietnamese speak_alice_outfits_e8149e61:

    # alice "Lie down. What do you want me to wear?"
    alice "Nằm xuống đi. Anh muốn em mặc gì nào?"

# game/characters/alice/speak.rpy:413
translate vietnamese speak_alice_outfits_35cada6d:

    # alice "Hehe~ You love this outfit so much, right?"
    alice "Hehe~ Anh yêu thích bộ trang phục này lắm đúng không?"

# game/characters/alice/speak.rpy:423
translate vietnamese speak_alice_outfits_011de6f2:

    # alice "Time for another workout."
    alice "đến lúc cho một bài tập luyện khác rồi."

# game/characters/alice/speak.rpy:428
translate vietnamese speak_alice_outfits_af5f5d0a:

    # alice "You want to torture my pussy again, daddy?"
    alice "Bố lại muốn tra tấn âm đạo của con nữa hả daddy?"

# game/characters/alice/speak.rpy:460
translate vietnamese speak_alice_outfits_8f7b6901:

    # alice "Sure, what is it?"
    alice "Được thôi, anh muốn hỏi gì?"

# game/characters/alice/speak.rpy:466
translate vietnamese speak_alice_menu2_408e4544:

    # alice "I'm mostly responsible for making things. I use things Lexi brings from outside to make clothes or whatever we need. I have limited tools, though."
    alice "Em chủ yếu chịu trách nhiệm về việc tạo ra mọi thứ. Em sử dụng những thứ Lexi mang từ bên ngoài để may quần áo hoặc bất cứ thứ gì chúng em cần. Tuy nhiên, em có một số hạn chế về công cụ."

# game/characters/alice/speak.rpy:467
translate vietnamese speak_alice_menu2_be011575:

    # alice "Sometimes I go out to explore instead of Lexi."
    alice "Đôi khi em ra ngoài khám phá thay vì Lexi."

# game/characters/alice/speak.rpy:468
translate vietnamese speak_alice_menu2_0375c1e9:

    # me "Then if I find some materials outside, can I count on you to process them into something useful."
    me "Vậy thì, nếu anh tìm thấy một số vật liệu bên ngoài, anh có thể tin tưởng vào em để biến chúng thành một cái gì đó hữu ích."

# game/characters/alice/speak.rpy:471
translate vietnamese speak_alice_menu2_9c5668fc:

    # alice "Sure, if I'm in the mood."
    alice "Chắc chắn rồi, nếu em có tâm trạng."

# game/characters/alice/speak.rpy:472
translate vietnamese speak_alice_menu2_3820771a:

    # me "..."
    me "..."

# game/characters/alice/speak.rpy:473
translate vietnamese speak_alice_menu2_ec565397:

    # me "Do you have mood swings often?"
    me "Em có thường xuyên thay đổi tâm trạng không?"

# game/characters/alice/speak.rpy:474
translate vietnamese speak_alice_menu2_60af599e:

    # alice "Heh. Every.. single.. day.."
    alice "Heh. Tất... cả... mọi... ngày..."

# game/characters/alice/speak.rpy:475
translate vietnamese speak_alice_menu2_b1096efb:

    # "Her playful voice when saying it makes you wonder if she was an actress."
    "Giọng nói vui tươi của em ấy khi nói khiến bạn tự hỏi liệu em ấy có phải là một diễn viên không."

# game/characters/alice/speak.rpy:476
translate vietnamese speak_alice_menu2_c0b319a3:

    # th "Not everyone can change their approach so quickly."
    th "Không phải ai cũng có thể thay đổi cách tiếp cận của mình một cách nhanh chóng đâu."

# game/characters/alice/speak.rpy:477
translate vietnamese speak_alice_menu2_d59a6b9a:

    # me "Great."
    me "Tuyệt."

# game/characters/alice/speak.rpy:480
translate vietnamese speak_alice_menu2_efa8586f:

    # alice "You can always try."
    alice "Anh luôn có thể thử."

# game/characters/alice/speak.rpy:487
translate vietnamese speak_alice_menu2_1daaeb54:

    # alice "From the beginning. I arrived here on the fifth day of The End."
    alice "Từ đầu thì. Em đến đây vào ngày thứ năm của The End."

# game/characters/alice/speak.rpy:488
translate vietnamese speak_alice_menu2_15307cda:

    # me "You were lucky then."
    me "Lúc đó em may thật đấy."

# game/characters/alice/speak.rpy:489
translate vietnamese speak_alice_menu2_50ddb3b5:

    # alice "I surely was. I would most likely be dead by now if I wasn't here."
    alice "Em chắc chắn là như vậy. Bây giờ em rất có thể đã chết nếu không ở đây."

# game/characters/alice/speak.rpy:490
translate vietnamese speak_alice_menu2_3820771a_1:

    # me "..."
    me "..."

# game/characters/alice/speak.rpy:493
translate vietnamese speak_alice_menu2_940d8424:

    # alice "What, don't tell me you are tearing up. Don't be a pussy!"
    alice "Cái gì, đừng nói với em là anh đang rơi nước mắt nhá? Đúng là đồ đàn bà mà!"

# game/characters/alice/speak.rpy:494
translate vietnamese speak_alice_menu2_dd2a341e:

    # "Your jaw drops hearing that."
    "Hàm của bạn rơi xuống khi nghe thấy điều đó."

# game/characters/alice/speak.rpy:495
translate vietnamese speak_alice_menu2_56c09f6e:

    # alice "Want to know some more about me?"
    alice "Muốn biết thêm về em hả?"

# game/characters/alice/speak.rpy:496
translate vietnamese speak_alice_menu2_12038dc2:

    # me "Thank you, but no."
    me "Cảm ơn em, nhưng không cần đâu."

# game/characters/alice/speak.rpy:499
translate vietnamese speak_alice_menu2_a827a5cb:

    # alice "Hmph. No fun."
    alice "Hứ. Không vui."

# game/characters/alice/speak.rpy:500
translate vietnamese speak_alice_menu2_e4dc27b7:

    # me "Gonna tear up?"
    me "Định khóc hả?"

# game/characters/alice/speak.rpy:503
translate vietnamese speak_alice_menu2_b9266d70:

    # alice "Pussy."
    alice "Đồ đàn bà."

# game/characters/alice/speak.rpy:504
translate vietnamese speak_alice_menu2_3820771a_2:

    # me "..."
    me "..."

# game/characters/alice/speak.rpy:505
translate vietnamese speak_alice_menu2_340be731:

    # th "Be smarter [me], you are older than her.."
    th "Thông minh lên đi [me], mày lớn tuổi hơn em nó mà..."

# game/characters/alice/speak.rpy:506
translate vietnamese speak_alice_menu2_0c482c39:

    # me "Fine, you win."
    me "Tốt thôi, em thắng."

# game/characters/alice/speak.rpy:509
translate vietnamese speak_alice_menu2_8a94e027:

    # alice "As always."
    alice "Như mọi khi."

# game/characters/alice/speak.rpy:515
translate vietnamese speak_alice_menu2_7fa423c8:

    # alice "Outside there are generally three rules."
    alice "Bên ngoài thường có ba quy tắc."

# game/characters/alice/speak.rpy:517
translate vietnamese speak_alice_menu2_c9973e05:

    # alice "One, don't be curious. The child you hear crying is ten stories tall and will eat you in a second. People you want to talk with will take everything you have and eat your flesh if they're hungry. I think you understand the meaning."
    alice "Thứ nhất, đừng tò mò. Đứa trẻ mà anh nghe thấy nó khóc thì cao bằng tòa nhà mười tầng và sẽ thịt anh trong một giây. Những người mà muốn nói chuyện sẽ lấy mọi thứ anh có và ăn thịt anh nếu họ đói."

# game/characters/alice/speak.rpy:518
translate vietnamese speak_alice_menu2_ecd768ba:

    # me "Yeah."
    me "Yeah."

# game/characters/alice/speak.rpy:519
translate vietnamese speak_alice_menu2_3150aade:

    # alice "Two, never try to fight with anything that is not human. You will most likely die without even leaving a body behind."
    alice "Thứ hai, đừng bao giờ cố gắng oánh lộn với những thứ không phải là con người. anh rất có thể sẽ mất xác luôn đấy."

# game/characters/alice/speak.rpy:520
translate vietnamese speak_alice_menu2_a1c73fe1:

    # me "Got it."
    me "Hiểu rồi."

# game/characters/alice/speak.rpy:522
translate vietnamese speak_alice_menu2_6640fe68:

    # alice "There is an exception, cultists. NEVER, EVER try to fight them!"
    alice "Có một ngoại lệ, những tên cuồng đạo. KHÔNG BAO GIỜ dây dưa với chúng!"

# game/characters/alice/speak.rpy:523
translate vietnamese speak_alice_menu2_7166776a:

    # me "Oh, why is that?"
    me "Tại sao thế?"

# game/characters/alice/speak.rpy:526
translate vietnamese speak_alice_menu2_87c43513:

    # alice "It's not something that can be explained with words, and I sincerely hope you never have a chance to find out why. Let's just say they are much more terrifying than monsters."
    alice "Đó không phải là thứ mà có thể giải thích bằng lời, và em thực lòng hy vọng anh không bao giờ có cơ hội tìm ra lý do tại sao. Hãy chỉ nói rằng chúng đáng sợ hơn nhiều so với quái vật."

# game/characters/alice/speak.rpy:527
translate vietnamese speak_alice_menu2_a81398a0:

    # me "What about the third rule?"
    me "Còn quy tắc thứ ba?"

# game/characters/alice/speak.rpy:530
translate vietnamese speak_alice_menu2_46832772:

    # alice "Three, don't stay outside after dark. EVER!"
    alice "Thứ ba, đừng ở ngoài sau khi trời tối. KHÔNG BAO GIỜ!"

# game/characters/alice/speak.rpy:532
translate vietnamese speak_alice_menu2_0c44ae01:

    # alice "The night is filled with whispers that make people turn crazy. Weirdly enough, these whispers can only be heard in the fog. So stay indoors at night, and you should be ok."
    alice "Màn đêm tràn ngập những tiếng thì thầm khiến lòng người trở nên điên loạn. Những lời thì thầm này chỉ có thể nghe thấy trong sương mù. Vì vậy, hãy ở trong nhà vào ban đêm, và anh sẽ ổn."

# game/characters/alice/speak.rpy:533
translate vietnamese speak_alice_menu2_10a7e12b:

    # me "I understand, thank you."
    me "Anh hiểu rồi, cảm ơn."

# game/characters/alice/speak.rpy:534
translate vietnamese speak_alice_menu2_8b1d83a7:

    # alice "There is something else, a rumor."
    alice "Còn một thứ khác nữa, một tin đồn."

# game/characters/alice/speak.rpy:535
translate vietnamese speak_alice_menu2_0cf8453a:

    # me "Yeah?"
    me "Tin đồn nào?"

# game/characters/alice/speak.rpy:536
translate vietnamese speak_alice_menu2_679efdff:

    # alice "I've heard that those that returned after spending a night outside, aside from spouting nonsense, all say one thing in common."
    alice "Em nghe nói rằng những người quay trở lại sau một đêm ở ngoài trời, ngoài những lời nhảm nhỉ ra, tất cả đều nói một điều giống nhau."

# game/characters/alice/speak.rpy:539
translate vietnamese speak_alice_menu2_247be621:

    # alice "Three minutes after stepping outside their shadow came to life. They all said their shadows stood up from the ground and followed them."
    alice "Ba phút sau khi bước ra ngoài cái bóng của họ đã trở nên sống động. Tất cả đều nói cái bóng của họ đứng lên khỏi mặt đất và đi theo họ."

# game/characters/alice/speak.rpy:541
translate vietnamese speak_alice_menu2_ea04d2c2:

    # alice "It was blacker than night, and although its face was unrecognizable, they swear it was smiling literally from ear to ear. They all claimed their shadows are the ones whispering in the fog."
    alice "Nó đen hơn cả màn đêm, và mặc dù không thể nhận ra khuôn mặt của nó, nhưng họ thề rằng nó đang mỉm cười từ tai này sang tai khác theo đúng nghĩa đen. Họ đều khẳng định bóng của họ là thứ thì thầm trong sương mù."

# game/characters/alice/speak.rpy:542
translate vietnamese speak_alice_menu2_4a74795b:

    # me "Is it for real?"
    me "Nó có thật không?"

# game/characters/alice/speak.rpy:545
translate vietnamese speak_alice_menu2_ebcca8c8:

    # alice "Who knows.. I never met any of those madmen personally."
    alice "Ai mà biết... Em chưa bao giờ gặp bất kỳ ai trong số những người bị điên cả."

# game/characters/alice/speak.rpy:546
translate vietnamese speak_alice_menu2_ab422b1b:

    # alice "Whether it's true or not, don't go outside at night."
    alice "Cho dù đó là sự thật hay không, đừng đi ra ngoài vào ban đêm."

# game/characters/alice/speak.rpy:547
translate vietnamese speak_alice_menu2_5c8f21bd:

    # me "I will remember that."
    me "anh sẽ nhớ lấy điều đó."

# game/characters/alice/speak.rpy:558
translate vietnamese speak_alice_menu2_d180988e:

    # alice "Great."
    alice "Tốt."

# game/characters/alice/speak.rpy:569
translate vietnamese speak_alice_magical_outfit_rachel_5e4cc63c:

    # alice "You need a what?"
    alice "Anh cần một cái gì cơ?"

# game/characters/alice/speak.rpy:570
translate vietnamese speak_alice_magical_outfit_rachel_40498bbf:

    # me "Exactly what you heard, dear."
    me "Chính xác những gì em đã nghe đấy em iu."

# game/characters/alice/speak.rpy:571
translate vietnamese speak_alice_magical_outfit_rachel_2e6ed0d1:

    # alice "Magical outfit?"
    alice "Trang phục pháp sư?"

# game/characters/alice/speak.rpy:572
translate vietnamese speak_alice_magical_outfit_rachel_f81d13e6:

    # me "Exactly."
    me "Chính xác."

# game/characters/alice/speak.rpy:577
translate vietnamese speak_alice_magical_outfit_rachel_ecc00eae:

    # alice "What does that mean?"
    alice "Điều đó nghĩa là gì?"

# game/characters/alice/speak.rpy:578
translate vietnamese speak_alice_magical_outfit_rachel_d04b0371:

    # me "I don't know. An outift for a girl that's a magician I guess."
    me "Anh không biết. Anh đoán là một bộ trang phục cho một cô gái pháp sư."

# game/characters/alice/speak.rpy:579
translate vietnamese speak_alice_magical_outfit_rachel_ee741fcf:

    # alice "Who do you need it for?"
    alice "Anh cần nó cho ai?"

# game/characters/alice/speak.rpy:580
translate vietnamese speak_alice_magical_outfit_rachel_393067ab:

    # me "I would rather not tell..."
    me "Anh không muốn nói..."

# game/characters/alice/speak.rpy:585
translate vietnamese speak_alice_magical_outfit_rachel_1c77a2e1:

    # alice "[me].."
    alice "[me]..."

# game/characters/alice/speak.rpy:586
translate vietnamese speak_alice_magical_outfit_rachel_3820771a:

    # me "..."
    me "..."

# game/characters/alice/speak.rpy:587
translate vietnamese speak_alice_magical_outfit_rachel_12be3dc7:

    # alice "Does it have anything to do with Rachel?"
    alice "Nó có liên quan gì đến Rachel không?"

# game/characters/alice/speak.rpy:588
translate vietnamese speak_alice_magical_outfit_rachel_a790d9ec:

    # me "You know her?"
    me "Em biết cô ấy á?"

# game/characters/alice/speak.rpy:593
translate vietnamese speak_alice_magical_outfit_rachel_8be9a55d:

    # alice "No shit, Sherlock. She is living here. We all know her."
    alice "Tất nhiên là có rồi. Cô ấy đang sống ở đây. Tất cả bọn em đều biết về cô ấy."

# game/characters/alice/speak.rpy:594
translate vietnamese speak_alice_magical_outfit_rachel_37b01c69:

    # me "I guess I could've expected that."
    me "Anh đoán là mình cũng lường trước được rồi điều đó rồi."

# game/characters/alice/speak.rpy:599
translate vietnamese speak_alice_magical_outfit_rachel_c1b9263a:

    # alice "Yeah, we are not blind. How many girls are you going to bring back home?"
    alice "Vâng, chúng em không mù đâu. Anh sẽ định đưa bao nhiêu cô gái nữa về nhà nữa?"

# game/characters/alice/speak.rpy:600
translate vietnamese speak_alice_magical_outfit_rachel_1f14bced:

    # me "She had nowhere else to go.."
    me "Cô ấy không có nơi nào khác để đi..."

# game/characters/alice/speak.rpy:601
translate vietnamese speak_alice_magical_outfit_rachel_011e97e6:

    # alice "Yeah, sure."
    alice "Ừ phải rồi."

# game/characters/alice/speak.rpy:606
translate vietnamese speak_alice_magical_outfit_rachel_dfc283ca:

    # alice "You're so audacious I'm just speechless."
    alice "Anh trơ tráo quá đấy, em đến cạn lời rồi."

# game/characters/alice/speak.rpy:607
translate vietnamese speak_alice_magical_outfit_rachel_257186c2:

    # me "I'm sorry [alice]."
    me "Anh xin lỗi [alice]."

# game/characters/alice/speak.rpy:612
translate vietnamese speak_alice_magical_outfit_rachel_178d1453:

    # alice "You want magician girl outfit, right? Fine. I'll do it."
    alice "Anh muốn trang phục nữ pháp sư phải không? Rồi. em sẽ làm nó."

# game/characters/alice/speak.rpy:613
translate vietnamese speak_alice_magical_outfit_rachel_123f9f03:

    # me "Thank you. What do you need?"
    me "Cảm ơn. Em cần gì?"

# game/characters/alice/speak.rpy:614
translate vietnamese speak_alice_magical_outfit_rachel_9a0f4f5b:

    # alice "16 pieces of cloth."
    alice "16 mảnh vải."

# game/characters/alice/speak.rpy:615
translate vietnamese speak_alice_magical_outfit_rachel_e6ddb289:

    # me "Got it. I will get it."
    me "Hiểu rồi. Anh sẽ lấy nó."

# game/characters/alice/speak.rpy:626
translate vietnamese speak_alice_magical_outfit_rachel2_89e33485:

    # alice "Alright. I'll get it done right now."
    alice "Được rồi. Em sẽ hoàn thành nó ngay bây giờ."

# game/characters/alice/speak.rpy:627
translate vietnamese speak_alice_magical_outfit_rachel2_12cf7b69:

    # me "Thanks."
    me "Cảm ơn."

# game/characters/alice/speak.rpy:654
translate vietnamese speak_alice_tasks_83fa7619:

    # alice "'Key~"
    alice "'OK~"

# game/characters/alice/speak.rpy:663
translate vietnamese speak_alice_tasks_aedbeae6:

    # alice "Got it. I'll get it done as soon as possible."
    alice "Hiểu rồi. Em sẽ hoàn thành nó sớm nhất có thể."

# game/characters/alice/speak.rpy:664
translate vietnamese speak_alice_tasks_d59a6b9a:

    # me "Great."
    me "Tuyệt."

# game/characters/alice/speak.rpy:665
translate vietnamese speak_alice_tasks_0fdab02e:

    # alice "Don't let anything happen to you in the meantime."
    alice "Đừng để bất cứ điều gì xảy ra với anh trong thời gian chờ đợi đấy."

# game/characters/alice/speak.rpy:666
translate vietnamese speak_alice_tasks_0d37dacf:

    # me "I can take care of myself."
    me "Anh có thể tự lo bản thân mà."

# game/characters/alice/speak.rpy:670
translate vietnamese speak_alice_tasks_84d39b46:

    # alice "Mm. Be careful, [me]."
    alice "Mm. Hãy cẩn thận, [me]."

# game/characters/alice/speak.rpy:683
translate vietnamese speak_alice_tasks_5a78ff99:

    # alice "You don't want me to tear apart my own clothes so that you can make me make new ones, so that you can give them to me as a gift right?"
    alice "Anh không muốn em xé quần áo của chính mình để anh có thể làm cho em những cái mới, và tặng chúng như một món quà đúng không?"

# game/characters/alice/speak.rpy:684
translate vietnamese speak_alice_tasks_3820771a:

    # me "..."
    me "..."

# game/characters/alice/speak.rpy:688
translate vietnamese speak_alice_tasks_55a998d4:

    # alice "Rrrrright?"
    alice "Phải khônggggg?"

# game/characters/alice/speak.rpy:689
translate vietnamese speak_alice_tasks_d7144f1e:

    # me "Of course not.."
    me "Dĩ nhiên là không rồi..."

# game/characters/alice/speak.rpy:690
translate vietnamese speak_alice_tasks_015461a6:

    # alice "I hope so."
    alice "Em cũng mong là như vậy."

# game/characters/alice/speak.rpy:703
translate vietnamese speak_alice_tasks_b033a4f6:

    # alice "Don't worry. I'll make your outfit much more durable than it was."
    alice "Đừng lo. Em sẽ làm cho trang phục của anh bền hơn nhiều so với ban đầu."

# game/characters/alice/speak.rpy:704
translate vietnamese speak_alice_tasks_12cf7b69:

    # me "Thanks."
    me "Mơn."

# game/characters/alice/speak.rpy:705
translate vietnamese speak_alice_tasks_eb2ca64a:

    # alice "You won't die with me."
    alice "Anh sẽ không chết khi còn em đâu."

# game/characters/alice/speak.rpy:706
translate vietnamese speak_alice_tasks_511d56d2:

    # me "Haha, I believe you."
    me "Haha, Anh tin em đấy."

# game/characters/alice/speak.rpy:722
translate vietnamese speak_alice_tasks_5be308d9:

    # alice "I will. Come see me soon."
    alice "Em sẽ. Đến đây sớm nha."

# game/characters/alice/speak.rpy:723
translate vietnamese speak_alice_tasks_9b39cc4a:

    # me "You have my word."
    me "Anh hứa."

# game/characters/alice/speak.rpy:724
translate vietnamese speak_alice_tasks_30e104f3:

    # alice "Bye~"
    alice "Bye~"

# game/characters/alice/speak.rpy:725
translate vietnamese speak_alice_tasks_4095caad:

    # me "See you."
    me "Gặp lại sau."

# game/characters/alice/speak.rpy:733
translate vietnamese speak_alice_question_jack_9b1f5d58:

    # alice "Oh.. He was a good, really nice person."
    alice "Oh... Anh ấy là một người tốt, thực sự tốt."

# game/characters/alice/speak.rpy:734
translate vietnamese speak_alice_question_jack_5500212e:

    # me "Sorry, I know it's been a long time since you last saw him."
    me "Xin lỗi, Anh biết đã lâu lắm rồi kể từ lần cuối em nhìn thấy anh ấy."

# game/characters/alice/speak.rpy:735
translate vietnamese speak_alice_question_jack_230f70ff:

    # alice "Well, that's how it is outside. I just hope he did die instead of becoming a monster.."
    alice "Bên ngoài là vậy đấy. Em chỉ mong anh ấy chết thay vì biến thành một con quái vật..."

# game/characters/alice/speak.rpy:736
translate vietnamese speak_alice_question_jack_7d5722c2:

    # me "Poor thing."
    me "Tội nghiệp."

# game/characters/alice/speak.rpy:737
translate vietnamese speak_alice_question_jack_68aac134:

    # me "What did he do? In the hotel I mean."
    me "Anh ấy đã làm gì? Ý anh là trong khách sạn."

# game/characters/alice/speak.rpy:741
translate vietnamese speak_alice_question_jack_6863b254:

    # alice "Well, before The End he was going to run this hotel. So much for that.."
    alice "Trước khi có The End, anh ấy điều hành khách sạn này. Quá nhiều cho điều đó..."

# game/characters/alice/speak.rpy:742
translate vietnamese speak_alice_question_jack_d9064223:

    # alice "But I guess in a way, he did get to do it."
    alice "Nhưng em đoán theo một cách nào đó, anh ấy đã làm được."

# game/characters/alice/speak.rpy:746
translate vietnamese speak_alice_question_jack_e1405e39:

    # alice "When I first got here, he was pretty much set up. He had provisions, a few weapons, and since the water and gas were running, we didn't really have any issues."
    alice "Khi em lần đầu tiên đến đây, anh ấy đã sắp đặt khá nhiều. Anh ấy có đồ dự phòng, một ít vũ khí, và lúc nước và gas sắp hết, bọn em cũng chả có vấn đề gì hết."

# game/characters/alice/speak.rpy:747
translate vietnamese speak_alice_question_jack_8894c0f2:

    # me "So you just asked to live here? And he just accepted?"
    me "Thế thì em chỉ nói là muốn sống ở đây? Và anh ấy cho luôn á?"

# game/characters/alice/speak.rpy:748
translate vietnamese speak_alice_question_jack_2944c552:

    # alice "Well, for me it was a chance to stay relatively safe, but before I could even ask for it.."
    alice "Đối với em đó là một cơ hội để ở lại một nơi tương đối an toàn, nhưng trước khi em có thể hỏi về nó..."

# game/characters/alice/speak.rpy:749
translate vietnamese speak_alice_question_jack_9f928e6b:

    # alice "He just told me to take a room. As long as I contributed, I could stay."
    alice "Anh ấy chỉ nói em lấy phòng đi. Miễn là em góp sức, thì em có thể ở lại."

# game/characters/alice/speak.rpy:750
translate vietnamese speak_alice_question_jack_aadc6870:

    # me "He didn't question you at all?"
    me "Anh ấy không hỏi em chút nào luôn?"

# game/characters/alice/speak.rpy:754
translate vietnamese speak_alice_question_jack_f2c30fa9:

    # alice "Yes, but only basic, really simple things. Like my name, if I knew of other safe places and such. He wasn't interested at all in my personal life, I guess."
    alice "Có, nhưng chỉ những điều căn bản thôi bản, thực sự rất đơn giản. Giống như tên của em, nếu tôi biết về những nơi an toàn khác và những nơi khác. Anh ấy không quan tâm chút nào đến cuộc sống cá nhân của em."

# game/characters/alice/speak.rpy:755
translate vietnamese speak_alice_question_jack_e7d9e0e8:

    # me "What about scavenging? Did he ask you to help?"
    me "Còn kiếm đồ thì sao? Anh ấy có nhờ em giúp không?"

# game/characters/alice/speak.rpy:756
translate vietnamese speak_alice_question_jack_7187ae19:

    # alice "I offered to help, but he didn't really support or refuse it. Just told me to be armed and ready if I was going outside."
    alice "Em đã đề nghị giúp đỡ, nhưng anh ấy không thực sự ủng hộ hoặc từ chối nó. Chỉ bảo là hãy trang bị vũ khí và sẵn sàng nếu em muốn ra ngoài."

# game/characters/alice/speak.rpy:757
translate vietnamese speak_alice_question_jack_966fe5ca:

    # me "I see. And he went out to scavenge himself."
    me "Anh hiểu rồi. Và anh ấy tự đi ra ngoài để kiếm đồ sao."

# game/characters/alice/speak.rpy:761
translate vietnamese speak_alice_question_jack_678573e6:

    # alice "Of course. It seemed he was quite experienced, too. He was always calm and confident about it, honestly."
    alice "Tất nhiên. Có vẻ như anh ấy cũng khá là có kinh nghiệm. Anh ấy luôn bình tĩnh và tự tin về điều đó."

# game/characters/alice/speak.rpy:765
translate vietnamese speak_alice_question_jack_c6dd826e:

    # alice "I thought he just knew what he was doing, but it always worried me.."
    alice "Em nghĩ anh ấy biết những gì mình ấy đang làm, nhưng điều đó luôn khiến em lo lắng..."

# game/characters/alice/speak.rpy:766
translate vietnamese speak_alice_question_jack_461623ee:

    # alice "Even the best will face defeat eventually. Nothing good lasts forever."
    alice "Ngay cả những người giỏi nhất cuối cùng cũng sẽ phải đối mặt với thất bại. Không có gì tốt tồn tại mãi mãi cả."

# game/characters/alice/speak.rpy:767
translate vietnamese speak_alice_question_jack_edf882a7:

    # me ".. Anything else you could tell me?"
    me "... Còn cái gì khác em có thể nói với anh không?"

# game/characters/alice/speak.rpy:771
translate vietnamese speak_alice_question_jack_75b3cbfd:

    # alice "Not really, unless you ask for something specific. Just don't end up like him."
    alice "Không hẳn, trừ khi anh hỏi một cái gì đó cụ thể. Chỉ cần đừng kết thúc như anh ấy là được."

# game/characters/alice/speak.rpy:772
translate vietnamese speak_alice_question_jack_39ec0b96:

    # me "I'll keep it mind. Thanks."
    me "Anh sẽ ghi nhớ. Cảm ơn."

translate vietnamese strings:

    # game/characters/alice/speak.rpy:21
    old "I need another cosplay outfit... {gold}(Story){/gold}"
    new "Anh cần một bộ đồ cosplay khác... {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "I need a magical girl outfit... {gold}(Story){/gold}"
    new "Anh cần một bộ đồ nữ pháp sư... {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "Give 16 cloth for Rachel outfit {gold}(Story){/gold}"
    new "Đưa 16 tấm vải cho trang phục Rachel {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "How are you feeling? {gold}(Story){/gold}"
    new "Em cảm thấy thế nào? {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "Ask how she feels {gold}(Story){/gold}"
    new "Hỏi cô ấy cảm thấy thế nào {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "How about some fun? {gold}(Story){/gold}"
    new "Làm chút thì sao? {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "Are you alright? {gold}(Story){/gold}"
    new "Em ổn chứ? {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "Ask about her mood. {gold}(Story){/gold}"
    new "Hỏi về tâm trạng của cô ấy. {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "Bring on your questions. {gold}(Story){/gold}"
    new "Mang lên những câu hỏi của bạn. {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "It's time to help [mina] {gold}(Story){/gold}"
    new "Đã đến lúc giúp đỡ [mina] {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "Can I use your tv? {trust}(Trust+){/trust}"
    new "Anh có thể dùng TV của em không? {trust}(tin tưởng+){/trust}"

    # game/characters/alice/speak.rpy:21
    old "Do you want to train together? {trust}(Trust+){/trust}"
    new "Em có muốn tập luyện cùng nhau không? {trust}(Tin tưởng+){/trust}"

    # game/characters/alice/speak.rpy:21
    old "Don't wake me up anymore"
    new "Đừng đánh thức anh nữa"

    # game/characters/alice/speak.rpy:21
    old "Wake me up"
    new "Đánh thức Anh"

    # game/characters/alice/speak.rpy:21
    old "Make me an outfit"
    new "May cho anh một bộ quần áo"

    # game/characters/alice/speak.rpy:21
    old "Kiss {lust}(Lust+){/lust}"
    new "Hôn {lust}(Ham muốn+){/lust}"

    # game/characters/alice/speak.rpy:21
    old "Have fun {lust}(Lust+){/lust}"
    new "Làm vài nháy {lust}(Ham muốn+){/lust}"

    # game/characters/alice/speak.rpy:21
    old "Tasks"
    new "Nhiệm vụ"

    # game/characters/alice/speak.rpy:21
    old "I have some questions."
    new "Anh có một số câu hỏi."

    # game/characters/alice/speak.rpy:43
    old "Give antibiotics {#visible}"
    new "Cho thuốc kháng sinh"

    # game/characters/alice/speak.rpy:43
    old "Use extraordinary abilities {#visible}"
    new "Sử dụng khả năng phi thường"

    # game/characters/alice/speak.rpy:43
    old "I wish you swift recovery"
    new "Anh ước em nhanh chóng bình phục"

    # game/characters/alice/speak.rpy:169
    old "Alice"
    new "Alice"

    # game/characters/alice/speak.rpy:169
    old "Lexi"
    new "Lexi"

    # game/characters/alice/speak.rpy:178
    old "Stranded {#visible}"
    new "Stranded {#visible}"

    # game/characters/alice/speak.rpy:178
    old "School Girl {#visible}"
    new "Nữ sinh {#visible}"

    # game/characters/alice/speak.rpy:178
    old "Maid {#visible}"
    new "Hầu gái {#visible}"

    # game/characters/alice/speak.rpy:265
    old "Tentacles {#visible}"
    new "Tentacles {#visible}"

    # game/characters/alice/speak.rpy:309
    old "Kiss"
    new "Hôn"

    # game/characters/alice/speak.rpy:309
    old "Wooden Horse"
    new "Ngựa gỗ"

    # game/characters/alice/speak.rpy:317
    old "Casual"
    new "Bình thường"

    # game/characters/alice/speak.rpy:317
    old "School Girl"
    new "Nữ sinh"

    # game/characters/alice/speak.rpy:317
    old "Maid"
    new "Hầu gái"

    # game/characters/alice/speak.rpy:317
    old "Nightdress"
    new "Váy ngủ"

    # game/characters/alice/speak.rpy:317
    old "Stranded"
    new "rách rưới"

    # game/characters/alice/speak.rpy:317
    old "Gym"
    new "Phòng tập"

    # game/characters/alice/speak.rpy:429
    old "Naked"
    new "Khỏa thân"

    # game/characters/alice/speak.rpy:429
    old "Pyjamas"
    new "Đồ ngủ"

    # game/characters/alice/speak.rpy:461
    old "What do you do around here?"
    new "Em làm gì quanh đây?"

    # game/characters/alice/speak.rpy:461
    old "How long have you been here?"
    new "Em đã ở đây bao lâu?"

    # game/characters/alice/speak.rpy:461
    old "Do you have any advice for exploration?"
    new "Em có lời khuyên nào về thám hiểm không?"

    # game/characters/alice/speak.rpy:461
    old "Could you tell me about Jack?"
    new "Em có thể cho anh biết về Jack?"

    # game/characters/alice/speak.rpy:634
    old "Rachel magician outfit unlocked"
    new "Đã mở khóa trang phục pháp sư Rachel"

    # game/characters/alice/speak.rpy:648
    old "Cancel current task {#visible}"
    new "Hủy công việc hiện tại {#visible}"

    # game/characters/alice/speak.rpy:648
    old "Get me some battle gear (Duration: [aliceTasks.getGear.duration]) {gold}(Cards+){/gold} {#visible}"
    new "Lấy cho anh một số thiết bị chiến đấu (Thời gian: [aliceTasks.getGear.duration]) {gold}(Thẻ+){/gold} {#visible}"

    # game/characters/alice/speak.rpy:648
    old "Dismantle old clothes (Duration: [aliceTasks.dismantleClothes.duration]) {loot}(Cloth+){/loot} {#visible}"
    new "Bỏ quần áo cũ (Thời gian: [aliceTasks.dismantleClothes.duration]) {loot}(Vải+){/loot} {#visible}"

    # game/characters/alice/speak.rpy:648
    old "Reinforce coat (Duration: [aliceTasks.reinforceCoat.duration]) {trust}(Buff+){/trust} {#visible}"
    new "Gia cố áo khoác (Thời gian: [aliceTasks.reinforceCoat.duration]) {trust}(Buff+){/trust} {#visible}"

    # game/characters/alice/speak.rpy:648
    old "Rest"
    new "Nghỉ"
# : Translation updated at 2022-02-06 08:10

# game/characters/alice/speak.rpy:124
translate vietnamese alice_menu1_menuev9_5d6b5979:

    # me "Wait, what? I'm very go-" id alice_menu1_menuev9_5d6b5979
    me "Chờ đã, cái gì? Anh là m-" id alice_menu1_menuev9_5d6b5979

# game/characters/alice/speak.rpy:154
translate vietnamese alice_menu1_menuev9_82b1d477:

    # alice "Sure thing, daddy~" id alice_menu1_menuev9_82b1d477
    alice "Tất nhiên rồi daddy~" id alice_menu1_menuev9_82b1d477

# game/characters/alice/speak.rpy:161
translate vietnamese alice_menu1_menuev9_82b1d477_1:

    # alice "Sure thing, daddy~" id alice_menu1_menuev9_82b1d477_1
    alice "Tất nhiên rồi daddy~" id alice_menu1_menuev9_82b1d477_1

# game/characters/alice/speak.rpy:181
translate vietnamese speak_alice_menuev9_b211a70c:

    # alice "Daddy wants to pound [alice]'s little pussy again?"
    alice "Daddy muốn nện âm hộ của [alice] lần nữa chứ?"

# game/characters/alice/speak.rpy:182
translate vietnamese speak_alice_menuev9_2d7dbb9a:

    # alice "Should I dress up for you?"
    alice "Em có nên lên đồ cho anh không?"

# game/characters/alice/speak.rpy:205
translate vietnamese speak_alice_menuev9_1b3457b5:

    # alice "You know I could use you for something..."
    alice "Anh biết em có thể sử dụng anh cho vài việc mà..."

# game/characters/alice/speak.rpy:206
translate vietnamese speak_alice_menuev9_e20c2277:

    # me "Oh? What is it?"
    me "Ồ? Và đó là gì?"

# game/characters/alice/speak.rpy:209
translate vietnamese speak_alice_menuev9_a5d3abf8:

    # alice "I could use some cleaning down there~"
    alice "Em có thể dùng anh để dọn dẹp dưới này~"

# game/characters/alice/speak.rpy:210
translate vietnamese speak_alice_menuev9_376747c4:

    # me "Haha, got it."
    me "Haha, được rồi."

# game/characters/alice/speak.rpy:211
translate vietnamese speak_alice_menuev9_8bd0d7e1:

    # alice "Should I change?"
    alice "Em có nên thay đồ không?"

# game/characters/alice/speak.rpy:217
translate vietnamese speak_alice_menuev9_cb8a02c6:

    # alice "Ohh... This little girl is going to be played by you again mister."
    alice "Ohh... Cô gái nhỏ này sẽ lại được chơi bởi anh, thưa anh."

# game/characters/alice/speak.rpy:227
translate vietnamese speak_alice_menuev9_b971ae2f:

    # me "Wanna reward me once more?"
    me "Muốn thưởng cho em lần nữa không?"

# game/characters/alice/speak.rpy:230
translate vietnamese speak_alice_menuev9_81e18fa7:

    # alice "You can just ask directly you know?"
    alice "Anh chỉ cần hỏi thẳng em thôi mà?"

# game/characters/alice/speak.rpy:231
translate vietnamese speak_alice_menuev9_c1769394:

    # me "Then will you blow me?"
    me "Vậy em sẽ bú kẹc cho anh chứ?"

# game/characters/alice/speak.rpy:234
translate vietnamese speak_alice_menuev9_7b75f538:

    # alice "W-What are you talking about mister? I'm only a little gir.."
    alice "Ô-Ông đang nói gì vậy? Tôi chỉ là một cô bé..."

# game/characters/alice/speak.rpy:235
translate vietnamese speak_alice_menuev9_3820771a:

    # me ".."
    me "..."

# game/characters/alice/speak.rpy:236
translate vietnamese speak_alice_menuev9_388c714f:

    # me "Come to daddy."
    me "Đến với daddy nào."

# game/characters/alice/speak.rpy:239
translate vietnamese speak_alice_menuev9_0b60a0c3:

    # alice "Hehe~"
    alice "Hehe~"

# game/characters/alice/speak.rpy:240
translate vietnamese speak_alice_menuev9_e2b90efc:

    # alice "Lay down, daddy~"
    alice "Nằm xuống đi daddy~"

# game/characters/alice/speak.rpy:241
translate vietnamese speak_alice_menuev9_fec91136:

    # "You do as told and lie down waiting for her to join."
    "Bạn làm như cô ấy nói và nằm xuống đợi cô ấy tham gia."

# game/characters/alice/speak.rpy:242
translate vietnamese speak_alice_menuev9_a69a8806:

    # alice "What outfit do you want this little girl to wear, daddy?"
    alice "Anh muốn em mặc trang phục nào đây?"

# game/characters/alice/speak.rpy:248
translate vietnamese speak_alice_menuev9_f4b76968:

    # alice "Hehe~ You pervert~"
    alice "Hehe~ Đồ biến thái~"

# game/characters/alice/speak.rpy:261
translate vietnamese speak_alice_menuev9_f6adff98:

    # me "Can you give me a footjob?"
    me "Bạn có thể dùng chân được không?"

# game/characters/alice/speak.rpy:266
translate vietnamese speak_alice_menuev9_4bcaf3b9:

    # alice "You're becoming more and more daring."
    alice "Anh đang ngày càng trở nên táo bạo hơn đấy."

# game/characters/alice/speak.rpy:267
translate vietnamese speak_alice_menuev9_f660d172:

    # me "So?"
    me "Thì?"

# game/characters/alice/speak.rpy:268
translate vietnamese speak_alice_menuev9_c313e6b5:

    # alice "And impatient."
    alice "Và mất kiên nhẫn nữa."

# game/characters/alice/speak.rpy:269
translate vietnamese speak_alice_menuev9_f660d172_1:

    # me "So?"
    me "Thì?"

# game/characters/alice/speak.rpy:270
translate vietnamese speak_alice_menuev9_74f05a4e:

    # alice "..."
    alice "..."

# game/characters/alice/speak.rpy:273
translate vietnamese speak_alice_menuev9_e8149e61:

    # alice "Lie down. What do you want me to wear?"
    alice "Nằm xuống đi. Anh muốn em mặc gì?"

# game/characters/alice/speak.rpy:279
translate vietnamese speak_alice_menuev9_35cada6d:

    # alice "Hehe~ You love this outfit so much, right?"
    alice "Hehe~ Anh yêu bộ trang phục này rất nhiều, phải không?"

# game/characters/alice/speak.rpy:289
translate vietnamese speak_alice_menuev9_011de6f2:

    # alice "Time for another workout."
    alice "Đến lúc cho một bài tậ khác rồi."

# game/characters/alice/speak.rpy:294
translate vietnamese speak_alice_menuev9_af5f5d0a:

    # alice "You want to torture my pussy again, daddy?"
    alice "Anh muốn tra tấn âm hộ của em một lần nữa hả?"

# game/characters/alice/speak.rpy:326
translate vietnamese speak_alice_menuev9_8f7b6901:

    # alice "Sure, what is it?"
    alice "Được rồi, vậy nó là gì?"

# game/characters/alice/speak.rpy:566
translate vietnamese speak_alice_alice_str_e98034ac:

    # alice "So you are going to get me to make a gift for myself? How.. original.."
    alice "Vậy anh định bắt em làm một món quà cho chính mình sao?"

# game/characters/alice/speak.rpy:567
translate vietnamese speak_alice_alice_str_f660d172:

    # me "So?"
    me "Thế?"

# game/characters/alice/speak.rpy:571
translate vietnamese speak_alice_alice_str_0115c481:

    # alice "It's simple. I'll start immediately."
    alice "Cũng đơn giản thôi. Em sẽ bắt đầu ngay lập tức."

# game/characters/alice/speak.rpy:572
translate vietnamese speak_alice_alice_str_640dffc0:

    # me "Good luck."
    me "Chúc may mắn."

# game/characters/alice/speak.rpy:573
translate vietnamese speak_alice_alice_str_74f05a4e:

    # alice "..."
    alice "..."

# game/characters/alice/speak.rpy:577
translate vietnamese speak_alice_alice_str_83f16524:

    # alice "Here."
    alice "Đây."

# game/characters/alice/speak.rpy:578
translate vietnamese speak_alice_alice_str_3f5d85fc:

    # me "Good job."
    me "Làm tốt lắm."

# game/characters/alice/speak.rpy:579
translate vietnamese speak_alice_alice_str_37267f30:

    # alice "No problem, I was bored anyway."
    alice "Không sao hết, đằng nào thì em cũng đang chán này."

# game/characters/alice/speak.rpy:580
translate vietnamese speak_alice_alice_str_9dcc12f0:

    # me "[alice]."
    me "[alice]."

# game/characters/alice/speak.rpy:581
translate vietnamese speak_alice_alice_str_74d8ac69:

    # alice "Yeah?"
    alice "Sao vậy?"

# game/characters/alice/speak.rpy:582
translate vietnamese speak_alice_alice_str_4e5be125:

    # me "I've got a gift for you."
    me "Anh có một món quà cho em đây."

# game/characters/alice/speak.rpy:586
translate vietnamese speak_alice_alice_str_5eeaea5b:

    # alice "Cheap."
    alice "Phèn vãi."

# game/characters/alice/speak.rpy:587
translate vietnamese speak_alice_alice_str_10582fda:

    # me "It's called convenient."
    me "Nó được gọi là tiện lợi đấy em."

# game/characters/alice/speak.rpy:588
translate vietnamese speak_alice_alice_str_f6549f2b:

    # alice "Cheap as fuck."
    alice "Giẻ ráck."

# game/characters/alice/speak.rpy:589
translate vietnamese speak_alice_alice_str_570f8a6c:

    # me "You don't want it?"
    me "Em không muốn sao?"

# game/characters/alice/speak.rpy:593
translate vietnamese speak_alice_alice_str_c8de865d:

    # alice "Gimme."
    alice "Đưa em."

# game/characters/alice/speak.rpy:594
translate vietnamese speak_alice_alice_str_aa93b10c:

    # me "Good girl."
    me "Ngoan."

# game/characters/alice/speak.rpy:602
translate vietnamese speak_alice_alice_sch_1a4fbd01:

    # alice "Someone's going to get naughty, right daddy?"
    alice "Ai đó đang trở nên hư hỏng này"

# game/characters/alice/speak.rpy:603
translate vietnamese speak_alice_alice_sch_651554f5:

    # me "I get it you're going to do it?"
    me "Anh hiểu em sẽ làm điều đó sao?"

# game/characters/alice/speak.rpy:604
translate vietnamese speak_alice_alice_sch_cde3d046:

    # alice "Of course, daddy~"
    alice "Tất nhiên rồi~"

# game/characters/alice/speak.rpy:610
translate vietnamese speak_alice_alice_sch_3286c790:

    # alice "Now I got it."
    alice "Giờ thì em hiểu rồi."

# game/characters/alice/speak.rpy:611
translate vietnamese speak_alice_alice_sch_5bc2d7b3:

    # me "Well... Are you going to show me?"
    me "Chà... anh định cho em xem à?"

# game/characters/alice/speak.rpy:612
translate vietnamese speak_alice_alice_sch_cba3e917:

    # alice "Be a good boy and I'll think about it."
    alice "Hãy là một cậu bé ngoan và em sẽ nghĩ về điều đó."

# game/characters/alice/speak.rpy:613
translate vietnamese speak_alice_alice_sch_07ed60be:

    # me "I'm always good."
    me "Anh luôn luôn ngoan mà."

# game/characters/alice/speak.rpy:621
translate vietnamese speak_alice_alice_maid_2614c887:

    # alice "Grace isn't enough for you? You beast~"
    alice "Grace còn chưa đủ cho anh sao? Đồ quái thú~"

# game/characters/alice/speak.rpy:622
translate vietnamese speak_alice_alice_maid_125c7056:

    # alice "I'll be right back."
    alice "Em sẽ quay lại ngay."

# game/characters/alice/speak.rpy:628
translate vietnamese speak_alice_alice_maid_68df90bf:

    # alice "Okay!~ Done!"
    alice "Okay!~ xong rồi!"

# game/characters/alice/speak.rpy:629
translate vietnamese speak_alice_alice_maid_bdab3c6b:

    # me "Sweet!"
    me "Tuyệt!"

# game/characters/alice/speak.rpy:637
translate vietnamese speak_alice_lexi_ten_aae4df87:

    # alice "Why do you want me to make something like this?"
    alice "Tại sao anh lại muốn em làm mấy cái như này vậy?"

# game/characters/alice/speak.rpy:638
translate vietnamese speak_alice_lexi_ten_a8870946:

    # me "Just tell me if you can do it."
    me "Chỉ cần anh biết em có làm được không thôi."

# game/characters/alice/speak.rpy:642
translate vietnamese speak_alice_lexi_ten_3a186df0:

    # alice "No worries. I got it."
    alice "Không sao. Em lo được."

# game/characters/alice/speak.rpy:648
translate vietnamese speak_alice_lexi_ten_4cbbde7b:

    # alice "I made your weird thing."
    alice "Em đã làm xong cái thứ kỳ lạ của anh rồi đây."

# game/characters/alice/speak.rpy:661
translate vietnamese speak_alice_outfits_kiara_c296745b:

    # alice "Who?"
    alice "Ai?"

# game/characters/alice/speak.rpy:662
translate vietnamese speak_alice_outfits_kiara_44e5ad59:

    # me "Well... You won't know her."
    me "Chà... em sẽ không biết cô ấy đâu."

# game/characters/alice/speak.rpy:666
translate vietnamese speak_alice_outfits_kiara_eda2db3c:

    # alice "Ehh.."
    alice "Ể..."

# game/characters/alice/speak.rpy:667
translate vietnamese speak_alice_outfits_kiara_3820771a:

    # me "..."
    me "..."

# game/characters/alice/speak.rpy:668
translate vietnamese speak_alice_outfits_kiara_63ebf2db:

    # alice "What are her sizes?"
    alice "Kích thước của cô ấy là bao nhiêu?"

# game/characters/alice/speak.rpy:669
translate vietnamese speak_alice_outfits_kiara_32898ed9:

    # th "As if I know..."
    th "Làm như là mình biết vậy..."

# game/characters/alice/speak.rpy:670
translate vietnamese speak_alice_outfits_kiara_7b0f67e5:

    # me "Make them Mina's size."
    me "Làm cho chúng có kích thước giống Mina đi."

# game/characters/alice/speak.rpy:671
translate vietnamese speak_alice_outfits_kiara_5570ddf0:

    # th "I think she can make them fit herself with some power. Probably."
    th "Mình nghĩ em ấy có thể làm cho chúng vừa với năng lực của em ấy. Chắc vậy."

# game/characters/alice/speak.rpy:675
translate vietnamese speak_alice_outfits_kiara_b00edcf0:

    # alice "Okay, Mr. Adventurer."
    alice "Được rồi, thưa nhà thám hiểm."

# game/characters/alice/speak.rpy:676
translate vietnamese speak_alice_outfits_kiara_82c76ca5:

    # me "Thanks, [alice]."
    me "Cảm ơn nha [alice]."

# game/characters/alice/speak.rpy:680
translate vietnamese speak_alice_outfits_kiara_02ce5de8:

    # alice "When will you tell me about your rendezvous outside?"
    alice "Khi nào anh sẽ cho em biết về điểm hẹn của anh ở bên ngoài đây?"

# game/characters/alice/speak.rpy:681
translate vietnamese speak_alice_outfits_kiara_230731c3:

    # me "Maybe tomorrow.. Or a day after tomorrow.. Or.."
    me "Có lẽ ngày mai... Hoặc ngày kia... Hoặc..."

# game/characters/alice/speak.rpy:685
translate vietnamese speak_alice_outfits_kiara_ebad61b8:

    # alice "Or never?"
    alice "Hoặc không bao giờ?"

# game/characters/alice/speak.rpy:686
translate vietnamese speak_alice_outfits_kiara_71766273:

    # me "Yeah.. Yeah. That's a compromise I could accept."
    me "Yeah... Yeah là một thỏa hiệp mà anh có thể chấp nhận được."

# game/characters/alice/speak.rpy:687
translate vietnamese speak_alice_outfits_kiara_74f05a4e:

    # alice ".."
    alice "..."

# game/characters/alice/speak.rpy:691
translate vietnamese speak_alice_outfits_kiara_a11d089c:

    # alice "Well, whatever. Just don't bring your hoes home."
    alice "Sao cũng được. Đừng đem bồ anh về nhà đấy."

# game/characters/alice/speak.rpy:695
translate vietnamese speak_alice_outfits_kiara_dc937837:

    # me "Sometimes it's just stronger than me."
    me "Đôi khi con tim sẽ áp đảo anh đấy."

# game/characters/alice/speak.rpy:699
translate vietnamese speak_alice_outfits_kiara_eda2db3c_1:

    # alice "Ehh.."
    alice "Ể..."

# game/characters/alice/speak.rpy:700
translate vietnamese speak_alice_outfits_kiara_80a82614:

    # alice "You're hopeless, [me]."
    alice "Anh đúng là vô vọng rồi [me]."

# game/characters/alice/speak.rpy:704
translate vietnamese speak_alice_outfits_kiara_7dee622e:

    # alice "When doing various things outside, just consider, we're also living here. Don't put us in danger just to get laid, okay?"
    alice "Khi làm những việc khác nhau bên ngoài, hãy xem xét, chúng ta cũng đang sống ở đây. Đừng khiến chúng ta gặp nguy hiểm, được chứ?"

# game/characters/alice/speak.rpy:706
translate vietnamese speak_alice_outfits_kiara_188d9695:

    # me "Got it. Don't worry."
    me "Hiểu rồi. Đừng lo."

# game/characters/alice/speak.rpy:712
translate vietnamese speak_alice_outfits_kiara_4ee90635:

    # alice "Your outside hoe again?"
    alice "Lại là vợ bé của anh ở ngoài kia hả?"

# game/characters/alice/speak.rpy:713
translate vietnamese speak_alice_outfits_kiara_5f5cfd66:

    # me "I wouldn't call her that, though."
    me "Anh không gọi cô ấy như vậy đâu."

# game/characters/alice/speak.rpy:717
translate vietnamese speak_alice_outfits_kiara_4963bffa:

    # alice "Whatever, Mina's size?"
    alice "sao cũng được, cỡ của Mina hả?"

# game/characters/alice/speak.rpy:718
translate vietnamese speak_alice_outfits_kiara_3f6c9cde:

    # me "Mina's size."
    me "Cỡ của Mina."

# game/characters/alice/speak.rpy:729
translate vietnamese speak_alice_kiara_ang_bf733006:

    # alice "Here you go."
    alice "Của anh đây."

# game/characters/alice/speak.rpy:730
translate vietnamese speak_alice_kiara_ang_52a1db70:

    # me "Many thanks, [alice]."
    me "Cảm ơn em rất nhiều."

# game/characters/alice/speak.rpy:741
translate vietnamese speak_alice_kiara_par_ab1d8716:

    # alice "Enjoy the party.."
    alice "Thưởng thức bữa tiệc nha..."

# game/characters/alice/speak.rpy:742
translate vietnamese speak_alice_kiara_par_1ba957cc:

    # me "Hey, don't be jealous."
    me "Này, đừng ghen tị mà."

# game/characters/alice/speak.rpy:743
translate vietnamese speak_alice_kiara_par_29314cd1:

    # alice "As if someone would be jealous about you."
    alice "làm như có ai sẽ ghen tị với anh vậy."

# game/characters/alice/speak.rpy:744
translate vietnamese speak_alice_kiara_par_282f3876:

    # me "Hahaha!"
    me "Hahaha!"

# game/characters/alice/speak.rpy:755
translate vietnamese speak_alice_kiara_nym_10a760a0:

    # alice "Take it."
    alice "Lấy đi."

# game/characters/alice/speak.rpy:756
translate vietnamese speak_alice_kiara_nym_96db5d1c:

    # me "Thanks! What's up with you?"
    me "Cảm ơn! Có chuyện gì với em vậy?"

# game/characters/alice/speak.rpy:757
translate vietnamese speak_alice_kiara_nym_9ed9c9ac:

    # alice "Nothing, take it to your slut, and let me rest. It was annoying doing this fishnet."
    alice "Không có gì cả, hãy đưa nó cho con đĩ của anh đi, và để em nghỉ đi. Thật là khó chịu khi phải làm cái lưới cá này mà."

# game/characters/alice/speak.rpy:758
translate vietnamese speak_alice_kiara_nym_f9b08f48:

    # me "Heads up."
    me "Ngẩng đầu lên đi."

# game/characters/alice/speak.rpy:759
translate vietnamese speak_alice_kiara_nym_74f05a4e:

    # alice "..."
    alice "..."

# game/characters/alice/speak.rpy:760
translate vietnamese speak_alice_kiara_nym_8e0c5a1a:

    # me "Why did you make it, then?"
    me "Vậy tại sao em làm ra nó?"

# game/characters/alice/speak.rpy:764
translate vietnamese speak_alice_kiara_nym_fb321aeb:

    # alice "As if I knew, idiot. Go already."
    alice "Làm như em biết vậy đồ ngốc. Đi luôn đi."

# game/characters/alice/speak.rpy:765
translate vietnamese speak_alice_kiara_nym_a0a697c9:

    # me "Thank you."
    me "Cảm ơn."

# game/characters/alice/speak.rpy:766
translate vietnamese speak_alice_kiara_nym_7d834efa:

    # alice "I'm amazing. I know. Go, go."
    alice "Em thật tuyệt vời. Em biết. Đi đi."

# game/characters/alice/speak.rpy:767
translate vietnamese speak_alice_kiara_nym_3820771a:

    # me "..."
    me "..."

translate vietnamese strings:

    # game/characters/alice/speak.rpy:331
    old "She's responsible for crafting things at the Jade Hotel. Every so often she takes on the role of the scavenger."
    new "Cô ấy chịu trách nhiệm chế tạo những thứ đồ ở khách sạn Jade. Cô ấy thường xuyên đảm nhận vai trò của người kiếm đồ."

    # game/characters/alice/speak.rpy:352
    old "She’s been at the Jade Hotel since the beginning. She arrived on the fifth day of The End."
    new "Cô ấy đã ở khách sạn Jade từ những ngày đầu. Cô ấy đến vào ngày thứ năm của The End."

    # game/characters/alice/speak.rpy:382
    old "The End"
    new "The End"

    # game/characters/alice/speak.rpy:382
    old "On the outside, curiosity remains the swiftest way to the afterlife."
    new "Nhìn bên ngoài, sự tò mò vẫn là con đường nhanh nhất dẫn đến thế giới bên kia."

    # game/characters/alice/speak.rpy:397
    old "At night, in the depths of fog, the whispers appear. They have turned anyone that has heard them crazy."
    new "Vào ban đêm, trong làn sương mù sâu thẳm, những tiếng thì thầm hiện ra. Chúng đã biến bất cứ ai nghe thấy chúng trở nên phát điên."

    # game/characters/alice/speak.rpy:406
    old "The rumor says that three minutes after steeping into the fog at night, your shadow rises from the ground and follows you. It's smiling literally from ear to ear. Victims all claim the whispers you hear outside come from the shadow."
    new "Tin đồn nói rằng ba phút sau khi bước vào sương mù trong đêm, bóng của bạn nhô lên khỏi mặt đất và đi theo bạn. Nó mỉm cười từ tai này sang tai khác. Các nạn nhân đều khẳng định những lời thì thầm mà bạn nghe thấy bên ngoài đến từ bóng tối."

    # game/characters/alice/speak.rpy:418
    old "The hotel's owner's name was Jack. He disappeared a week before my arrival."
    new "Chủ nhân trước của khách sạn tên là Jack. Anh ấy biến mất một tuần trước khi tôi đến."

    # game/characters/alice/speak.rpy:552
    old "Outfits"
    new "Trang phục"

    # game/characters/alice/speak.rpy:552
    old "Weave"
    new "Dệt"
# : Translation updated at 2022-02-19 01:56

translate vietnamese strings:

    # game/characters/alice/speak.rpy:21
    old "I need another cosplay outfit.. {gold}(Story){/gold}"
    new "Anh cần một bộ đồ cosplay khác... {gold}(Cốt truyện){/gold}"

    # game/characters/alice/speak.rpy:21
    old "I need a magical girl outfit.. {gold}(Story){/gold}"
    new "Anh cần một bộ đồ nữ pháp sư... {gold}(Cốt truyện){/gold}"
# : Translation updated at 2022-05-12 09:37

# game/characters/alice/speak.rpy:634
translate vietnamese speak_alice_alice_nun_61d78026:

    # alice "A nun outfit? For me? What for?"
    alice "Một bộ đồ nữ tu? Cho em? Để làm gì?"

# game/characters/alice/speak.rpy:635
translate vietnamese speak_alice_alice_nun_da213152:

    # me "It's a cosplay like any else."
    me "Đó là một bộ cosplay giống như bất kỳ người nào khác."

# game/characters/alice/speak.rpy:639
translate vietnamese speak_alice_alice_nun_b01a0f65:

    # alice "A nun, though? You really get a boner thinking about nuns?"
    alice "Tuy nhiên, một nữ tu á? Anh thực sự thấy hứng tình nghĩ về các nữ tu sao?"

# game/characters/alice/speak.rpy:640
translate vietnamese speak_alice_alice_nun_d728a550:

    # me "No, I get it thinking about you in a nun outfit."
    me "Không, anh thấy nứng khi nghĩ về em trong trang phục nữ tu."

# game/characters/alice/speak.rpy:644
translate vietnamese speak_alice_alice_nun_31f6bbb4:

    # alice "That makes more sense."
    alice "Cái này nghe lọt tai hơn đấy."

# game/characters/alice/speak.rpy:645
translate vietnamese speak_alice_alice_nun_894cda5c:

    # alice "Gimme a moment."
    alice "Đợi em một chút."

# game/characters/alice/speak.rpy:652
translate vietnamese speak_alice_alice_nun_ded409e4:

    # alice "How's that?"
    alice "Thế nào?"

# game/characters/alice/speak.rpy:653
translate vietnamese speak_alice_alice_nun_210db714:

    # me "Nice. Fits you well."
    me "Tuyệt lắm. Rất hợp với em."

# game/characters/alice/speak.rpy:657
translate vietnamese speak_alice_alice_nun_8e63d855:

    # alice "Do you want to play test this new outfit?" nointeract
    alice "Anh muốn chơi thử với bộ đồ này không?" nointeract

# game/characters/alice/speak.rpy:672
translate vietnamese speak_alice_alice_nun_1468f3c4:

    # alice "No fun~"
    alice "KHông vui chút nào hết~"

# game/characters/alice/speak.rpy:673
translate vietnamese speak_alice_alice_nun_967e52eb:

    # me "Sorry babe."
    me "Xin lỗi em yêu."

translate vietnamese strings:

    # game/characters/alice/speak.rpy:656
    old "Let's skip this"
    new "Hãy bỏ cái này đi"
# : Translation updated at 2022-08-20 13:45

translate vietnamese strings:

    # game/characters/alice/speak.rpy:169
    old "> Outfits"
    new "> Trang phục"

    # game/characters/alice/speak.rpy:171
    old "Nun {#visible}"
    new "Sơ {#visible}"
# : Translation updated at 2022-09-08 19:14

# game/characters/alice/speak.rpy:217
translate vietnamese speak_alice_sexMenu_1b3457b5:

    # alice "You know I could use you for something.."
    alice "Con biết ta có thể dùng con để làm gì đó đấy..."

# game/characters/alice/speak.rpy:218
translate vietnamese speak_alice_sexMenu_e20c2277:

    # me "Oh? What is it?"
    me "Oh? Và đó là gì vậy?"

# game/characters/alice/speak.rpy:222
translate vietnamese speak_alice_sexMenu_a5d3abf8:

    # alice "I could use some cleaning down there~"
    alice "Ta có thể cần được lau dọn dưới này đấy~"

# game/characters/alice/speak.rpy:223
translate vietnamese speak_alice_sexMenu_376747c4:

    # me "Haha, got it."
    me "Haha, rõ rồi."

# game/characters/alice/speak.rpy:224
translate vietnamese speak_alice_sexMenu_8bd0d7e1:

    # alice "Should I change?"
    alice "Em có nên thay đồ không?"

# game/characters/alice/speak.rpy:230
translate vietnamese speak_alice_sexMenu_cb8a02c6:

    # alice "Ohh.. This little girl is going to be played by you again mister."
    alice "Ohh... This little girl is going to be played by you again mister."

# game/characters/alice/speak.rpy:240
translate vietnamese speak_alice_sexMenu_b971ae2f:

    # me "Wanna reward me once more?"
    me "Muốn thưởng cho anh lần nữa không?"

# game/characters/alice/speak.rpy:244
translate vietnamese speak_alice_sexMenu_81e18fa7:

    # alice "You can just ask directly you know?"
    alice "Anh có thể hỏi trực tiếp luôn mà anh biết chứ?"

# game/characters/alice/speak.rpy:245
translate vietnamese speak_alice_sexMenu_c1769394:

    # me "Then will you blow me?"
    me "Vậy thì em sẽ bú buồi cho anh chứ?"

# game/characters/alice/speak.rpy:249
translate vietnamese speak_alice_sexMenu_7b75f538:

    # alice "W-What are you talking about mister? I'm only a little girl.."
    alice "Ô-Ông đang nói về cái gì vậy? Cháu chỉ là một cô gái nhỏ thôi mà..."

# game/characters/alice/speak.rpy:250
translate vietnamese speak_alice_sexMenu_3820771a:

    # me "..."
    me "..."

# game/characters/alice/speak.rpy:251
translate vietnamese speak_alice_sexMenu_388c714f:

    # me "Come to daddy."
    me "Đến với daddy nào."

# game/characters/alice/speak.rpy:255
translate vietnamese speak_alice_sexMenu_0b60a0c3:

    # alice "Hehe~"
    alice "Hehe~"

# game/characters/alice/speak.rpy:256
translate vietnamese speak_alice_sexMenu_e2b90efc:

    # alice "Lay down, daddy~"
    alice "Nằm xuống đi, daddy~"

# game/characters/alice/speak.rpy:257
translate vietnamese speak_alice_sexMenu_fec91136:

    # "You do as told and lie down waiting for her to join."
    "Bạn làm như những gì được nói và nằm xuống đợi cô ấy tham gia."

# game/characters/alice/speak.rpy:258
translate vietnamese speak_alice_sexMenu_a69a8806:

    # alice "What outfit do you want this little girl to wear, daddy?"
    alice "Vậy daddy muốn cô gái nhỏ này mặc trang phục gì nào?"

# game/characters/alice/speak.rpy:264
translate vietnamese speak_alice_sexMenu_f4b76968:

    # alice "Hehe~ You pervert~"
    alice "Hehe~ Đồ biến thái~"

# game/characters/alice/speak.rpy:277
translate vietnamese speak_alice_sexMenu_f6adff98:

    # me "Can you give me a footjob?"
    me "Em có thể footjob cho anh được không?"

# game/characters/alice/speak.rpy:281
translate vietnamese speak_alice_sexMenu_4bcaf3b9:

    # alice "You're becoming more and more daring."
    alice "anh ngày càng trở nên táo bạo hơn đấy."

# game/characters/alice/speak.rpy:282
translate vietnamese speak_alice_sexMenu_f660d172:

    # me "So?"
    me "Vậy?"

# game/characters/alice/speak.rpy:283
translate vietnamese speak_alice_sexMenu_c313e6b5:

    # alice "And impatient."
    alice "Và mất kiên nhẫn nữa."

# game/characters/alice/speak.rpy:284
translate vietnamese speak_alice_sexMenu_f660d172_1:

    # me "So?"
    me "Thì?"

# game/characters/alice/speak.rpy:285
translate vietnamese speak_alice_sexMenu_74f05a4e:

    # alice "..."
    alice "..."

# game/characters/alice/speak.rpy:289
translate vietnamese speak_alice_sexMenu_e8149e61:

    # alice "Lie down. What do you want me to wear?"
    alice "Nằm xuống đi. Muốn em mặc gì nào?"

# game/characters/alice/speak.rpy:295
translate vietnamese speak_alice_sexMenu_35cada6d:

    # alice "Hehe~ You love this outfit so much, right?"
    alice "Hehe~ Anh yêu bộ đồ này rất nhiều có đúng không?"

# game/characters/alice/speak.rpy:305
translate vietnamese speak_alice_sexMenu_011de6f2:

    # alice "Time for another workout."
    alice "Đến lúc để tập luyện tiếp rồi."

# game/characters/alice/speak.rpy:310
translate vietnamese speak_alice_sexMenu_af5f5d0a:

    # alice "You want to torture my pussy again, daddy?"
    alice "Anh lại muốn tra tấn cái lồn của em nữa sao?"

# game/characters/alice/speak.rpy:341
translate vietnamese speak_alice_sexMenu_8f7b6901:

    # alice "Sure, what is it?"
    alice "Được thôi, nó là gì vậy?"
# : Translation updated at 2022-11-13 15:18

translate vietnamese strings:

    # game/characters/alice/speak.rpy:171
    old "Playmate {#visible}"
    new "Bạn chịch {#visible}"
# : Translation updated at 2023-08-04 16:46

# game/characters/alice/speak.rpy:198
translate vietnamese speak_alice_menuev9_15096fb7:

    # alice "Get ready for a love bite~"
    alice "Hãy chuẩn bị cho một cú cắn yêu đi~"

# game/characters/alice/speak.rpy:199
translate vietnamese speak_alice_menuev9_88eab5f5:

    # me "Haha."
    me "Haha."

translate vietnamese strings:

    # game/characters/alice/speak.rpy:171
    old "Vampire {#visible}"
    new "Ma cà rồng {#visible}"
