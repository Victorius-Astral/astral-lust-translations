# : Translation updated at 2021-12-24 07:09

# game/characters/alice/cunn_1.rpy:25
translate vietnamese alice_cunn1_c839e5b0:

    # "You can't wait anymore."
    "Bạn không thể chờ đợi được nữa." # TL

# game/characters/alice/cunn_1.rpy:35
translate vietnamese alice_cunn1_a688eb90:

    # "You don't let her wait any longer and bury yourself between her thighs."
    "Không để cô ấy đợi lâu hơn bạn vùi mặt vào giữa đùi của cô ấy." # TL

# game/characters/alice/cunn_1.rpy:36
translate vietnamese alice_cunn1_9403bafe:

    # alice "Ummm~"
    alice "Ưmmm~" # TL

# game/characters/alice/cunn_1.rpy:37
translate vietnamese alice_cunn1_c03d28ea:

    # "As your tongue dances on her pussy, you can feel how terrifyingly tight she actually is."
    "Khi lưỡi của bạn nhảy múa trên âm hộ của cô ấy, bạn có thể cảm thấy cô ấy thực sự chặt đến mức nào." # TL

# game/characters/alice/cunn_1.rpy:38
translate vietnamese alice_cunn1_5e89a703:

    # alice "Mmm.. just like that."
    alice "Mmm... Cứ như vậy đi." # TL

# game/characters/alice/cunn_1.rpy:39
translate vietnamese alice_cunn1_6de696ef:

    # "She seems to melt under your service."
    "Cô ấy dường như đang tan chảy dưới sự phục vụ của bạn." # TL

# game/characters/alice/cunn_1.rpy:49
translate vietnamese alice_cunn1_f2b053cf:

    # "You start to lick her even faster, making her ecstasy reach new levels."
    "Bạn bắt đầu liếm cô ấy nhanh hơn, làm cho sự khoái cảm của cô ấy đạt đến cấp độ mới." # TL

# game/characters/alice/cunn_1.rpy:50
translate vietnamese alice_cunn1_dbd8e68f:

    # alice "Ahhh! More, lick me more!"
    alice "Ahhh! Nữa đi, liếm em nữa đi!" # TL

# game/characters/alice/cunn_1.rpy:51
translate vietnamese alice_cunn1_5245ff49:

    # "[alice] appears to completely give in to the pleasure. You can't believe a pussy can taste this good."
    "[alice] dường như hoàn toàn đầu hàng trước cơn khoái lạc. Bạn không thể tin được một cái lồn có thể ngon như thế này." # TL

# game/characters/alice/cunn_1.rpy:52
translate vietnamese alice_cunn1_830b5768:

    # alice "Mmnn~ [me]~"
    alice "Mmnn~ [me]~" # TL # Victorius - AI

# game/characters/alice/cunn_1.rpy:53
translate vietnamese alice_cunn1_29b76d86:

    # "As she calls your name so playfully in her moans your dick is about to burst from your pants."
    "Cái cách cô ấy gọi tên bạn một cách khiêu gợi trong tiếng rên của cô ấy, khiến thằng nhỏ của bạn sắp sửa bung ra khỏi quần." # TL

# game/characters/alice/cunn_1.rpy:63
translate vietnamese alice_cunn1_f795fb20:

    # me "Who's a good little girl?"
    me "Ai là con đĩ của anh nào?" # TL

# game/characters/alice/cunn_1.rpy:65
translate vietnamese alice_cunn1_8fda8a83:

    # alice "I am! Please make your little girl cum daddy!"
    alice "Là em! Hãy làm cho con đĩ của anh ra đi!" # TL

# game/characters/alice/cunn_1.rpy:66
translate vietnamese alice_cunn1_50b23672:

    # th "Shit, I wish I could fuck her right now!"
    th "Đạ mấu, Mình ước gì mình có thể đụ em ấy ngay bây giờ!" # TL

# game/characters/alice/cunn_1.rpy:67
translate vietnamese alice_cunn1_42c3730b:

    # alice "Ahh~ I-I'm cumminggg!!!"
    alice "Ahh~ Em đang ra!!!" # TL

# game/characters/alice/cunn_1.rpy:73
translate vietnamese alice_cunn1_f73075dd:

    # "[alice] is cumming!"
    "[alice] Đang ra!" # TL

# game/characters/alice/cunn_1.rpy:78
translate vietnamese alice_cunn1_f73075dd_1:

    # "[alice] is cumming!"
    "[alice] Đang ra!" # TL

# game/characters/alice/cunn_1.rpy:86
translate vietnamese alice_cunn1_c32f5cc2:

    # alice "So gooooood hehe~"
    alice "Thật tuyệttttt hehe~" # TL

# game/characters/alice/cunn_1.rpy:87
translate vietnamese alice_cunn1_8e0312e6:

    # "You leave her to rest."
    "Bạn để cô ấy nghỉ ngơi." # TL
# : Translation updated at 2022-02-26 15:07

# game/characters/alice/cunn_1.rpy:50
translate vietnamese alice_cunn1_0e51e6f6:

    # alice "Ahhh!~"
    alice "Ahhh!~" # TL # Victorius - AI

