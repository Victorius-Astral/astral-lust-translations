# : Translation updated at 2021-12-24 07:09

# game/characters/alice/ev_11.rpy:9
translate vietnamese alice_11_9f30d9e0:

    # me "How is my young woman feeling?"
    me "Người phụ nữ trẻ của anh cảm thấy thế nào rồi?" # TL

# game/characters/alice/ev_11.rpy:14
translate vietnamese alice_11_59669d75:

    # alice "Oh, wow. You're joking about taking my virginity."
    alice "Oh, quào. Anh còn nói đùa về việc lấy đi trinh tiết của em đấy." # TL

# game/characters/alice/ev_11.rpy:15
translate vietnamese alice_11_e5c0f9f9:

    # me "What joking, it was a serious question."
    me "Đùa cái gì, đó là một câu hỏi nghiêm túc đấy." # TL

# game/characters/alice/ev_11.rpy:16
translate vietnamese alice_11_5e930aad:

    # alice "Yeah, sure, be honest, you're proud of yourself, aren't you?"
    alice "Vâng, chắc chắn rồi, hãy nói thật đi, anh tự hào về chính mình, phải không?" # TL

# game/characters/alice/ev_11.rpy:17
translate vietnamese alice_11_2924c5fd:

    # me "Pff. Of course I am. I got the best girl in the world for myself."
    me "Pff. Tất nhiên rồi. Anh đã có được cô gái tuyệt nhất trên thế giới cho riêng mình mà lị." # TL

# game/characters/alice/ev_11.rpy:23
translate vietnamese alice_11_f3891591:

    # alice "Good point."
    alice "Được đấy." # TL

# game/characters/alice/ev_11.rpy:24
translate vietnamese alice_11_3274361e:

    # me "So, what's up?"
    me "Vậy, có chuyện gì sao?" # TL

# game/characters/alice/ev_11.rpy:28
translate vietnamese alice_11_28365e37:

    # alice "I think, we don't have enough outfits."
    alice "Em nghĩ, chúng ta không có đủ quần áo." # TL

# game/characters/alice/ev_11.rpy:29
translate vietnamese alice_11_ca1d5247:

    # me "Mhm, I could use something new."
    me "Mhm, anh có thể mặc một cái gì đó mới đấy." # TL

# game/characters/alice/ev_11.rpy:33
translate vietnamese alice_11_4e3af0ca:

    # alice "I wasn't talking about you, Mr. Narcissist. Two sets are enough for you."
    alice "Tôi không nói về ông đâu ngài tự luyến. Hai bộ là quá đủ cho anh rồi." # TL

# game/characters/alice/ev_11.rpy:34
translate vietnamese alice_11_ca1ef9e1:

    # me ".. Well, thanks?"
    me "...Chà, cảm ơn?" # TL

# game/characters/alice/ev_11.rpy:38
translate vietnamese alice_11_64ec7548:

    # alice "Lexi definitely needs something new, Mina too. Grace can manage, but she could use some more."
    alice "Lexi chắc chắn cần một cái gì đó mới, Mina cũng vậy. Grace thì chắc cũng xoay sở được, Nhưng có thể em ấy sẽ cần thêm." # TL

# game/characters/alice/ev_11.rpy:39
translate vietnamese alice_11_bf436f67:

    # me "Let me guess, I'm supposed to gather materials for that."
    me "Để anh đoán nhá, anh là người phải thu thập vải đúng không?" # TL

# game/characters/alice/ev_11.rpy:43
translate vietnamese alice_11_dd215bd0:

    # alice "Look at him. How smart, good boyfriend~"
    alice "Nhìn anh ấy này. Thật thông minh. một người Bạn trai tốt~" # TL

# game/characters/alice/ev_11.rpy:44
translate vietnamese alice_11_5c0dfe33:

    # me "Isn't it a little early in our relationship for blackmail?"
    me "Không phải là hơi sớm để tống tiền mối quan hệ của chúng ta à?" # TL

# game/characters/alice/ev_11.rpy:48
translate vietnamese alice_11_09e44c27:

    # alice "What blackmail, we both know who's going to benefit the most from new outfits."
    alice "Tống tiền cái gì, cả hai chúng ta đều biết ai sẽ là người hưởng lợi nhiều nhất từ những bộ trang phục mới đấy." # TL

# game/characters/alice/ev_11.rpy:49
translate vietnamese alice_11_3820771a:

    # me "..."
    me "..."

# game/characters/alice/ev_11.rpy:50
translate vietnamese alice_11_c71906cf:

    # th "Can't deny."
    th " Cũng không thể phủ nhận được." # TL

# game/characters/alice/ev_11.rpy:51
translate vietnamese alice_11_8cba92d4:

    # alice "You just thought you can't deny it, right?"
    alice "Anh vừa nghĩ rằng anh không thể phủ nhận nó, phải không?" # TL

# game/characters/alice/ev_11.rpy:52
translate vietnamese alice_11_af139e4b:

    # me "Are you reading my mind?"
    me "Em có đang đọc suy nghĩ của anh không đấy?" # TL

# game/characters/alice/ev_11.rpy:56
translate vietnamese alice_11_7936f166:

    # alice "Every good girl needs to know what her daddy has on his mind.."
    alice "Mọi cô gái ngoan đều cần phải biết daddy của mình nghĩ gì..." # TL

# game/characters/alice/ev_11.rpy:57
translate vietnamese alice_11_f36d58ed:

    # me "Then you're a very good girl."
    me "Vậy thì em phải là một cô gái rất tốt." # TL

# game/characters/alice/ev_11.rpy:61
translate vietnamese alice_11_8922f1b9:

    # alice "Only good?"
    alice "Chỉ tốt thôi á?" # TL

# game/characters/alice/ev_11.rpy:62
translate vietnamese alice_11_fcafb8dd:

    # me "The best girl."
    me "Cô gái tuyệt nhất." # TL

# game/characters/alice/ev_11.rpy:67
translate vietnamese alice_11_f968e0b2:

    # alice "Mhm~"
    alice "Ừ hứ~" # TL

# game/characters/alice/ev_11.rpy:71
translate vietnamese alice_11_40d78429:

    # alice "Mmmmnmmm~"
    alice "Mmmmnmmm~" # TL # Victorius - AI

# game/characters/alice/ev_11.rpy:72
translate vietnamese alice_11_5e754567:

    # "[alice] kisses you passionately, her tongue dancing around yours."
    "[alice] hôn bạn say đắm, lưỡi cô ấy nhảy múa xung quanh lưỡi bạn." # TL

# game/characters/alice/ev_11.rpy:76
translate vietnamese alice_11_300c246d:

    # alice "Your reward, daddy~"
    alice "Phần thưởng của anh này~" # TL

# game/characters/alice/ev_11.rpy:77
translate vietnamese alice_11_7f0c7ce2:

    # me "Shit, I could use more rewards like this."
    me "Đù, Anh có thể nhận nhiều phần thưởng như thế này hơn đấy." # TL

# game/characters/alice/ev_11.rpy:81
translate vietnamese alice_11_4300f2d4:

    # alice "You bet."
    alice "Cá là vậy." # TL

# game/characters/alice/ev_11.rpy:82
translate vietnamese alice_11_62b6ee04:

    # me "So when do you want to work on these outfits?"
    me "Vậy khi nào em muốn làm những bộ quần áo này?" # TL

# game/characters/alice/ev_11.rpy:86
translate vietnamese alice_11_c01d5466:

    # alice "Hmm.. I don't know, I need to design them first. It might take a while, remember to come back when I'm ready."
    alice "Hmm... Em cũng không biết nữa, em cần thiết kế chúng trước. Có thể mất một lúc, hãy nhớ quay lại khi em đã sẵn sàng." # TL

# game/characters/alice/ev_11.rpy:87
translate vietnamese alice_11_8a815fb0:

    # me "Come back when you're ready? How would I know that?"
    me "Quay lại khi em đã sẵn sàng? Làm sao anh biết được điều đó?" # TL

# game/characters/alice/ev_11.rpy:91
translate vietnamese alice_11_aea25f1d:

    # alice "Read the changelog."
    alice "đọc bảng cập nhật phiên bản." # TL

# game/characters/alice/ev_11.rpy:92
translate vietnamese alice_11_314086b3:

    # me "What?"
    me "Cái gì?" # TL

# game/characters/alice/ev_11.rpy:96
translate vietnamese alice_11_a6167a27:

    # alice "What?"
    alice "Hả?" # TL

# game/characters/alice/ev_11.rpy:97
translate vietnamese alice_11_3820771a_1:

    # me "..."
    me "..."

# game/characters/alice/ev_11.rpy:98
translate vietnamese alice_11_f6c0f697:

    # alice "I need to go."
    alice "Em phải đi rồi." # TL

# game/characters/alice/ev_11.rpy:99
translate vietnamese alice_11_947a6876:

    # me "Wait."
    me "Đợi đã." # TL

# game/characters/alice/ev_11.rpy:100
translate vietnamese alice_11_7efbc402:

    # alice "See ya!"
    alice "Gặp lại sau!" # TL

# game/characters/alice/ev_11.rpy:103
translate vietnamese alice_11_7a6affff:

    # "[alice] left in a hurry."
    "[alice] rời đi một cách vội vàng." # TL

# game/characters/alice/ev_11.rpy:104
translate vietnamese alice_11_a6221258:

    # th "What was that?"
    th "Cái gì vậy trời?" # TL
