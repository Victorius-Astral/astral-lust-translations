# TODO: Translation updated at 2021-12-24 07:09

# game/characters/alice/sleep.rpy:8
translate vietnamese sleep_alice_23ee33cd:

    # th "[alice].. [alice].. Little, horny [alice].."
    th "[alice].. [alice].. Một chút horny [alice].."

# game/characters/alice/sleep.rpy:10
translate vietnamese sleep_alice_9ebac051:

    # th "She looks so innocent when asleep."
    th "Em ấy trông rất ngây thơ khi ngủ."

# game/characters/alice/sleep.rpy:12
translate vietnamese sleep_alice_3fb5372e:

    # th "Little, blue sleeping angel. If I didn't know her better, I would believe it."
    th "Thiên thần nhỏ, màu xanh đang ngủ. Nếu mình không biết em ấy rõ hơn, mình sẽ tin điều đó."

# game/characters/alice/sleep.rpy:16
translate vietnamese sleep_alice_f4e11991:

    # th "Her skin is so soft.. I wonder how she does it in the apocalypse."
    th "Da cô ấy thật mềm .. Tôi tự hỏi cô ấy làm điều đó trong ngày tận thế như thế nào."

# game/characters/alice/sleep.rpy:19
translate vietnamese sleep_alice_26d391f2:

    # alice "Ym?"
    alice "Ym?"

# game/characters/alice/sleep.rpy:22
translate vietnamese sleep_alice_3820771a:

    # me "..."
    me "..."

# game/characters/alice/sleep.rpy:25
translate vietnamese sleep_alice_2c0a3960:

    # th "She didn't kill me previously, this time it won't be very bad.. Probably."
    th "Trước đây em ấy không giết mình, lần này sẽ không tệ lắm đâu .. Có lẽ."

# game/characters/alice/sleep.rpy:28
translate vietnamese sleep_alice_e83b84f9:

    # alice "[me]?"
    alice "[me]?"

# game/characters/alice/sleep.rpy:31
translate vietnamese sleep_alice_6ac6e842:

    # alice "Why am I not surprised?"
    alice "Tại sao em lại không ngạc nhiên nhỉ?"

# game/characters/alice/sleep.rpy:32
translate vietnamese sleep_alice_3e644d6e:

    # me "Maybe you have superpowers."
    me "Có thể em có siêu năng lực."

# game/characters/alice/sleep.rpy:35
translate vietnamese sleep_alice_97e71df6:

    # alice "Yeah, I'm super girl."
    alice "Yeah, em là super girl."

# game/characters/alice/sleep.rpy:37
translate vietnamese sleep_alice_09c10c4a:

    # alice "So how about I kick your ass?"
    alice "Vậy em đá đít anh thì sao?"

# game/characters/alice/sleep.rpy:38
translate vietnamese sleep_alice_3820771a_1:

    # me "..."
    me "..."

# game/characters/alice/sleep.rpy:39
translate vietnamese sleep_alice_3b33b360:

    # alice "Let me sleep, have you never heard of a beauty sleep?"
    alice "Hãy để em ngủ đi"

# game/characters/alice/sleep.rpy:53
translate vietnamese sleep_alice_leave_7adba209:

    # me "Sorry. I will leave you."
    me "Xin lỗi. Anh sẽ chim cút đây."

# game/characters/alice/sleep.rpy:54
translate vietnamese sleep_alice_leave_1f6fdc10:

    # alice "Good boy."
    alice "OK."

# game/characters/alice/sleep.rpy:60
translate vietnamese sleep_alice_sleepover_023d97dc:

    # "{color=#FF958F}[[Relationship check passed]{/color}"
    "{color=#FF958F}[[Mức độ Tình cảm Đạt yêu cầu]{/color}"

# game/characters/alice/sleep.rpy:62
translate vietnamese sleep_alice_sleepover_df5048c5:

    # alice "It didn't look like you were thinking about sleeping just now.."
    alice "Có vẻ như anh đang không nghĩ về việc ngủ.."

# game/characters/alice/sleep.rpy:63
translate vietnamese sleep_alice_sleepover_8fe57fc8:

    # me "It just came to me."
    me "..."

# game/characters/alice/sleep.rpy:64
translate vietnamese sleep_alice_sleepover_6d562db5:

    # alice "Alright, I guess, I can spare you some space."
    alice "Được rồi, emm đoán là, tôi có thể dành cho anh một chút chỗ."

# game/characters/alice/sleep.rpy:67
translate vietnamese sleep_alice_sleepover_122bb5f7:

    # alice "Comfy?"
    alice "Thoải mái chứ?"

# game/characters/alice/sleep.rpy:68
translate vietnamese sleep_alice_sleepover_6fd65820:

    # me "Very."
    me "Rất thoải mái."

# game/characters/alice/sleep.rpy:71
translate vietnamese sleep_alice_sleepover_ecaae7e2:

    # alice "Night."
    alice "Ngủ ngon."

# game/characters/alice/sleep.rpy:72
translate vietnamese sleep_alice_sleepover_c72b062c:

    # me "Night."
    me "Ngủ ngon."

# game/characters/alice/sleep.rpy:75
translate vietnamese sleep_alice_sleepover_bebfed96:

    # "[alice] snuggles into you."
    "[alice] rúc vào bạn."

# game/characters/alice/sleep.rpy:85
translate vietnamese sleep_alice_sleepover_7c64d06f:

    # me "Good morning, sunshine."
    me "Chào buổi sáng."

# game/characters/alice/sleep.rpy:86
translate vietnamese sleep_alice_sleepover_888d33a4:

    # alice "Morning."
    alice "Chào."

# game/characters/alice/sleep.rpy:87
translate vietnamese sleep_alice_sleepover_312ba666:

    # me "How was yo.. wait.."
    me "Em thế nào .. đợi đã.."

# game/characters/alice/sleep.rpy:90
translate vietnamese sleep_alice_sleepover_ea2c91fb:

    # me "Why are you naked?"
    me "Tại sao em lại khỏa thân?"

# game/characters/alice/sleep.rpy:93
translate vietnamese sleep_alice_sleepover_e3a2f8cc:

    # alice "You don't like it?"
    alice "Bạn không thích à?"

# game/characters/alice/sleep.rpy:94
translate vietnamese sleep_alice_sleepover_463a3296:

    # me "I do, I'm surprised, that's all."
    me "Anh có, anh chỉ thấy ngạc nhiên, thế thôi."

# game/characters/alice/sleep.rpy:97
translate vietnamese sleep_alice_sleepover_799ded38:

    # alice "I wanted to make sure my handsome boy wakes up on the right foot."
    alice "Em muốn chắc chắn rằng cậu bé đẹp trai của em sẽ thức dậy bằng chân phải."

# game/characters/alice/sleep.rpy:98
translate vietnamese sleep_alice_sleepover_19c66564:

    # me "I like where it is goin'."
    me "Anh thích cách mà việc này sẽ đến'."

# game/characters/alice/sleep.rpy:99
translate vietnamese sleep_alice_sleepover_4d75666d:

    # alice "You surely do."
    alice "Anh sẽ thích thôi."

# game/characters/alice/sleep.rpy:108
translate vietnamese sleep_alice_sleepover_5c1bf8d4:

    # "{color=#FF958F}[[Relationship check failed]{/color}"
    "{color=#FF958F}[[Mức độ tình cảm đạt yêu cầu]{/color}"

# game/characters/alice/sleep.rpy:109
translate vietnamese sleep_alice_sleepover_619217b5:

    # alice "How about you sleep in your own bed?"
    alice "Anh ngủ trên giường của chính mình thì sao?"

# game/characters/alice/sleep.rpy:110
translate vietnamese sleep_alice_sleepover_efe24489:

    # me "I guess I don't have a choice, do I?"
    me "Anh đoán mình không có sự lựa chọn, phải không?"

# game/characters/alice/sleep.rpy:111
translate vietnamese sleep_alice_sleepover_a3dd2bc6:

    # alice "Not this time."
    alice "Không phải lúc này."

# game/characters/alice/sleep.rpy:112
translate vietnamese sleep_alice_sleepover_fdd6494c:

    # th "This time?"
    th "lúc này?"

# game/characters/alice/sleep.rpy:113
translate vietnamese sleep_alice_sleepover_d006677a:

    # alice "Are you going to hold my thigh for long?"
    alice "Anh định ôm đùi em bao lâu nữa?"

# game/characters/alice/sleep.rpy:118
translate vietnamese sleep_alice_hj_5dd074dc:

    # me "How did you undress me without waking me up?"
    me "Làm thế nào em cởi quần áo của anh mà không đánh thức anh?"

# game/characters/alice/sleep.rpy:121
translate vietnamese sleep_alice_hj_c2861e65:

    # alice "Secret~ Let me just tell, you could learn a lot from me about dealing with sleeping people."
    alice "Bí mật ~ Để em kể nhé, anh có thể học được nhiều điều từ em về cách đối phó với những người đang ngủ."

# game/characters/alice/sleep.rpy:122
translate vietnamese sleep_alice_hj_43ab313c:

    # me "This, I can see. I'll make sure to be a good student, professor."
    me "Điều này, anh có thể thấy. Anh sẽ đảm bảo là một học sinh giỏi, thưa giáo sư."

# game/characters/alice/sleep.rpy:123
translate vietnamese sleep_alice_hj_511a40af:

    # alice "Good boy, but first, let's deal with this big problem we have here."
    alice "Tốt lắm, nhưng trước tiên, hãy giải quyết vấn đề lớn mà chúng ta gặp phải ở đây."

# game/characters/alice/sleep.rpy:124
translate vietnamese sleep_alice_hj_69cf01ff:

    # me "Brilliant idea."
    me "Ý tưởng tuyệt vời."

# game/characters/alice/sleep.rpy:131
translate vietnamese sleep_alice_hj_3260d0a2:

    # "[alice] starts to move her palm over your dick while her tongue plays with your nipple, sending sparks of electricity down your spine."
    "[alice] bắt đầu di chuyển lòng bàn tay của cô ấy trên dương vật của bạn trong khi lưỡi cô ấy chơi với núm vú của bạn, gửi các tia điện xuống cột sống của bạn."

# game/characters/alice/sleep.rpy:132
translate vietnamese sleep_alice_hj_90fb30f1:

    # me "Ohh.. You're good at this."
    me "Ohh.. Em giỏi cái này đấy."

# game/characters/alice/sleep.rpy:133
translate vietnamese sleep_alice_hj_2de21fb9:

    # alice "Haha~"
    alice "Haha~"

# game/characters/alice/sleep.rpy:134
translate vietnamese sleep_alice_hj_b03afed4:

    # alice "Praise me more."
    alice "Khen em nhiều hơn đi."

# game/characters/alice/sleep.rpy:141
translate vietnamese sleep_alice_hj_426d91d6:

    # "As if wanting to give you a reason to praise her, she starts to lick your nipple in another gentler way."
    "Như thể muốn cho bạn lý do để khen ngợi cô ấy, cô ấy bắt đầu liếm núm vú của bạn theo một cách khác dịu dàng hơn."

# game/characters/alice/sleep.rpy:143
translate vietnamese sleep_alice_hj_e83e91e9:

    # "It gives you a sensual, almost spiritual, feeling of happiness and purity."
    "Nó mang lại cho bạn một cảm giác gợi cảm, gần như lên mây, hạnh phúc và tinh khiết."

# game/characters/alice/sleep.rpy:144
translate vietnamese sleep_alice_hj_166d9144:

    # me "Ahh!"
    me "Ahh!"

# game/characters/alice/sleep.rpy:145
translate vietnamese sleep_alice_hj_fd220f70:

    # alice "Hehe~ Where is my praise?"
    alice "Hehe~ Lời khen ngợi của em đâu?"

# game/characters/alice/sleep.rpy:146
translate vietnamese sleep_alice_hj_4e84449c:

    # me "Good girl! You're the best."
    me "Cô gái ngoan! Em là tuyệt nhất."

# game/characters/alice/sleep.rpy:147
translate vietnamese sleep_alice_hj_22033369:

    # alice "I know~"
    alice "I know~"

# game/characters/alice/sleep.rpy:154
translate vietnamese sleep_alice_hj_0f0ba74d:

    # "You just lie back, eyes closed, enjoying this extraterrestrial feeling."
    "Bạn chỉ cần nằm lại, nhắm mắt, tận hưởng cảm giác ngoài trái đất này."

# game/characters/alice/sleep.rpy:155
translate vietnamese sleep_alice_hj_bf045400:

    # me "Very.. good.. girl.."
    me "Very.. good.. girl.."

# game/characters/alice/sleep.rpy:156
translate vietnamese sleep_alice_hj_4040ff0d:

    # alice "Haha~ I love it when you praise me."
    alice "Haha~ Em thích nó khi anh khen ngợi em."

# game/characters/alice/sleep.rpy:163
translate vietnamese sleep_alice_hj_1735f07e:

    # me "I'm close, babe."
    me "Sắp rồi, babe."

# game/characters/alice/sleep.rpy:164
translate vietnamese sleep_alice_hj_5ee5ec93:

    # alice "Cum for me. But, please don't get any on my ceiling. I'm too short to clean there."
    alice "Ra cho em. Nhưng, xin đừng bắn bất kỳ thứ gì trên trần nhà của em. Em quá ngắn để làm sạch ở đó."

# game/characters/alice/sleep.rpy:165
translate vietnamese sleep_alice_hj_4a8ccba2:

    # me "Hahaha. Don't worry if I get any, I'll raise you to clean it."
    me "Hahaha. Đừng lo lắng nếu anh có bắn lên, tôi sẽ nuôi em để làm sạch nó."

# game/characters/alice/sleep.rpy:166
translate vietnamese sleep_alice_hj_43242706:

    # alice "Deal."
    alice "Chốt đơn."

# game/characters/alice/sleep.rpy:173
translate vietnamese sleep_alice_hj_ae2cb410:

    # me "Ahhmn!"
    me "Ahhmn!"

# game/characters/alice/sleep.rpy:176
translate vietnamese sleep_alice_hj_bfe648fc:

    # alice "It seems I only need to clean my bed, and the floor."
    alice "Có vẻ như em chỉ cần lau giường và sàn nhà."

# game/characters/alice/sleep.rpy:177
translate vietnamese sleep_alice_hj_1ef2dafc:

    # me "I'll try harder next time."
    me "Anh sẽ cố gắng hơn vào lần sau."

# game/characters/alice/sleep.rpy:180
translate vietnamese sleep_alice_hj_de080d43:

    # alice "Please, don't. I really don't want to clean the ceiling."
    alice "Làm ơn, đừng. Em thực sự không muốn làm sạch trần nhà."

# game/characters/alice/sleep.rpy:190
translate vietnamese sleep_alice_sleepover_end_97fe4c4a:

    # alice "Alright, I'm going to prepare for the day."
    alice "Được rồi, em sẽ chuẩn bị cho ngày hôm nay."

# game/characters/alice/sleep.rpy:191
translate vietnamese sleep_alice_sleepover_end_cdd7aae0:

    # me "Mm. See you."
    me "Mm. gặp em sau."

# game/characters/alice/sleep.rpy:192
translate vietnamese sleep_alice_sleepover_end_4b7e1b8b:

    # alice "Mhm. I hope we see each other really soon, in similar dress-code."
    alice "Mhm. Em hy vọng chúng ta sẽ sớm gặp lại nhau, trong trang phục giống nhau."

# game/characters/alice/sleep.rpy:193
translate vietnamese sleep_alice_sleepover_end_2a78d98e:

    # me "I'll take that invitation for sure."
    me "Anh chắc chắn sẽ nhận lời mời đó."

# game/characters/alice/sleep.rpy:194
translate vietnamese sleep_alice_sleepover_end_46d4138c:

    # alice "I don't doubt that. Don't make me wait too long."
    alice "Em không nghi ngờ điều đó đâu. Đừng bắt em phải đợi lâu."

translate vietnamese strings:

    # game/characters/alice/sleep.rpy:13
    old "Spread her legs"
    new "Banh chân ra"

    # game/characters/alice/sleep.rpy:13
    old "Leave"
    new "Rời đi"

    # game/characters/alice/sleep.rpy:41
    old "I want to sleep with you"
    new "Anh muốn ngủ vơi em"
