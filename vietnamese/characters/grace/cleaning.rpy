# TODO: Translation updated at 2021-12-24 07:09

# game/characters/grace/cleaning.rpy:18
translate vietnamese grace_cleaning_c36e51e3:

    # grace "Mm. Thank you."
    grace "Mm. Cảm ơn anh."

# game/characters/grace/cleaning.rpy:19
translate vietnamese grace_cleaning_b36321e0:

    # me "No need, we all live here. It's normal for me to want to help."
    me "Không cần đâu, tất cả chúng ta đều sống ở đây mà. Giúp đỡ nhau là chuyện nên làm thôi."

# game/characters/grace/cleaning.rpy:20
translate vietnamese grace_cleaning_d849132a:

    # grace "You make sense. Shall we?"
    grace "Cũng đúng. Vậy chúng ta vào việc chứ?"

# game/characters/grace/cleaning.rpy:21
translate vietnamese grace_cleaning_52187447:

    # me "Of course."
    me "Tất nhiên."

# game/characters/grace/cleaning.rpy:40
translate vietnamese grace_cleaning_f1a_a0eb8380:

    # me "Does anyone ever use the couch here?"
    me "Có ai sử dụng cái ghế ở đây không?"

# game/characters/grace/cleaning.rpy:41
translate vietnamese grace_cleaning_f1a_6851a82a:

    # grace "Why are you asking..?"
    grace "Tại sao anh lại hỏi vậy...?"

# game/characters/grace/cleaning.rpy:42
translate vietnamese grace_cleaning_f1a_a741086e:

    # me "I just thought that anyone would rather stay in their room, what is there to do sitting here?"
    me "Anh thấy thà nằm trong phòng của mình chứ ngồi đây làm gì?"

# game/characters/grace/cleaning.rpy:43
translate vietnamese grace_cleaning_f1a_92c29431:

    # grace "Well.. Alice told me that, when they had just gotten here, Lexi would."
    grace "Chà... Alice đã nói với em rằng, khi họ vừa đến đây, Lexi ngồi ở chỗ này đấy."

# game/characters/grace/cleaning.rpy:44
translate vietnamese grace_cleaning_f1a_933313e1:

    # me "Oh, really?"
    me "Oh, vậy à?"

# game/characters/grace/cleaning.rpy:48
translate vietnamese grace_cleaning_f1a_a0ba554f:

    # grace "Yes. To protect Mina. She was afraid some monster will suddenly appear inside."
    grace "Vâng. Để bảo vệ Mina. chị ấy sợ một con quái vật nào đó sẽ đột ngột xuất hiện bên trong."

# game/characters/grace/cleaning.rpy:51
translate vietnamese grace_cleaning_f1a_ffa0c70a:

    # th "I wonder if back then Mina was already attracted to Alice."
    th "Không biết hồi đó Mina có bị Alice làm cho say nắng chưa nhỉ."

# game/characters/grace/cleaning.rpy:52
translate vietnamese grace_cleaning_f1a_0902dc09:

    # th "Must've been hard to peek when your older [lex_min] doesn't want to leave you alone."
    th "Chắc hẳn rất khó để nhìn trộm khi người [lex_min] lớn tuổi của em nó không muốn để em nó một mình."

# game/characters/grace/cleaning.rpy:53
translate vietnamese grace_cleaning_f1a_8d86dd70:

    # th "Then again, maybe she also felt anxious, and was happy to have Lexi close."
    th "Hoặc có lẽ em ấy cũng cảm thấy lo lắng, và rất vui khi có Lexi ở bên."

# game/characters/grace/cleaning.rpy:55
translate vietnamese grace_cleaning_f1a_f3c1691e:

    # me "So, the others didn't use it at all?"
    me "Vậy, những người khác hoàn toàn không sử dụng nó sao?"

# game/characters/grace/cleaning.rpy:59
translate vietnamese grace_cleaning_f1a_9c4bd0c1:

    # grace "Not that I remember.."
    grace "Em không nhớ..."

# game/characters/grace/cleaning.rpy:60
translate vietnamese grace_cleaning_f1a_07264de6:

    # grace "Well, we sometimes do when waiting till toilet will be free."
    grace "Chà, đôi khi chúng ta vẫn ngồi đây đợi đến khi nhà vệ sinh được dùng xong."

# game/characters/grace/cleaning.rpy:61
translate vietnamese grace_cleaning_f1a_d6c4a2ab:

    # me "I guess it makes sense."
    me "Cũng đúng."

# game/characters/grace/cleaning.rpy:67
translate vietnamese grace_cleaning_f1a_0e0ad35e:

    # grace "[dog] loves to stay here, though."
    grace "[dog] cũng thích ngồi đây mà."

# game/characters/grace/cleaning.rpy:68
translate vietnamese grace_cleaning_f1a_e7eca864:

    # me "Haha, yeah. Seems like he likes his new home."
    me "Haha, yeah. Có vẻ như cậu bé này thích ngôi nhà mới của mình."

# game/characters/grace/cleaning.rpy:72
translate vietnamese grace_cleaning_f1a_328bc9fe:

    # grace "I don't know why he likes to stay here, though."
    grace "Em không biết tại sao cậu ta thích ngồi ở đây nữa."

# game/characters/grace/cleaning.rpy:73
translate vietnamese grace_cleaning_f1a_27cf07ff:

    # me "Maybe he admires how clean it looks after you're done with it?"
    me "Có thể nó ngưỡng mộ vẻ sạch sẽ của cái ghế sau khi em lau xong đấy?"

# game/characters/grace/cleaning.rpy:74
translate vietnamese grace_cleaning_f1a_3d77e88d:

    # grace "I don't really do much with it, though. The couch is much harder to clean because of the [dog]'s fur."
    grace "Tuy nhiên, em lại tốn thêm thời gian. Ghế khó làm sạch hơn nhiều vì lông của [dog]"

# game/characters/grace/cleaning.rpy:76
translate vietnamese grace_cleaning_f1a_7f2380ce:

    # me "So, couch aside, anything I should know about cleaning the corridor?"
    me "Vậy, hãy bỏ cái ghế sang một bên đi, có điều gì anh nên biết về việc dọn dẹp hành lang không?"

# game/characters/grace/cleaning.rpy:80
translate vietnamese grace_cleaning_f1a_476324da:

    # grace "Not really, no."
    grace "Cũng không hẳn."

# game/characters/grace/cleaning.rpy:81
translate vietnamese grace_cleaning_f1a_9577c2ee:

    # me "Well, that's fine. This corridor isn't too big, so at least we'll be done in no time."
    me "Thôi kệ đi. Hành lang này cũng không quá lớn đâu, chúng ta sẽ xong nhanh thôi."

# game/characters/grace/cleaning.rpy:85
translate vietnamese grace_cleaning_f1a_0f09b6c6:

    # "You two get back to the work, and indeed you finish in no time."
    "Hai bạn quay trở lại công việc và bạn thực sự hoàn thành ngay lập tức."

# game/characters/grace/cleaning.rpy:90
translate vietnamese grace_cleaning_f1a_83757b68:

    # grace "Mhm. Thank you for your help, [me]."
    grace "Mhm. Cám ơn vì đã giúp nha anh [me]."

# game/characters/grace/cleaning.rpy:98
translate vietnamese grace_cleaning_f1b_7dca4aef:

    # me "Grace, do you need anything to clean the bathroom?"
    me "Grace, em có cần gì để dọn phòng tắm không?"

# game/characters/grace/cleaning.rpy:102
translate vietnamese grace_cleaning_f1b_3b0ef405:

    # grace "What do you mean? We are just going to clean it as any room.."
    grace "Ý anh là gì? Chúng ta chỉ cần dọn dẹp nó như mấy phòng khác thôi mà..."

# game/characters/grace/cleaning.rpy:103
translate vietnamese grace_cleaning_f1b_7434dc03:

    # me "I'm talking about humidity. Do you have anything ready for that?"
    me "Anh đang nói về độ ẩm đấy. Em có gì sẵn sàng cho việc đó không?"

# game/characters/grace/cleaning.rpy:104
translate vietnamese grace_cleaning_f1b_8086234b:

    # grace "I don't get it.."
    grace "Em không hiểu..."

# game/characters/grace/cleaning.rpy:105
translate vietnamese grace_cleaning_f1b_b034b462:

    # me "Did your mom never tell you about that?"
    me "Có phải mẹ em chưa bao giờ dạy với em về điều này không?"

# game/characters/grace/cleaning.rpy:106
translate vietnamese grace_cleaning_f1b_de0277bc:

    # grace "She said it was humid in some days of summer, but nothing else."
    grace "Mẹ em nói rằng trời ẩm ướt trong một số ngày của mùa hè, và không có gì khác."

# game/characters/grace/cleaning.rpy:107
translate vietnamese grace_cleaning_f1b_e05f653f:

    # me "[grace], you know how you use hot water to shower? That hot water comes with steam, right?"
    me "[grace], em biết cách em sử dụng nước nóng để tắm chứ? Nước nóng kèm theo hơi nước, đúng không?"

# game/characters/grace/cleaning.rpy:111
translate vietnamese grace_cleaning_f1b_00be2ccd:

    # grace "Mm, it does."
    grace "Mm, Nó có."

# game/characters/grace/cleaning.rpy:112
translate vietnamese grace_cleaning_f1b_d989bc94:

    # me "That steam would elevate, get to the top of the room, and liquify."
    me "Hơi nước đó sẽ bốc lên cao, lên đến tận cùng căn phòng và hóa lỏng."

# game/characters/grace/cleaning.rpy:113
translate vietnamese grace_cleaning_f1b_7402cda2:

    # grace "Mm."
    grace "Mm."

# game/characters/grace/cleaning.rpy:114
translate vietnamese grace_cleaning_f1b_1e90d75e:

    # me "And if it doesn't dry properly.. It ends up forming mold. And that leads to fungus, [grace]."
    me "Và nếu nó không được làm khô đúng cách... Nó sẽ tạo thành nấm mốc. Và điều đó dẫn đến nấm đấy [grace]."

# game/characters/grace/cleaning.rpy:118
translate vietnamese grace_cleaning_f1b_7b71b640:

    # grace "Ohh.."
    grace "Ohh..."

# game/characters/grace/cleaning.rpy:119
translate vietnamese grace_cleaning_f1b_e22087af:

    # me "Yes. That's why we need to clean every nook and cranny when it comes to the bathroom."
    me "Đó là lý do tại sao chúng ta cần làm sạch mọi ngóc ngách khi đến phòng tắm."

# game/characters/grace/cleaning.rpy:120
translate vietnamese grace_cleaning_f1b_40c496c7:

    # grace "We could get sick.."
    grace "Chúng ta có thể bị ốm..."

# game/characters/grace/cleaning.rpy:121
translate vietnamese grace_cleaning_f1b_7396d372:

    # me "Exactly. Well, depending on the building, mold can form anywhere."
    me "Chính xác. Tùy thuộc vào tòa nhà, nấm mốc có thể hình thành ở bất kỳ đâu."

# game/characters/grace/cleaning.rpy:122
translate vietnamese grace_cleaning_f1b_d4700c52:

    # me "Cheap apartments with thin walls would end up accumulating humidity."
    me "Căn hộ giá rẻ với tường mỏng sẽ tích tụ độ ẩm."

# game/characters/grace/cleaning.rpy:123
translate vietnamese grace_cleaning_f1b_2c397b75:

    # me "Or, when water pipe breaks, it can flood a wall, floor or ceiling."
    me "Hoặc khi vỡ đường ống nước có thể làm ngập tường, sàn hoặc trần nhà."

# game/characters/grace/cleaning.rpy:124
translate vietnamese grace_cleaning_f1b_2859eb1e:

    # grace "And left alone, they end up ruined.."
    grace "Và bị bỏ đấy, rồi chúng sẽ hỏng..."

# game/characters/grace/cleaning.rpy:125
translate vietnamese grace_cleaning_f1b_c9ae341b:

    # grace "Alice said the building was well made, so we should be safe from most of that."
    grace "Alice nói rằng tòa nhà được làm rất tốt, nên chúng ta sẽ an toàn trước hầu hết những vấn đề như vậy."

# game/characters/grace/cleaning.rpy:126
translate vietnamese grace_cleaning_f1b_41b7ca5d:

    # me "Hygiene in home depends on the bathroom. Got it?"
    me "Vệ sinh trong nhà phụ thuộc vào phòng tắm. Hiểu chưa?"

# game/characters/grace/cleaning.rpy:130
translate vietnamese grace_cleaning_f1b_0bd4e3bd:

    # grace "I will clean every nook and cranny!"
    grace "Em sẽ dọn sạch mọi ngóc ngách!"

# game/characters/grace/cleaning.rpy:131
translate vietnamese grace_cleaning_f1b_b0caf5ea:

    # th "She seems happy to take care of it."
    th "Em ấy có vẻ hạnh phúc khi lau dọn."

# game/characters/grace/cleaning.rpy:132
translate vietnamese grace_cleaning_f1b_2db85240:

    # me "You did great today [grace]."
    me "Hôm nay em làm tốt lắm [grace]."

# game/characters/grace/cleaning.rpy:136
translate vietnamese grace_cleaning_f1b_baf8f4db:

    # grace "Hehe~"
    grace "Hehe~"

# game/characters/grace/cleaning.rpy:145
translate vietnamese grace_cleaning_f2a_b01c6394:

    # me "I don't think I've seen you cleaning the girls' rooms before, [grace]."
    me "Anh thấy em chưa bao giờ dọn phòng của các chị bao giờ cả."

# game/characters/grace/cleaning.rpy:146
translate vietnamese grace_cleaning_f2a_812d5cbf:

    # grace "I don't. Unless they ask me to."
    grace "Em không dọn. Trừ khi bọn họ nhờ em ."

# game/characters/grace/cleaning.rpy:147
translate vietnamese grace_cleaning_f2a_8406b83f:

    # me "Why's that? Sounds like you are willing to do it."
    me "Tại sao? Nghe có vẻ như em sẵn sàng làm điều đó vậy."

# game/characters/grace/cleaning.rpy:151
translate vietnamese grace_cleaning_f2a_a12de227:

    # grace "I am. But mommy didn't like me cleaning her room, so I don't clean others' rooms."
    grace "Em có dọn. Nhưng mẹ em không thích em dọn phòng của mẹ, nên em cũng không dọn phòng của người khác."

# game/characters/grace/cleaning.rpy:152
translate vietnamese grace_cleaning_f2a_2f6560ef:

    # me "So, did your mom clean your room?"
    me "Vậy mẹ có dọn phòng cho em không?"

# game/characters/grace/cleaning.rpy:153
translate vietnamese grace_cleaning_f2a_26ffc236:

    # grace "She insisted to do it. But I kept it clean, so she didn't really have much to do."
    grace "Bà ấy cứ khăng khăng làm điều đó. Nhưng em đã giữ nó sạch sẽ, vì vậy mẹ không có nhiều việc phải làm."

# game/characters/grace/cleaning.rpy:157
translate vietnamese grace_cleaning_f2a_a586bf6f:

    # grace "I was cleaning the rest of the house instead."
    grace "Thay vào đó em dọn dẹp phần còn lại của ngôi nhà."

# game/characters/grace/cleaning.rpy:158
translate vietnamese grace_cleaning_f2a_387b8342:

    # grace "What about {i}your{/i} room? Do you need cleaning?"
    grace "Còn phòng {i}của anh{/i} thì sao? Anh có cần dọn dẹp không?"

# game/characters/grace/cleaning.rpy:159
translate vietnamese grace_cleaning_f2a_7d85d3e8:

    # me "It's fine. You can clean it sometimes, though."
    me "Không cần đâu. Nhưng thi thoảng vẫn cần dọn dẹp chút."

# game/characters/grace/cleaning.rpy:160
translate vietnamese grace_cleaning_f2a_96f7f399:

    # grace "Mm. I was doing it already when you were outside."
    grace "Ừm. Em dọn phòng của anh vào những lúc anh ở bên ngoài đấy."

# game/characters/grace/cleaning.rpy:161
translate vietnamese grace_cleaning_f2a_fb0139dd:

    # me "Eh? You were?"
    me "Hể? Em dọn á?"

# game/characters/grace/cleaning.rpy:162
translate vietnamese grace_cleaning_f2a_3e0a0ca0:

    # grace "Mm. Why do you think it's always clean?"
    grace "Mm. Thế anh nghĩ tại sao nó luôn sạch sẽ?"

# game/characters/grace/cleaning.rpy:163
translate vietnamese grace_cleaning_f2a_aa23db04:

    # me "I never thought of it.."
    me "Anh chưa bao giờ nghĩ về việc đó cả..."

# game/characters/grace/cleaning.rpy:164
translate vietnamese grace_cleaning_f2a_213ca5d5:

    # me "Anyway, thank you for not making me live in a mess."
    me "Dù sao cũng cảm ơn em vì đã giúp anh không phải sống trong một mớ hỗn độn nha."

# game/characters/grace/cleaning.rpy:165
translate vietnamese grace_cleaning_f2a_7402cda2:

    # grace "Mm."
    grace "Ừm."

# game/characters/grace/cleaning.rpy:166
translate vietnamese grace_cleaning_f2a_632ef231:

    # me "But what about not cleaning other rooms?"
    me "Nhưng không dọn phòng khác là sao?"

# game/characters/grace/cleaning.rpy:170
translate vietnamese grace_cleaning_f2a_70d428b4:

    # grace "You're a man, I don't clean girls' rooms."
    grace "Anh là đàn ông, em không dọn phòng cho con gái."

# game/characters/grace/cleaning.rpy:171
translate vietnamese grace_cleaning_f2a_3fdb2b7e:

    # th "Yeah.. Could've expected this."
    th "Yeah... Mình cũng đã lường trước điều này rồi."

# game/characters/grace/cleaning.rpy:174
translate vietnamese grace_cleaning_f2a_ef78831e:

    # th "Let's just hope she doesn't find [alice] in my bed..'"
    th "Hãy mong là em ấy không tìm thấy [alice] trên giường của mình đi..."

# game/characters/grace/cleaning.rpy:178
translate vietnamese grace_cleaning_f2a_d800aba8:

    # me "Seems we're done."
    me "Có vẻ như chúng ta đã xong rồi đấy."

# game/characters/grace/cleaning.rpy:179
translate vietnamese grace_cleaning_f2a_7402cda2_1:

    # grace "Mm."
    grace "Mm."

# game/characters/grace/cleaning.rpy:184
translate vietnamese grace_cleaning_f2a_1229877d:

    # grace "Thank you for the help, [me]."
    grace "Cảm ơn anh vì sự giúp đỡ."

# game/characters/grace/cleaning.rpy:185
translate vietnamese grace_cleaning_f2a_cdbf13a2:

    # me "Sure, [grace]. Always happy to help."
    me "Chắc chắn rồi, [grace]. Anh luôn sẵn lòng giúp đỡ mà."

# game/characters/grace/cleaning.rpy:196
translate vietnamese grace_cleaning_f2b_89b31c6b:

    # me "Do you help [mina] with her boxes?"
    me "Em có giúp [mina] với đống hộp của cô ấy không?"

# game/characters/grace/cleaning.rpy:200
translate vietnamese grace_cleaning_f2b_fe1b3e6a:

    # grace "We all have supplies in there. I don't really touch them, though."
    grace "Tất cả chúng ta đều có nguồn cung cấp trong đó. Mặc dù em cũng không hay động vào chúng mấy."

# game/characters/grace/cleaning.rpy:201
translate vietnamese grace_cleaning_f2b_1bd8b8c6:

    # grace "It's her job. When I need anything I'm telling her this and she brings me the things I need."
    grace "Đó là công việc của chị ấy. Khi em cần bất cứ thứ gì, em sẽ nói với chị ấy và chị sẽ mang đến cho em những thứ em cần."

# game/characters/grace/cleaning.rpy:202
translate vietnamese grace_cleaning_f2b_aa3a1011:

    # me "Would you help her if she asked?"
    me "Em có giúp cô ấy nếu cô ấy hỏi không?"

# game/characters/grace/cleaning.rpy:206
translate vietnamese grace_cleaning_f2b_b5bdc377:

    # grace "Mm. We all need to help each other."
    grace "Mm. Tất cả chúng ta cần giúp đỡ lẫn nhau."

# game/characters/grace/cleaning.rpy:207
translate vietnamese grace_cleaning_f2b_9f49322f:

    # me "Seems mom taught you well in that regard."
    me "Có vẻ như mẹ đã dạy em rất tốt về mặt đó."

# game/characters/grace/cleaning.rpy:208
translate vietnamese grace_cleaning_f2b_482cdf25:

    # grace "Mm. Mommy is the best."
    grace "Mm. Mommy là tuyệt nhất."

# game/characters/grace/cleaning.rpy:209
translate vietnamese grace_cleaning_f2b_b4ccf1ba:

    # grace "I clean and cook, while Mina handles the logistics. It's our segregation of duties."
    grace "Em dọn dẹp và nấu ăn, trong khi Mina lo việc hậu cần. Đó là sự phân biệt giữa nhiệm vụ của chúng em."

# game/characters/grace/cleaning.rpy:210
translate vietnamese grace_cleaning_f2b_02442429:

    # grace "But that doesn't mean I can't help her with her duties if she needs me."
    grace "Nhưng điều đó không có nghĩa là em không thể giúp chị ấy thực hiện nhiệm vụ của mình nếu chị ấy cần em."

# game/characters/grace/cleaning.rpy:211
translate vietnamese grace_cleaning_f2b_7a80dd2b:

    # me "That's the right mindset to have. Well done, [grace]."
    me "Đó là tư duy đúng đắn cần có đấy. Làm tốt lắm [grace]."

# game/characters/grace/cleaning.rpy:215
translate vietnamese grace_cleaning_f2b_baf8f4db:

    # grace "Hehe~"
    grace "Hehe~"

# game/characters/grace/cleaning.rpy:216
translate vietnamese grace_cleaning_f2b_b72c1265:

    # me "What about [alice]? Do you help with her work?"
    me "Còn [alice] thì sao? Em có giúp đỡ công việc của em ấy không?"

# game/characters/grace/cleaning.rpy:220
translate vietnamese grace_cleaning_f2b_eb2b09f6:

    # grace "I do. But only simple things, like when she needs someone to hold something. I can't really help her in complex things."
    grace "Em có. Nhưng chỉ những điều đơn giản thôi, như  là khi chị ấy cần người để giữ một thứ gì đó. Em thực sự không thể giúp chị ấy trong những việc phức tạp."

# game/characters/grace/cleaning.rpy:221
translate vietnamese grace_cleaning_f2b_6918e6fa:

    # grace "I don't really know how to use those tools. Mommy didn't have those."
    grace "Em thực sự không biết cách sử dụng những công cụ đó. Mẹ không có những thứ đó."

# game/characters/grace/cleaning.rpy:222
translate vietnamese grace_cleaning_f2b_b3d8e26e:

    # me "I assumed you could sew."
    me "Anh thấy em biết biết may vá mà."

# game/characters/grace/cleaning.rpy:226
translate vietnamese grace_cleaning_f2b_f7370ab7:

    # grace "Why's that?"
    grace "Sao anh nghĩ vậy?"

# game/characters/grace/cleaning.rpy:227
translate vietnamese grace_cleaning_f2b_de102edb:

    # me "Seems like a useful skill you could learn at home, and so your mother would've taught you."
    me "Đó là một kỹ năng hữu ích mà em có thể học ở nhà, vì vậy nên mẹ sẽ dạy em."

# game/characters/grace/cleaning.rpy:231
translate vietnamese grace_cleaning_f2b_3850fce9:

    # grace "She never did. That's a pity, it's indeed useful skill."
    grace "Mẹ chưa bao giờ làm vậy. Thật đáng tiếc, đó thực sự một là kỹ năng rất hữu ích."

# game/characters/grace/cleaning.rpy:232
translate vietnamese grace_cleaning_f2b_2f0f598c:

    # me "Maybe Alice would be willing to teach you."
    me "Có lẽ Alice sẽ sẵn lòng dạy em đấy."

# game/characters/grace/cleaning.rpy:236
translate vietnamese grace_cleaning_f2b_94c82021:

    # grace "Mm. I will ask her."
    grace "Mm. Em sẽ hỏi chị ấy."

# game/characters/grace/cleaning.rpy:240
translate vietnamese grace_cleaning_f2b_e5ee0686:

    # grace "All clean."
    grace "Mọi thứ đều sạch sẽ."

# game/characters/grace/cleaning.rpy:245
translate vietnamese grace_cleaning_f2b_cc2b23f6:

    # grace "Thank you for your help."
    grace "Cảm ơn sự giúp đỡ của anh."

# game/characters/grace/cleaning.rpy:246
translate vietnamese grace_cleaning_f2b_ecf54156:

    # me "It's the least I could do."
    me "Đó là điều tối thiểu nhất anh có thể làm được."

# game/characters/grace/cleaning.rpy:255
translate vietnamese grace_cleaning_lobbya_5fe740e5:

    # me "I had the impression before, but the lobby is pretty spacious."
    me "Anh đã có ấn tượng trước đây rồi, nhưng sảnh khá là rộng rãi đấy."

# game/characters/grace/cleaning.rpy:256
translate vietnamese grace_cleaning_lobbya_a9d71b40:

    # grace "Yes, it takes quite the time to clean."
    grace "Vâng, cũng tốn kha khá thời gian để làm sạch đấy."

# game/characters/grace/cleaning.rpy:257
translate vietnamese grace_cleaning_lobbya_d46474ef:

    # me "Well, it is an hotel after all. I guess it was bound to be big."
    me "mà, dù sao thì nó cũng là một cái khách sạn mà. Nên nó nhất định phải lớn."

# game/characters/grace/cleaning.rpy:258
translate vietnamese grace_cleaning_lobbya_a0799d84:

    # me "I bet it got dirtier when it was open. More people coming and going."
    me "Anh cá là nó sẽ bẩn hơn khi được mở cửa đấy. Nhiều người đến và đi."

# game/characters/grace/cleaning.rpy:262
translate vietnamese grace_cleaning_lobbya_92414fc5:

    # grace "I don't think so."
    grace "Em không nghĩ vậy."

# game/characters/grace/cleaning.rpy:263
translate vietnamese grace_cleaning_lobbya_07b79967:

    # me "Hmm? Why?"
    me "Hmm? Tại sao?"

# game/characters/grace/cleaning.rpy:264
translate vietnamese grace_cleaning_lobbya_48862eef:

    # grace "You all are venturing out often, bringing a lot of dirt. It's very messy outside.."
    grace "Tất cả mọi người thường xuyên ra ngoài, mang theo rất nhiều bụi bẩn. VÌ bên ngoài rất lộn xộn..."

# game/characters/grace/cleaning.rpy:265
translate vietnamese grace_cleaning_lobbya_84cfb7d2:

    # me "Oh.. Never thought of that. Sorry about dirtying the floor."
    me "Oh... Anh còn không để tâm đến luôn đấy. Xin lỗi vì làm bẩn sàn nhà."

# game/characters/grace/cleaning.rpy:266
translate vietnamese grace_cleaning_lobbya_0c2ad000:

    # grace "There's nothing you can do about it. We all have things to do. Cleaning is my part."
    grace "Trường hợp bất khả kháng thôi. Tất cả chúng ta đều có những việc phải làm. Dọn dẹp là phần của em."

# game/characters/grace/cleaning.rpy:267
translate vietnamese grace_cleaning_lobbya_a01aace8:

    # me "Glad you understand."
    me "Em hiểu là tốt rồi."

# game/characters/grace/cleaning.rpy:271
translate vietnamese grace_cleaning_lobbya_9768d522:

    # grace "But how do you manage to get the dirtiest of everyone? You even put on that weird coat going out."
    grace "Nhưng tại sao anh lại là người bẩn nhất vậy? Anh thậm chí còn mặc cái áo khoác kỳ lạ đó khi đi ra ngoài nữa."

# game/characters/grace/cleaning.rpy:272
translate vietnamese grace_cleaning_lobbya_314086b3:

    # me "What?"
    me "Hả?"

# game/characters/grace/cleaning.rpy:273
translate vietnamese grace_cleaning_lobbya_b47a4855:

    # grace "Your shoes are super dirty every time. I can always tell when you were out there."
    grace "Giày của Anh luôn siêu siêu bẩn. Em luôn có thể biết khi nào anh ở ngoài đó."

# game/characters/grace/cleaning.rpy:274
translate vietnamese grace_cleaning_lobbya_476cc045:

    # me "How do you know it's me? It could've been the girls."
    me "Làm sao em biết đó là anh? Đó cũng có thể là các cô gái khác mà."

# game/characters/grace/cleaning.rpy:275
translate vietnamese grace_cleaning_lobbya_77486076:

    # grace "The marks on the floor don't look like girls' shoe's sole."
    grace "Dấu vết trên sàn nhà trông không giống đế giày của con gái."

# game/characters/grace/cleaning.rpy:276
translate vietnamese grace_cleaning_lobbya_f392e810:

    # me "Ok, then it may be me.. But what does it have to do with my coat?"
    me "Ok, thì nó có thể là anh... Nhưng nó liên quan gì đến áo khoác của anh?"

# game/characters/grace/cleaning.rpy:277
translate vietnamese grace_cleaning_lobbya_a0894ee3:

    # grace "It's weird. I feel disturbed looking at it."
    grace "Nó rất là kỳ lạ. Em cảm thấy băn khoăn khi nhìn vào nó."

# game/characters/grace/cleaning.rpy:278
translate vietnamese grace_cleaning_lobbya_3820771a:

    # me "..."
    me "..."

# game/characters/grace/cleaning.rpy:279
translate vietnamese grace_cleaning_lobbya_6ac290d3:

    # grace "So, how do you manage to get so dirty?"
    grace "Vậy, làm sao mà anh có thể bẩn như thế được vậy?"

# game/characters/grace/cleaning.rpy:280
translate vietnamese grace_cleaning_lobbya_01f213fb:

    # me "Well, with my weird coat, I end up in weird places."
    me "Chà, với chiếc áo khoác kỳ lạ của anh, anh đã đến những nơi kỳ lạ."

# game/characters/grace/cleaning.rpy:281
translate vietnamese grace_cleaning_lobbya_0fe0d384:

    # grace "Is that because of your coat?"
    grace "Đó có phải là do áo khoác của anh không??"

# game/characters/grace/cleaning.rpy:282
translate vietnamese grace_cleaning_lobbya_d2882e3f:

    # th "I feel surreal right now.. Am I going crazy or is it [grace]?"
    th "Mình thấy lú quá... Mình đang phát điên hay là tại [grace]?"

# game/characters/grace/cleaning.rpy:283
translate vietnamese grace_cleaning_lobbya_3147d1e9:

    # me "The coat is fine, [grace].. It's just a normal clothing.. Thank you for worrying."
    me "Cái áo khoác ổn mà [grace]... Đó chỉ là một cái trang phục bình thường thôi... Cảm ơn em đã lo lắng."

# game/characters/grace/cleaning.rpy:284
translate vietnamese grace_cleaning_lobbya_604dd0e3:

    # th "I don't remember from where I've got it, though.."
    th "Nhưng, mình không nhớ là nó được lấy ở đâu..."

# game/characters/grace/cleaning.rpy:289
translate vietnamese grace_cleaning_lobbya_a4856c27:

    # grace "You're welcome. Thanks for helping me clean."
    grace "Không có gì. Cảm ơn vì đã giúp em dọn dẹp."

# game/characters/grace/cleaning.rpy:290
translate vietnamese grace_cleaning_lobbya_c8b89d77:

    # me "You're welcome as well."
    me "Không cần khách sáo."

# game/characters/grace/cleaning.rpy:300
translate vietnamese grace_cleaning_lobbyb_7e4b9942:

    # me "You know how we talked about how big this place is?"
    me "Em biết chúng ta đã nói về nơi này rộng lớn như thế nào không?"

# game/characters/grace/cleaning.rpy:301
translate vietnamese grace_cleaning_lobbyb_f9cc1b67:

    # grace "Yes, it's really big."
    grace "Vâng, nó thực sự rất lớn."

# game/characters/grace/cleaning.rpy:303
translate vietnamese grace_cleaning_lobbyb_3334675e:

    # me "You know what would be great? A projector."
    me "Em biết điều gì sẽ là tuyệt vời không? Một cái máy chiếu."

# game/characters/grace/cleaning.rpy:307
translate vietnamese grace_cleaning_lobbyb_3d5b8937:

    # grace "Like in the cinema?"
    grace "Giống như trong rạp chiếu phim?"

# game/characters/grace/cleaning.rpy:308
translate vietnamese grace_cleaning_lobbyb_692f7054:

    # me "Oh, so you knew about them."
    me "Oh, vậy em đã biết về chúng."

# game/characters/grace/cleaning.rpy:312
translate vietnamese grace_cleaning_lobbyb_7003e426:

    # grace "Yes, I just never saw one."
    grace "Vâng, nhưng Em chưa bao giờ nhìn thấy một cái."

# game/characters/grace/cleaning.rpy:313
translate vietnamese grace_cleaning_lobbyb_4d5353d2:

    # me "Of course. So, with all this space, we could hang on a screen."
    me "Chõ này rộng mà, chúng ta có thể treo trên một màn hình."

# game/characters/grace/cleaning.rpy:317
translate vietnamese grace_cleaning_lobbyb_8b964618:

    # grace "A green screen?"
    grace "Một màn hình xanh?"

# game/characters/grace/cleaning.rpy:318
translate vietnamese grace_cleaning_lobbyb_c9495906:

    # me "No, that's what people use for special effects. I'm talking about a white one, where the projection would play."
    me "Không, đó là cái người ta dùng cho các hiệu ứng đặc biệt. Anh đang nói về cái màu trắng cơ, nơi hình chiếu sẽ được phát."

# game/characters/grace/cleaning.rpy:322
translate vietnamese grace_cleaning_lobbyb_54b3a97c:

    # grace "I don't get it."
    grace "Em không hiểu."

# game/characters/grace/cleaning.rpy:323
translate vietnamese grace_cleaning_lobbyb_a170fca6:

    # me "Imagine this wall being a TV screen? Easily bigger than Alice's."
    me "Hãy tưởng tượng bức tường này là một màn hình TV? Lớn hơn của Alice."

# game/characters/grace/cleaning.rpy:324
translate vietnamese grace_cleaning_lobbyb_6bd81f9f:

    # grace "I guess.. But can't we just watch movies in her room?"
    grace "... Nhưng chúng ta chỉ cần xem phim trong phòng của chị ấy là được mà?"

# game/characters/grace/cleaning.rpy:325
translate vietnamese grace_cleaning_lobbyb_e71cecd8:

    # me "It's about being able to have your own cinema at home."
    me "Đó là về việc em có thể sở hữu rạp chiếu phim của riêng mình tại nhà."

# game/characters/grace/cleaning.rpy:326
translate vietnamese grace_cleaning_lobbyb_3d04923f:

    # me "Have you ever wanted to go to the movies?"
    me "Em đã bao giờ muốn đi xem phim chưa?"

# game/characters/grace/cleaning.rpy:327
translate vietnamese grace_cleaning_lobbyb_312a2ce0:

    # grace "Well, I guess it wouldn't be bad.. I haven't really prepared popcorn before."
    grace "Vậy chắc cũng tốt...      ."

# game/characters/grace/cleaning.rpy:328
translate vietnamese grace_cleaning_lobbyb_8706f07c:

    # me "Ah, we could actually get that.. I doubt it's high in the list of priorities for scavengers."
    me "Ah, chúng ta thực sự có thể đạt được điều đó... Anh nghi ngờ nó nằm trong danh sách ưu tiên của những người nhặt đồ."

# game/characters/grace/cleaning.rpy:332
translate vietnamese grace_cleaning_lobbyb_1d264bd0:

    # grace "Mm, I don't think things like projectors are in demand."
    grace "Mm, em không nghĩ những thứ như máy chiếu là nhu cầu đâu."

# game/characters/grace/cleaning.rpy:333
translate vietnamese grace_cleaning_lobbyb_ff340cad:

    # me "Either way, I think we could make it happen..."
    me "Dù bằng cách nào đi nữa anh nghĩ chúng ta có thể làm cho nó xảy ra..."

# game/characters/grace/cleaning.rpy:334
translate vietnamese grace_cleaning_lobbyb_88fd581f:

    # me "Worst case, we will just keep watching movies in Alice's room. Could be worse."
    me "Trường hợp tệ nhất, chúng ta sẽ phải tiếp tục xem phim trong phòng của Alice. Có thể tệ hơn."

# game/characters/grace/cleaning.rpy:335
translate vietnamese grace_cleaning_lobbyb_7402cda2:

    # grace "Mm."
    grace "Mm."

# game/characters/grace/cleaning.rpy:339
translate vietnamese grace_cleaning_lobbyb_35138234:

    # grace "But wouldn't it be dangerous to watch movies in the night..? I don't want to be in the lobby at that time."
    grace "Nhưng có nguy hiểm không nếu xem phim trong đêm..? Em không muốn ở sảnh vào lúc đó."

# game/characters/grace/cleaning.rpy:340
translate vietnamese grace_cleaning_lobbyb_83087c50:

    # me "Why?"
    me "Tại sao?"

# game/characters/grace/cleaning.rpy:341
translate vietnamese grace_cleaning_lobbyb_2538d440:

    # grace "Once when I get to the toilet at night, I've seen scary face pressed against the glass in the outside doors. It disappeared after a while, but I don't want to see it again."
    grace "Một lần khi em đi vệ sinh vào ban đêm, em nhìn thấy khuôn mặt đáng sợ áp vào kính ở cửa ngoài. Nó biến mất sau một lúc, nhưng em không muốn nhìn thấy nó lần nữa."

# game/characters/grace/cleaning.rpy:342
translate vietnamese grace_cleaning_lobbyb_b8ff1c98:

    # me "Shit, that sounds scary."
    me "Nghe có vẻ đáng sợ."

# game/characters/grace/cleaning.rpy:343
translate vietnamese grace_cleaning_lobbyb_2f0b2531:

    # me "We don't have to watch them at night, then."
    me "Vậy thì chúng ta không xem vào ban đêm nữa."

# game/characters/grace/cleaning.rpy:344
translate vietnamese grace_cleaning_lobbyb_fdb4e830:

    # grace "..."
    grace "..."

# game/characters/grace/cleaning.rpy:345
translate vietnamese grace_cleaning_lobbyb_2aedea7a:

    # me "*Sigh* Or we can just build it as a separate room.."
    me "*Thở dài* Hoặc chúng ta có thể xây nó như một căn phòng riêng biệt.."

# game/characters/grace/cleaning.rpy:349
translate vietnamese grace_cleaning_lobbyb_1cd940d9:

    # "Bad memories mad [grace] not wanting to talk anymore, so you two clean in silence."
    "Ký ức tồi tệ [grace] không muốn nói chuyện nữa nên hai người im lặng."

# game/characters/grace/cleaning.rpy:354
translate vietnamese grace_cleaning_lobbyb_45e6ef64:

    # grace "Thanks for the help, [me]."
    grace "Cảm ơn vì đã giúp đỡ."

# game/characters/grace/cleaning.rpy:355
translate vietnamese grace_cleaning_lobbyb_0377f1ec:

    # me "My pleasure, [grace]."
    me "Rất hân hạn."
# TODO: Translation updated at 2022-02-06 08:10

# game/characters/grace/cleaning.rpy:48
translate vietnamese grace_cleaning_f1a_5075838f:

    # grace "Yes, to protect Mina. She was afraid some monster will suddenly appear inside."
    grace "Vâng, để bảo vệ Mina. Chị ấy sợ một con quái vật nào đó sẽ đột ngột xuất hiện bên trong."

# game/characters/grace/cleaning.rpy:52
translate vietnamese grace_cleaning_f1a_381d854d:

    # th "Should have been hard to peek when your older [lex_min] doesn't want to leave you alone."
    th "Có lẽ rất khó để nhìn trộm khi người [lex_min] lớn tuổi của em ấy không muốn để em ấy một mình."

# game/characters/grace/cleaning.rpy:53
translate vietnamese grace_cleaning_f1a_052de262:

    # th "Then again, maybe she also felt anxious and was happy to have Lexi close."
    th "Nhưng có lẽ em ấy cũng vừa cảm thấy lo lắng và hạnh phúc khi có Lexi ở bên."

# game/characters/grace/cleaning.rpy:85
translate vietnamese grace_cleaning_f1a_1ba53d41:

    # "You two get back to work. Indeed, you finish in no time."
    "Hai người trở lại làm việc. Thật vậy, bạn hoàn thành ngay lập tức."

# game/characters/grace/cleaning.rpy:102
translate vietnamese grace_cleaning_f1b_3cc847ff:

    # grace "What do you mean? We are just going to clean it like any room.."
    grace "Ý anh là gì? Chúng ta chỉ cần dọn nó như những phòng khác thôi mà.."

# game/characters/grace/cleaning.rpy:123
translate vietnamese grace_cleaning_f1b_d8374d01:

    # me "Or, when a water pipe breaks, it can flood a wall, floor, or ceiling."
    me "Hoặc khi ống nước bị vỡ có thể làm ngập tường, sàn hoặc trần nhà."

# game/characters/grace/cleaning.rpy:126
translate vietnamese grace_cleaning_f1b_11d98465:

    # me "Hygiene in the home depends on the bathroom. Got it?"
    me "Vệ sinh trong nhà phụ thuộc vào phòng tắm. Hiểu chứ?"

# game/characters/grace/cleaning.rpy:147
translate vietnamese grace_cleaning_f2a_ca9984dd:

    # me "Why is that? Sounds like you are willing to do it."
    me "Tại sao? Nghe như em sẵn sàng làm điều đó vậy."

# game/characters/grace/cleaning.rpy:153
translate vietnamese grace_cleaning_f2a_f166c519:

    # grace "She insisted on it. But I kept it clean, so she didn't really have much to do."
    grace "Mẹ em khăng khăng làm việc đó. Nhưng em đã giữ nó sạch sẽ, vì vậy mẹ thực sự không có nhiều việc phải làm."

# game/characters/grace/cleaning.rpy:171
translate vietnamese grace_cleaning_f2a_277164f5:

    # th "Yeah.. Could've expected that."
    th "Mình cũng đoán được phần nào rồi."

# game/characters/grace/cleaning.rpy:201
translate vietnamese grace_cleaning_f2b_3b46319e:

    # grace "It's her job. When I need anything, I'm telling her this, and she brings me the things I need."
    grace "Đó là công việc của chị ấy. Khi em cần bất cái gì, em sẽ nói với chị ấy, và chị ấy sẽ mang đến cho tôi những thứ em cần."

# game/characters/grace/cleaning.rpy:221
translate vietnamese grace_cleaning_f2b_e769a53b:

    # grace "I don't know how to use those tools. Mommy didn't have those."
    grace "Em không biết cách sử dụng những công cụ đó. Mẹ không có những thứ đó."

# game/characters/grace/cleaning.rpy:231
translate vietnamese grace_cleaning_f2b_0e38ff1b:

    # grace "She never did. That's a pity, it's indeed an useful skill."
    grace "Mẹ chưa bao giờ làm vậy cả. Thật đáng tiếc, nó thực sự là một kỹ năng khá hữu ích."

# game/characters/grace/cleaning.rpy:257
translate vietnamese grace_cleaning_lobbya_4e49d942:

    # me "Well, it is a hotel after all. I guess it was bound to be big."
    me "Chà, dù gì thì đó cũng là một khách sạn mà. Lớn cũng là chuyện đương nhiên rồi."

# game/characters/grace/cleaning.rpy:282
translate vietnamese grace_cleaning_lobbya_b0bacab3:

    # th "I feel surreal right now.. Am I going crazy, or is it [grace]?"
    th "Mình cảm thấy kỳ quái quá.. mình đang phát điên, hay là [grace]?"

# game/characters/grace/cleaning.rpy:283
translate vietnamese grace_cleaning_lobbya_8bc4c501:

    # me "The coat is fine, [grace].. It's just normal clothing.. Thank you for worrying."
    me "Cái áo khoác ổn mà, [grace].. nó chỉ là một món đồ bình thường thôi.. cảm ơn em vì đã lo lắng."

# game/characters/grace/cleaning.rpy:301
translate vietnamese grace_cleaning_lobbyb_022dd3cd:

    # grace "Yes, it's massive."
    grace "Vâng, nó rất lớn."

# game/characters/grace/cleaning.rpy:328
translate vietnamese grace_cleaning_lobbyb_f79d451f:

    # me "Ah, we could get that.. I doubt it's high in the list of priorities for scavengers."
    me "Ah, chúng ta có thể lấy được đấy.. anh nghi là nó nằm trong danh sách ưu tiên của những người kiếm đồ."

# game/characters/grace/cleaning.rpy:342
translate vietnamese grace_cleaning_lobbyb_5311cea2:

    # me "Shit. That sounds scary."
    me "Nghe kinh vậy."
