# : Translation updated at 2021-12-24 07:09

# game/characters/player/ev_10.rpy:4
translate vietnamese main_10_c0147db6:

    # "You appear in the library."
    "Bạn xuất hiện trong thư viện."

# game/characters/player/ev_10.rpy:7
translate vietnamese main_10_f93a653c:

    # kiara "Aest.."
    kiara "Aest..."

# game/characters/player/ev_10.rpy:8
translate vietnamese main_10_811d26b4:

    # th "What?"
    th "Cái gì?"

# game/characters/player/ev_10.rpy:11
translate vietnamese main_10_935c38df:

    # "Before she managed to finish speaking, everything turns hazy and dark."
    "Trước khi cô ấy kịp nói xong, mọi thứ trở nên mơ hồ và tối lại."

# game/characters/player/ev_10.rpy:12
translate vietnamese main_10_0bdebd06:

    # kiara "..har!!"
    kiara "...har!!"

# game/characters/player/ev_10.rpy:15
translate vietnamese main_10_48373a16:

    # "[kiara] appears before you."
    "[kiara] đến ngay trước mặt bạn."

# game/characters/player/ev_10.rpy:16
translate vietnamese main_10_d591b1fc:

    # kiara "..."
    kiara "..."

# game/characters/player/ev_10.rpy:17
translate vietnamese main_10_0550ca6a:

    # me "[kiara]? Everything alright? What happened?"
    me "[kiara]? Mọi thứ ổn chứ? Có chuyện gì sao?"

# game/characters/player/ev_10.rpy:20
translate vietnamese main_10_56d7e266:

    # kiara "You're a troublemaker indeed.."
    kiara "Anh đúng là một kẻ chuyên đem rắc rối đến đấy..."

# game/characters/player/ev_10.rpy:21
translate vietnamese main_10_1f877649:

    # me "What do you mean?"
    me "Là sao?"

# game/characters/player/ev_10.rpy:24
translate vietnamese main_10_a7c360d3:

    # "She doesn't respond, instead she keeps looking at your chest."
    "Cô ấy không nói gì, thay vào đó cô ấy nhìn chằm chằm vào ngực bạn."

# game/characters/player/ev_10.rpy:25
translate vietnamese main_10_9b6cbb87:

    # kiara "It's been a long time.. Aharon.."
    kiara "Lâu rồi không gặp... Aharon..."

# game/characters/player/ev_10.rpy:26
translate vietnamese main_10_a6efcc57:

    # th "Aharon?!"
    th "Aharon?!"

# game/characters/player/ev_10.rpy:27
translate vietnamese main_10_44206426:

    # aharon "You're still as stupid as you were back then."
    aharon "Ngươi vẫn ngu ngốc như ngày xưa."

# game/characters/player/ev_10.rpy:28
translate vietnamese main_10_d591b1fc_1:

    # kiara "..."
    kiara "..."

# game/characters/player/ev_10.rpy:31
translate vietnamese main_10_d20f144f:

    # "A pair of hands slither from inside of you!"
    "Một đôi tay thò ra từ bạn!"

# game/characters/player/ev_10.rpy:33
translate vietnamese main_10_0947a521:

    # me "What the fuck is happening?!"
    me "Cái đéo gì đang diễn ra vậy?!"

# game/characters/player/ev_10.rpy:36
translate vietnamese main_10_84fb9c44:

    # "You try to move, yet something seems to be binding you entirely."
    "Bạn cố gắng di chuyển, nhưng một cái gì đó dường như đang giữ bạn lại."

# game/characters/player/ev_10.rpy:37
translate vietnamese main_10_98b6ec89:

    # aharon "You still don't understand that the Eldritch Keeper will always remain only a keeper."
    aharon "Ngươi vẫn chưa hiểu rằng Thủ Hộ Eldritch sẽ luôn chỉ là một người canh giữ thôi."

# game/characters/player/ev_10.rpy:40
translate vietnamese main_10_b21ae33a:

    # kiara "You're insane, Aharon. Everything you do is filled with madness."
    kiara "Ngươi điên rồi Aharon. Mọi thứ ngươi làm đều toàn là sự điên rồ."

# game/characters/player/ev_10.rpy:43
translate vietnamese main_10_f4703699:

    # aharon "To be God is to be mad. You should understand this after becoming an Angel."
    aharon "Làm Thần là phải điên. Ngươi nên hiểu điều này sau khi trở thành một Thiên thần."

# game/characters/player/ev_10.rpy:44
translate vietnamese main_10_d591b1fc_2:

    # kiara "..."
    kiara "..."

# game/characters/player/ev_10.rpy:47
translate vietnamese main_10_13515c68:

    # aharon "It's useless to wait for It, It can't hear your prayers."
    aharon "Chờ đợi cũng chỉ vô ích thôi, nó không thể nghe thấy lời cầu nguyện của ngươi đâu."

# game/characters/player/ev_10.rpy:48
translate vietnamese main_10_d591b1fc_3:

    # kiara "..."
    kiara "..."

# game/characters/player/ev_10.rpy:49
translate vietnamese main_10_26041395:

    # kiara "You aren't here to deal with me, or I would be a puppet already, so what do you want."
    kiara "Ngươi không ở đây để đối phó với ta, hoặc ta sẽ là một con rối, vậy ngươi muốn gì."

# game/characters/player/ev_10.rpy:50
translate vietnamese main_10_fd605157:

    # aharon "You'll help me locate the Whispers of the Cosmos."
    aharon "Ngươi sẽ giúp ta xác định vị trí Lời thì thầm của vũ trụ."

# game/characters/player/ev_10.rpy:53
translate vietnamese main_10_78928ca5:

    # kiara "You're insane! We'll both lose control on the spot!"
    kiara "Mày điên rồi! Cả hai chúng ta sẽ mất kiểm soát ngay tại chỗ!"

# game/characters/player/ev_10.rpy:54
translate vietnamese main_10_adaaface:

    # aharon "Its blessing will save us from damnation."
    aharon "Phước lành của nó sẽ cứu chúng ta khỏi khổ kiếp."

# game/characters/player/ev_10.rpy:55
translate vietnamese main_10_0813bec6:

    # kiara "You sick lunatic, your god doesn't exist!"
    kiara "Đồ điên, Thần của ngươi không tồn tại đâu!"

# game/characters/player/ev_10.rpy:56
translate vietnamese main_10_066b166a:

    # kiara "It has not once responded to you for all these millennia. Why do you still keep your faith in It?"
    kiara "Nó chưa một lần đáp lại ngươi trong suốt hàng thiên niên kỷ. Tại sao ngươi vẫn giữ niềm tin của mình vào Nó?"

# game/characters/player/ev_10.rpy:57
translate vietnamese main_10_c9ad8a7b:

    # kiara "You're a King of Angels, how can you be so blind and naive?"
    kiara "Ngươi là Vua của thiên thần, sao ngươi có thể mù quáng và ngây thơ như vậy?"

# game/characters/player/ev_10.rpy:58
translate vietnamese main_10_88c27c64:

    # aharon "Just because God doesn't speak to the worms, doesn't mean It doesn't exist."
    aharon "Chỉ vì Chúa không nói chuyện với mấy con sâu, không có nghĩa là Nó không tồn tại."

# game/characters/player/ev_10.rpy:61
translate vietnamese main_10_461790cf:

    # kiara "You're thoroughly insane.."
    kiara "Ngươi hoàn toàn mất trí rồi..."

# game/characters/player/ev_10.rpy:62
translate vietnamese main_10_b2c8b27d:

    # aharon "Let's go."
    aharon "Đi nào."

# game/characters/player/ev_10.rpy:63
translate vietnamese main_10_8e94f749:

    # kiara "And if I refuse?"
    kiara "Và nếu ta từ chối?"

# game/characters/player/ev_10.rpy:64
translate vietnamese main_10_5645acbe:

    # aharon "I'll turn you into a puppet and use your abilities to find it."
    aharon "Ta sẽ biến cô thành một con rối và sử dụng khả năng của cô để tìm ra nó."

# game/characters/player/ev_10.rpy:67
translate vietnamese main_10_542e5da7:

    # kiara "... Very well.."
    kiara "... Tốt thôi..."

# game/characters/player/ev_10.rpy:70
translate vietnamese main_10_2f580e85:

    # me "[kiara]?"
    me "[kiara]?"

# game/characters/player/ev_10.rpy:73
translate vietnamese main_10_f96a35ec:

    # "Everything seems to return to normal."
    "Mọi thứ dường như trở lại bình thường."

# game/characters/player/ev_10.rpy:76
translate vietnamese main_10_3b96c802:

    # "Your legs weak, you have fallen to the ground."
    "Chân bạn yếu đi, bạn đã ngã xuống đất."

# game/characters/player/ev_10.rpy:77
translate vietnamese main_10_eed3f1d3:

    # th "What the fuck was that?!"
    th "Cái gì vừa xảy ra vậy?!"

# game/characters/player/ev_10.rpy:79
translate vietnamese main_10_bcdc56f0:

    # kiara "I'll send you back. I hope we have a chance to meet again, but next time come alone.."
    kiara "Tôi sẽ gửi anh trở lại. Tôi hy vọng chúng ta có cơ hội gặp lại nhau, nhưng lần sau hãy đến một mình..."

# game/characters/player/ev_10.rpy:80
translate vietnamese main_10_4989fe05:

    # me "I.."
    me "T..."

# game/characters/player/ev_10.rpy:83
translate vietnamese main_10_a6fcb974:

    # "You didn't finish as you appeared back in your room."
    "Bạn đã không thể nói hết khi bạn xuất hiện trở lại phòng của mình."

# game/characters/player/ev_10.rpy:84
translate vietnamese main_10_3820771a:

    # me "..."
    me "..."

# game/characters/player/ev_10.rpy:87
translate vietnamese main_10_506616c6:

    # th "Fuck!! Was He inside of me the whole time?!"
    th "Đĩ mẹ nó!! Thằng đấy ở trong người mình suốt thời gian qua sao?!"

# game/characters/player/ev_10.rpy:90
translate vietnamese main_10_1c65dd79:

    # th "Fuck!! Was he inside of me the whole time?!"
    th "Đĩ mẹ nó!! Thằng đấy ở trong người mình suốt thời gian qua sao?!"

# game/characters/player/ev_10.rpy:92
translate vietnamese main_10_3820771a_1:

    # me "..."
    me "..."

# game/characters/player/ev_10.rpy:94
translate vietnamese main_10_3820771a_2:

    # me "..."
    me "..."

# game/characters/player/ev_10.rpy:97
translate vietnamese main_10_3820771a_3:

    # me "..."
    me "..."

# game/characters/player/ev_10.rpy:98
translate vietnamese main_10_8341e13d:

    # me "I hope you'll be alright [kiara].. I'm so sorry.."
    me "Tôi hy vọng cô sẽ ổn [kiara]... Tôi rất xin lỗi..."

# game/characters/player/ev_10.rpy:99
translate vietnamese main_10_485d5cda:

    # me "I wanted to become powerful, to protect those dear to me, but I've brought harm to a friend again.."
    me "Mình muốn trở nên mạnh mẽ, để bảo vệ những người thân yêu của mình, nhưng mình lại gây hại cho bạn mình... lần nữa..."

# game/characters/player/ev_10.rpy:100
translate vietnamese main_10_3211a260:

    # me "I'm.. pathetic.."
    me "Thật... thảm hại..."

# game/characters/player/ev_10.rpy:101
translate vietnamese main_10_be7aa58b:

    # me "..."
    me "..."
