# : Translation updated at 2021-12-24 07:09

# game/characters/player/ev_4.rpy:5
translate vietnamese main_4_6ec07289:

    # "Suddenly everything turns black."
    "Đột nhiên mọi thứ chuyển sang màu đen." # TL

# game/characters/player/ev_4.rpy:10
translate vietnamese main_4_3a9dfdac:

    # "When your senses return you are standing inside a room."
    "Khi các giác quan của bạn trở lại, bạn đang đứng trong một căn phòng." # TL

# game/characters/player/ev_4.rpy:11
translate vietnamese main_4_67d4cc93:

    # th "What the heck?! Is this a library?"
    th "Cái quái gì vậy?! Đây là thư viện sao?" # TL

# game/characters/player/ev_4.rpy:17
translate vietnamese main_4_34583e30:

    # "You quickly run out of here."
    "Bạn nhanh chóng chạy ra khỏi đây." # TL

# game/characters/player/ev_4.rpy:23
translate vietnamese main_4_6ec07289_1:

    # "Suddenly everything turns black."
    "Đột nhiên mọi thứ chuyển sang màu đen." # TL

# game/characters/player/ev_4.rpy:25
translate vietnamese main_4_b7374f55:

    # "You have returned to the library."
    "Bạn đã trở lại thư viện." # TL

# game/characters/player/ev_4.rpy:26
translate vietnamese main_4_d69e2925:

    # th "It's this library again.."
    th "Lại là cái thư viện này..." # TL

# game/characters/player/ev_4.rpy:46
translate vietnamese main_4_1_1df1777a:

    # "You approach the girl."
    "Bạn tiếp cận cô gái." # TL

# game/characters/player/ev_4.rpy:48
translate vietnamese main_4_1_0c94aa99:

    # th "Well, maybe I'll find some useful information here."
    th "Có lẽ mình sẽ tìm thấy một ít thông tin hữu ích ở đây." # TL

# game/characters/player/ev_4.rpy:49
translate vietnamese main_4_1_31c3e9c4:

    # "As you walk farther in you notice a woman looking at you silently."
    "Khi bạn đi xa hơn, bạn nhận thấy một người phụ nữ đang lặng lẽ nhìn bạn." # TL

# game/characters/player/ev_4.rpy:50
translate vietnamese main_4_1_c7ff83f5:

    # me "Umm, hello. Sorry, I didn't mean to disturb you. I'm [me]."
    me "Umm, xin chào. Xin lỗi nha tôi không cố ý làm phiền cô đâu. Tôi là [me]." # TL

# game/characters/player/ev_4.rpy:53
translate vietnamese main_4_1_b69c207f:

    # kiara "Welcome to the Asthar Library guest, my name is Kiara, I'm the librarian here."
    kiara "Chào mừng đến với Thư viện của Asthar, tên tôi là Kiara, tôi là thủ thư ở đây." # TL

# game/characters/player/ev_4.rpy:54
translate vietnamese main_4_1_715a6f3b:

    # me "Do you know how I arrived here? I don't remember."
    me "Cô có biết làm thế nào tôi đến được đây không? Tôi không nhớ lắm." # TL

# game/characters/player/ev_4.rpy:56
translate vietnamese main_4_1_00a37208:

    # kiara "You seek answers that's why you arrived here."
    kiara "Anh đang tìm kiếm câu trả lời đó là lý do tại sao anh đến đây." # TL

# game/characters/player/ev_4.rpy:57
translate vietnamese main_4_1_d613c423:

    # me "I don't think I understand.."
    me "Tôi không nghĩ là tôi hiểu..." # TL

# game/characters/player/ev_4.rpy:58
translate vietnamese main_4_1_dd8e2300:

    # kiara "You will understand one day."
    kiara "Một ngày nào đó anh sẽ hiểu thôi." # TL

# game/characters/player/ev_4.rpy:59
translate vietnamese main_4_1_3b274f37:

    # me "Why did you stay here after The End?"
    me "Tại sao cô ở lại đây sau khi The End diễn ra?" # TL

# game/characters/player/ev_4.rpy:61
translate vietnamese main_4_1_12b3c5e2:

    # kiara "It's my job. The End didn't stop the need for knowledge."
    kiara "Đó là công việc của tôi. Tận Thế sẽ không thể ngăn cản được nhu cầu về kiến thức." # TL

# game/characters/player/ev_4.rpy:62
translate vietnamese main_4_1_b8ea8b09:

    # me "There are still people coming to the library now?"
    me "Bây giờ vẫn có người đến thư viện sao?" # TL

# game/characters/player/ev_4.rpy:64
translate vietnamese main_4_1_0b73c997:

    # kiara "Of course, not many, though. So what wisdom do you seek here?"
    kiara "Tất nhiên là không nhiều rồi. Thế, anh tìm kiếm tri thức nào ở đây?" # TL

# game/characters/player/ev_4.rpy:65
translate vietnamese main_4_1_eded6bd1:

    # me "How do you know I didn't come here just to scavenge what is left?"
    me "Sao cô biết tôi không đến đây chỉ để nhặt nhạnh những gì còn sót lại?" # TL

# game/characters/player/ev_4.rpy:66
translate vietnamese main_4_1_401e484b:

    # kiara "Everyone seeks answers to questions bothering them. Destiny wouldn't push you here if that wasn't the case."
    kiara "Mọi người đều tìm kiếm câu trả lời cho những câu hỏi làm phiền họ. Định mệnh sẽ không đẩy anh đến đây nếu không phải như vậy." # TL

# game/characters/player/ev_4.rpy:67
translate vietnamese main_4_1_fffd71df:

    # th "I don't know why but she gives me goosebumps, there is something weird about her that I can't pinpoint."
    th "Mình không biết tại sao nhưng cô ấy khiến mình nổi da gà, có điều gì đó kỳ lạ ở cô ấy mà mình không thể xác định chính xác." # TL

# game/characters/player/ev_4.rpy:69
translate vietnamese main_4_1_03dbb8d0:

    # kiara "So what knowledge are you after [me]? Keep in mind that every answer has its price."
    kiara "Vậy [me], anh đang theo đuổi kiến thức nào? Hãy nhớ rằng mọi câu trả lời đều có giá của nó." # TL

# game/characters/player/ev_4.rpy:70
translate vietnamese main_4_1_979f1011:

    # me "What do you want, food?"
    me "Cô muốn gì, thức ăn hả?" # TL

# game/characters/player/ev_4.rpy:71
translate vietnamese main_4_1_104143d5:

    # kiara "No, I'm not hungry, I'm lonely. I'll answer your questions, and you'll keep me company for a while."
    kiara "Không, tôi không đói, tôi cô đơn cơ. Tôi sẽ trả lời câu hỏi của anh, và anh sẽ phải ở bên tôi một lúc." # TL

# game/characters/player/ev_4.rpy:72
translate vietnamese main_4_1_baaf3156:

    # me "You want me to stay with you here? For how long?"
    me "Cô muốn tôi ở đây với cô sao? Trong bao lâu?" # TL

# game/characters/player/ev_4.rpy:74
translate vietnamese main_4_1_f6c394c0:

    # kiara "Hihi, no silly, I want to have sex with you that's all. You are free to go right after."
    kiara "Hihi, không đâu ngốc ạ, tôi muốn làm tình với anh thế thôi. Anh có thể đi sau khi xong Chuyện." # TL

# game/characters/player/ev_4.rpy:75
translate vietnamese main_4_1_c735a1d0:

    # th "Oh, so that's what she meant. Well, she is direct.."
    th "Oh, vì vậy đó là những gì cô ấy muốn nói. Chà, cô gái này thẳng tính đấy..." # TL

# game/characters/player/ev_4.rpy:83
translate vietnamese main_4_menu_6f7b3ca9:

    # kiara "It's called Asthar, which you probably know, there are many things one can learn about it, but they are mostly useless to you. What you should know is that you need to be especially careful here."
    kiara "Nó được gọi là Asthar, mà có thể anh đã biết rồi, có rất nhiều điều mà người ta có thể học về nó, nhưng chúng hầu như vô dụng đối với anh. Điều anh nên biết là anh cần phải đặc biệt cẩn thận ở đây." # TL

# game/characters/player/ev_4.rpy:84
translate vietnamese main_4_menu_5c984960:

    # me "Why is that?"
    me "Tại sao vậy?" # TL

# game/characters/player/ev_4.rpy:86
translate vietnamese main_4_menu_66cf4b6b:

    # kiara "Have you ever heard of the Cult of the Black?"
    kiara "Anh đã bao giờ nghe tới Giáo phái bóng tối chưa?" # TL

# game/characters/player/ev_4.rpy:87
translate vietnamese main_4_menu_9fad7fd9:

    # me "No, is that the name of the cult that brought about The End?"
    me "Không, nhưng mà đó có phải là tên của giáo phái đã mang đến tận thế không?" # TL

# game/characters/player/ev_4.rpy:88
translate vietnamese main_4_menu_62219bee:

    # kiara "Yes that's them, their headquarters are in the depths of Asthar."
    kiara "Đúng rồi, là chúng đấy, trụ sở chính của chúng ở sâu trong Asthar." # TL

# game/characters/player/ev_4.rpy:89
translate vietnamese main_4_menu_f4c2d150:

    # me "!!!"
    me "!!!"

# game/characters/player/ev_4.rpy:90
translate vietnamese main_4_menu_34ac3545:

    # me "How do you know this?"
    me "Làm sao mà cô biết điều này?" # TL

# game/characters/player/ev_4.rpy:92
translate vietnamese main_4_menu_fd5255f3:

    # kiara "I know many things. This is a repository of knowledge after all."
    kiara "Tôi biết rất nhiều thứ. Dù gì thì đây cũng là một kho kiến thức mà." # TL

# game/characters/player/ev_4.rpy:93
translate vietnamese main_4_menu_00a0dde2:

    # me "Where is this city anyway. I don't remember any city on Earth going by this name, and this city doesn't appear small."
    me "Thành phố này nằm ở đâu vậy. Tôi không nhớ có thành phố nào trên Trái đất có tên này, và thành phố này có vẻ cũng không nhỏ." # TL

# game/characters/player/ev_4.rpy:94
translate vietnamese main_4_menu_e7d15dd0:

    # kiara "It's certainly not small, in fact Asthar is enormous beyond your imagination."
    kiara "Nó chắc chắn không hề nhỏ, thực tế thì Asthar to lớn ngoài sức tưởng tượng của anh đấy." # TL

# game/characters/player/ev_4.rpy:95
translate vietnamese main_4_menu_2ed54c30:

    # me "Then which country is it in and how come I have never heard of such an enormous city?" id main_4_menu_2ed54c30
    me "Vậy thì nó ở đất nước nào và tại sao tôi chưa bao giờ nghe nói về một thành phố khổng lồ như vậy?" id main_4_menu_2ed54c30 # TL

# game/characters/player/ev_4.rpy:96
translate vietnamese main_4_menu_f478aaf8:

    # kiara "It's in the country that it needs to be."
    kiara "Nó ở trong quốc gia mà nó cần ở." # TL

# game/characters/player/ev_4.rpy:97
translate vietnamese main_4_menu_b02400af:

    # me "What does that mean?"
    me "Là sao?" # TL

# game/characters/player/ev_4.rpy:98
translate vietnamese main_4_menu_55baf065:

    # kiara "What is real is not necessarily real, what is illusory is not necessarily illusory."
    kiara "Những gì có thật chưa chắc đã có thật, những gì là hư ảo chưa chắc đã là hão huyền." # TL

# game/characters/player/ev_4.rpy:99
translate vietnamese main_4_menu_314086b3:

    # me "What?"
    me "Cái gì?" # TL

# game/characters/player/ev_4.rpy:100
translate vietnamese main_4_menu_5143f82f:

    # th "And here I thought that she seemed pretty normal.."
    th "Và mình từng nghĩ rằng cô ta có vẻ khá bình thường đấy..." # TL

# game/characters/player/ev_4.rpy:101
translate vietnamese main_4_menu_ba05c98e:

    # kiara "This city isn't in a fixed place. It doesn't exist in a conventional way known to you."
    kiara "Thành phố này không ở một nơi cố định. Nó không tồn tại theo cách thông thường mà anh biết." # TL

# game/characters/player/ev_4.rpy:102
translate vietnamese main_4_menu_811cd256:

    # me "How does it exist then?"
    me "Làm thế nào nó tồn tại vậy?" # TL

# game/characters/player/ev_4.rpy:104
translate vietnamese main_4_menu_af192ca9:

    # kiara "There are things beyond your understanding in this world, the human mind can't possibly grasp or imagine the power and means of Gods."
    kiara "Trên thế giới này có những thứ ngoài tầm hiểu biết của anh, trí óc con người không thể nắm bắt hay tưởng tượng được sức mạnh và ý nghĩ của Thần." # TL

# game/characters/player/ev_4.rpy:105
translate vietnamese main_4_menu_8dcf3a2d:

    # th "Are there really gods in this world?"
    th "Trên thế giới này thực sự có thần sao?" # TL

# game/characters/player/ev_4.rpy:107
translate vietnamese main_4_menu_e46d8ad9:

    # kiara "What else do you want to know?"
    kiara "Anh còn muốn biết gì nữa không?" # TL

# game/characters/player/ev_4.rpy:113
translate vietnamese main_4_menu_cc7f662a:

    # kiara "The Cult of the Black first traces its history to ancient times. Thousands of years before the current era."
    kiara "Giấu vết đầu tiên của Giáo Phái Bóng Đêm là từ thời cổ đại. Hàng ngàn năm trước thời đại hiện tại." # TL

# game/characters/player/ev_4.rpy:114
translate vietnamese main_4_menu_bfaeecb0:

    # me "So old!? How come they have remained unnoticed until now?"
    me "Lâu đến thế sao!? Tại sao chúng vẫn không được chú ý cho đến bây giờ?" # TL

# game/characters/player/ev_4.rpy:116
translate vietnamese main_4_menu_9be4998a:

    # kiara "You define them in standards of mortals which is wrong, to say at least."
    kiara "Anh định nghĩa chúng theo tiêu chuẩn của người phàm là sai, nó nghiêm trọng hơn nhiều." # TL

# game/characters/player/ev_4.rpy:117
translate vietnamese main_4_menu_4a80b66d:

    # me "Are they immortal then?"
    me "Vậy chúng có bất tử không?" # TL

# game/characters/player/ev_4.rpy:119
translate vietnamese main_4_menu_ff74fea5:

    # kiara "The upper echelon is."
    kiara "Các thượng cấp thì có đấy." # TL

# game/characters/player/ev_4.rpy:120
translate vietnamese main_4_menu_58fbf695:

    # me "How many are there?"
    me "Chúng có bao nhiêu người?" # TL

# game/characters/player/ev_4.rpy:122
translate vietnamese main_4_menu_916751ef:

    # kiara "The upper echelon of Cult of the Black consists of seven Elders, two High Elders and a leader, His name is Aharon. You don't want to meet any of Them. Were you to hear Their name somewhere I advice you to just run and pray to whichever God you believe in that They ignore someone calling Their name."
    kiara "Thượng cấp của Giáo Phái Bóng Đêm bao gồm bảy Trưởng lão, hai Hiền Nhân và một người lãnh đạo, Tên của hắn là Aharon. Anh không muốn gặp bất kỳ ai trong số Họ đâu. Anh có nghe thấy tên của họ ở đâu đó không, tôi khuyên anh chỉ nên chạy và cầu nguyện với bất cứ vị thần nào mà anh tin tưởng. Chúng sẽ Làm lơ người đang gọi tên của họ." # TL

# game/characters/player/ev_4.rpy:123
translate vietnamese main_4_menu_a26ceb89:

    # me "Didn't you just say their leader's name?"
    me "Không phải cô vừa nói tên của thủ lĩnh chúng sao?" # TL

# game/characters/player/ev_4.rpy:126
translate vietnamese main_4_menu_4b6a71b1:

    # kiara "This place is outside of the perception of Gods."
    kiara "Nơi này nằm ngoài nhận thức của các vị thần." # TL

# game/characters/player/ev_4.rpy:127
translate vietnamese main_4_menu_fd418994:

    # me "Gods? You're trying to say that he is a God?"
    me "Thần? Có phải cô muốn nói rằng hắn là một vị thần sao?" # TL

# game/characters/player/ev_4.rpy:129
translate vietnamese main_4_menu_cc552c7b:

    # kiara "He is not."
    kiara "Hắn không phải đâu." # TL

# game/characters/player/ev_4.rpy:130
translate vietnamese main_4_menu_8ff1ec88:

    # me "Then?"
    me "Vậy?" # TL

# game/characters/player/ev_4.rpy:132
translate vietnamese main_4_menu_0facc63e:

    # kiara "He is close to being one and for a mortal like you He is not different from one. Were you to say His name outside you wouldn't even know how you died."
    kiara "Hắn gần chạm đến mức đấy rồi và đối với một người phàm như anh, Hắn không khác mấy đâu. Nhưng nếu anh nói tên của Hắn ở bên ngoài thì anh thậm chí sẽ không biết bản thân mình đã chết như thế nào đâu." # TL

# game/characters/player/ev_4.rpy:133
translate vietnamese main_4_menu_492c475c:

    # th "Mortal like me? Is she implying that she isn't mortal?"
    th "Người phàm như mình? Có phải là đang ngụ ý rằng cô ấy không phải là người hả?" # TL

# game/characters/player/ev_4.rpy:134
translate vietnamese main_4_menu_71ba3eea:

    # me "Why did you tell me his name then?"
    me "Tại sao có lại cho tôi biết tên của ông ta?" # TL

# game/characters/player/ev_4.rpy:135
translate vietnamese main_4_menu_143cbba3:

    # kiara "I have a feeling that you would need this knowledge in the future."
    kiara "Tôi có cảm giác rằng anh sẽ cần đến kiến thức này trong tương lai." # TL

# game/characters/player/ev_4.rpy:140
translate vietnamese main_4_menu_529095fa:

    # kiara "They are animals and people, at least they once were."
    kiara "Chúng là động vật và con người, ít nhất chúng đã từng." # TL

# game/characters/player/ev_4.rpy:141
translate vietnamese main_4_menu_493c71c5:

    # "Hearing this answer makes your breathing rushed."
    "Nghe câu trả lời này khiến bạn thở gấp." # TL

# game/characters/player/ev_4.rpy:142
translate vietnamese main_4_menu_b0bb157a:

    # me "What!? What happened to them?!"
    me "Cái Gì!? Chuyện gì đã xảy ra với họ vậy?!" # TL

# game/characters/player/ev_4.rpy:144
translate vietnamese main_4_menu_82fb4c8f:

    # kiara "They were corrupted."
    kiara "Họ đã bị tha hóa." # TL

# game/characters/player/ev_4.rpy:145
translate vietnamese main_4_menu_7f4687b6:

    # me "Corrupted? What if I were to become corrupted?"
    me "Tha hóa? Điều gì sẽ xảy ra nếu tôi trở nên tha hóa?" # TL

# game/characters/player/ev_4.rpy:147
translate vietnamese main_4_menu_33c5a28e:

    # kiara "You would become a monster just like them."
    kiara "Anh sẽ trở thành một con quái vật giống như họ." # TL

# game/characters/player/ev_4.rpy:148
translate vietnamese main_4_menu_3104829e:

    # me "How can I stop this?"
    me "Làm thế nào tôi có thể ngăn chặn điều này?" # TL

# game/characters/player/ev_4.rpy:150
translate vietnamese main_4_menu_5a9d50ea:

    # kiara "The best option would be to find beings with abilities from domains of light or mind."
    kiara "Lựa chọn tốt nhất sẽ là tìm những thực thể với năng lực từ các lĩnh vực của ánh sáng hoặc tâm trí." # TL

# game/characters/player/ev_4.rpy:151
translate vietnamese main_4_menu_d5ee42d8:

    # me "What are those domains?"
    me "Những lĩnh vực đó là gì?" # TL

# game/characters/player/ev_4.rpy:152
translate vietnamese main_4_menu_9e5409c3:

    # kiara "You can understand them as a group of extraordinary powers and abilities belonging to similar authorities."
    kiara "Anh có thể hiểu chúng là một nhóm những sức mạnh và khả năng phi thường thuộc về những thế lực có khả năng tương tự." # TL

# game/characters/player/ev_4.rpy:153
translate vietnamese main_4_menu_788d1a7d:

    # me "Something like superpowers?"
    me "Như kiểu siêu năng lực á?" # TL

# game/characters/player/ev_4.rpy:155
translate vietnamese main_4_menu_ca4021c3:

    # kiara "Yes, but they are not from nothing, for most beings everything has its cause and effect. These abilities are fueled by its user's spirituality."
    kiara "Đúng, nhưng chúng không từ hư vô, đối với hầu hết sinh vật, mọi thứ đều có nhân quả của nó. Những khả năng này được thúc đẩy bởi năng lượng tâm linh của người sử dụng nó." # TL

# game/characters/player/ev_4.rpy:156
translate vietnamese main_4_menu_f2c1bddb:

    # me "So this spirituality is mana and those extraordinary beings are mages?"
    me "Vậy cái tâm linh này là mana và những sinh vật phi thường đó là pháp sư?" # TL

# game/characters/player/ev_4.rpy:158
translate vietnamese main_4_menu_e514ddbb:

    # kiara "Basically yes, extraordinary beings can be understood like eldritch versions of mages, but there are great differences between these beings. Some are more similar to mages, some to warriors, and some, like the ones from the light domain, resemble priests."
    kiara "Về cơ bản là vậy, những sinh vật phi thường có thể được hiểu giống như phiên bản kinh dị của các pháp sư, nhưng có sự khác biệt lớn giữa những sinh vật này. Một số giống với pháp sư hơn, một số giống chiến binh và một số, giống như người từ lĩnh vực ánh sáng, cũng giống với các linh mục." # TL

# game/characters/player/ev_4.rpy:159
translate vietnamese main_4_menu_48d0efdf:

    # me "So these domains are like classes in RPG games?"
    me "Thế các lĩnh vực này giống như các class trong trò chơi RPG á?" # TL

# game/characters/player/ev_4.rpy:161
translate vietnamese main_4_menu_f4360272:

    # kiara "I think you played too many games.."
    kiara "Tôi nghĩ là anh chơi Black Souls quá 180 phút rồi..." # TL

# game/characters/player/ev_4.rpy:162
translate vietnamese main_4_menu_3820771a:

    # me "..."
    me "..."

# game/characters/player/ev_4.rpy:165
translate vietnamese main_4_menu_ecba2327:

    # kiara "But yes, they are exactly like classes in games just that they are much weirder and scarier."
    kiara "Nhưng đúng vậy, chúng giống hệt như các lớp trong trò chơi chỉ khác là chúng kỳ lạ và đáng sợ hơn nhiều." # TL

# game/characters/player/ev_4.rpy:166
translate vietnamese main_4_menu_5816593b:

    # th "So is the world now like a game?"
    th "Thế giới bây giờ giống như một trò chơi sao?" # TL

# game/characters/player/ev_4.rpy:171
translate vietnamese main_4_menu_897c3f32:

    # kiara "Haha, I have lived for a long time and I have seen a lot during my life."
    kiara "Haha, tôi đã sống trong một thời gian dài và tôi đã nhìn thấy rất nhiều trong cuộc sống của mình." # TL

# game/characters/player/ev_4.rpy:172
translate vietnamese main_4_menu_365c7ea3:

    # th "You look like you're in your twenties at most.."
    th "Trông khác gì 9x không..." # TL

# game/characters/player/ev_4.rpy:173
translate vietnamese main_4_menu_35bd5ae7:

    # me "How old are you exactly?"
    me "Chính xác là cô bao nhiêu tuổi rồi?" # TL

# game/characters/player/ev_4.rpy:175
translate vietnamese main_4_menu_c1339fe7:

    # kiara "Don’t you know it’s impolite to seek a lady’s age?" id main_4_menu_c1339fe7
    kiara "Anh có biết là rất bất lịch sự khi hỏi tuổi của một quý cô không?" id main_4_menu_c1339fe7 # TL

# game/characters/player/ev_4.rpy:176
translate vietnamese main_4_menu_53f0f7eb:

    # "She doesn't want to answer."
    "Cô ấy không muốn trả lời." # TL

# game/characters/player/ev_4.rpy:181
translate vietnamese main_4_menu_4933cce9:

    # "Your question brings a look of absolute terror to her face."
    "Câu hỏi của bạn khiến cô ấy trở nên kinh hãi." # TL

# game/characters/player/ev_4.rpy:183
translate vietnamese main_4_menu_964d6e62:

    # kiara "Get out! Now!"
    kiara "Ra ngoài! Nhanh!" # TL

# game/characters/player/ev_4.rpy:184
translate vietnamese main_4_menu_62034552:

    # me "Wha.."
    me "Cá..." # TL

# game/characters/player/ev_4.rpy:186
translate vietnamese main_4_menu_9eaeb4f7:

    # kiara "Get out!!"
    kiara "Ra ngoài!!" # TL

# game/characters/player/ev_4.rpy:187
translate vietnamese main_4_menu_da9b368a:

    # "Her change shocks you greatly but you aren't given time to think about it as she throws you out."
    "Sự thay đổi của cô ấy khiến bạn rất sốc nhưng bạn không có thời gian để nghĩ về điều đó khi cô ấy ném bạn ra ngoài." # TL

# game/characters/player/ev_4.rpy:188
translate vietnamese main_4_menu_dd9d6582:

    # me "What's going on?"
    me "Chuyện gì đang diễn ra vậy?" # TL

# game/characters/player/ev_4.rpy:190
translate vietnamese main_4_menu_9f1c2665:

    # "She doesn't answer you and instead throws you outside of the door, you can't resist. You can hear the sound of a key being inserted and twisted."
    "Cô ấy không trả lời bạn và thay vào đó ném bạn ra ngoài cửa, bạn không thể chống cự. Bạn có thể nghe thấy âm thanh của chìa được đút vào và vặn." # TL

# game/characters/player/ev_4.rpy:191
translate vietnamese main_4_menu_b2f1e464:

    # th "What the hell was that?"
    th "Cái gì vậy vậy trời?" # TL

# game/characters/player/ev_4.rpy:193
translate vietnamese main_4_menu_aaac3fcf:

    # "You have appeared in the middle of the road you were on before arriving at the library."
    "Bạn đã xuất hiện ở giữa con đường bạn đang đi trước khi đến thư viện." # TL

# game/characters/player/ev_4.rpy:194
translate vietnamese main_4_menu_3820771a_1:

    # me "..."
    me "..."

# game/characters/player/ev_4.rpy:196
translate vietnamese main_4_menu_6407aadd:

    # "Without other options you turn around and go back."
    "Không còn lựa chọn nào khác, bạn quay lưng đi về." # TL

# game/characters/player/ev_4.rpy:204
translate vietnamese main_4_sex_bda60694:

    # kiara "It seems that I'm not the only lonely person here, not that I mind."
    kiara "Có vẻ như tôi không phải là người cô đơn duy nhất ở đây, tôi cũng không thấy phiền đâu." # TL

# game/characters/player/ev_4.rpy:206
translate vietnamese main_4_sex_15d69067:

    # "[kiara] immediately pulls up her skirt, lies down on the desk, and pulls aside her panties."
    "[kiara] ngay lập tức kéo váy lên, nằm xuống bàn và kéo quần lót sang một bên." # TL

# game/characters/player/ev_4.rpy:207
translate vietnamese main_4_sex_883b8895:

    # kiara "What are you waiting for, just come and fuck me already."
    kiara "Còn chần chừ gì nữa, vào việc luôn đi." # TL

# game/characters/player/ev_4.rpy:209
translate vietnamese main_4_sex_2390cd0f:

    # "Seeing she's dripping wet, you don't hesitate pushing your cock into her!"
    "Nhìn cô ấy đang ướt đẫm, bạn không ngần ngại đẩy con cặc của mình vào cô ấy!" # TL

# game/characters/player/ev_4.rpy:211
translate vietnamese main_4_sex_eafba839:

    # th "Shit, she is so wet that it went in without any resistance at all!"
    th "Mẹ kiếp, cô ấy ướt đến nỗi nó đi vào mà không có chút khó khăn nào!" # TL

# game/characters/player/ev_4.rpy:212
translate vietnamese main_4_sex_a1387e2a:

    # kiara "Yes.. Harder! Fuck me harder!"
    kiara "Đúng rồi... Mạnh hơn! Địt tôi mạnh hơn đi!" # TL

# game/characters/player/ev_4.rpy:213
translate vietnamese main_4_sex_706cd899:

    # me "You don't need to ask twice."
    me "Không cần phải nói đến hai lần đâu." # TL

# game/characters/player/ev_4.rpy:220
translate vietnamese main_4_menu1_6fa12dc9:

    # kiara "Mmmnnn~"
    kiara "Mmmnnn~" # TL # Victorius - AI

# game/characters/player/ev_4.rpy:221
translate vietnamese main_4_menu1_a1387e2a:

    # kiara "Yes.. Harder! Fuck me harder!"
    kiara "Đúng rồi... Mạnh hơn! Địt tôi mạnh hơn đi!" # TL

# game/characters/player/ev_4.rpy:222
translate vietnamese main_4_menu1_706cd899:

    # me "You don't need to ask twice."
    me "Không cần phải nói đến hai lần đâu." # TL

# game/characters/player/ev_4.rpy:229
translate vietnamese main_4_menu1_6fa12dc9_1:

    # kiara "Mmmnnn~"
    kiara "Mmmnnn~" # TL # Victorius - AI

# game/characters/player/ev_4.rpy:230
translate vietnamese main_4_menu1_a1387e2a_1:

    # kiara "Yes.. Harder! Fuck me harder!"
    kiara "Đúng rồi... Mạnh hơn! Địt tôi mạnh hơn đi!" # TL

# game/characters/player/ev_4.rpy:231
translate vietnamese main_4_menu1_706cd899_1:

    # me "You don't need to ask twice."
    me "Không cần phải nói đến hai lần đâu." # TL

# game/characters/player/ev_4.rpy:239
translate vietnamese main_4_menu1_6fa12dc9_2:

    # kiara "Mmmnnn~"
    kiara "Mmmnnn~" # TL # Victorius - AI

# game/characters/player/ev_4.rpy:240
translate vietnamese main_4_menu1_a1387e2a_2:

    # kiara "Yes.. Harder! Fuck me harder!"
    kiara "Đúng rồi... Mạnh hơn! Địt tôi mạnh hơn đi!" # TL

# game/characters/player/ev_4.rpy:241
translate vietnamese main_4_menu1_706cd899_2:

    # me "You don't need to ask twice."
    me "Không cần phải nói đến hai lần đâu." # TL

# game/characters/player/ev_4.rpy:248
translate vietnamese main_4_sex1_966847d6:

    # "You increase your speed making her scream in pleasure."
    "Bạn tăng tốc khiến cô ấy hét lên sung sướng." # TL

# game/characters/player/ev_4.rpy:249
translate vietnamese main_4_sex1_625c2c87:

    # kiara "Ahh! More! Fuck me more!"
    kiara "Ahh! Nữa đi! Địt tôi nữa đi!" # TL

# game/characters/player/ev_4.rpy:250
translate vietnamese main_4_sex1_17ad2901:

    # me "You are one insatiable bitch aren't you?"
    me "Cô là một con điếm vô độ có phải không?" # TL

# game/characters/player/ev_4.rpy:251
translate vietnamese main_4_sex1_7372a713:

    # kiara "Yes!! I'm your bitch so fuck me! Fuck me!"
    kiara "Đúng vậy!! Tôi là con đĩ của anh nên đụ tôi đi! Đụ em đi!" # TL

# game/characters/player/ev_4.rpy:252
translate vietnamese main_4_sex1_91a733d1:

    # "Kiara lost herself completely in lust, and you are no different."
    "Kiara đã hoàn toàn đánh mất bản thân trong dục vọng, và bạn cũng không khác gì." # TL

# game/characters/player/ev_4.rpy:259
translate vietnamese main_4_menu2_625c2c87:

    # kiara "Ahh! More! Fuck me more!"
    kiara "Ahh! Nữa! Địt tôi nữa đi!" # TL

# game/characters/player/ev_4.rpy:266
translate vietnamese main_4_menu2_625c2c87_1:

    # kiara "Ahh! More! Fuck me more!"
    kiara "Ahh! Nữa! Địt tôi nữa đi!" # TL

# game/characters/player/ev_4.rpy:272
translate vietnamese main_4_sex2_6f102118:

    # me "Ngh! Fuck I'll cum in no time!"
    me "Ngh! Tôi sắp ra rồi!" # TL

# game/characters/player/ev_4.rpy:273
translate vietnamese main_4_sex2_be39421d:

    # kiara "Mmmmnn~ Please fill me with your seed! Make it drip from my pussy!"
    kiara "Mmmmnn~ Hãy lấp đầy em với hạt giống của anh! Làm cho nó chảy ra từ âm hộ của em điii!" # TL

# game/characters/player/ev_4.rpy:274
translate vietnamese main_4_sex2_61c4033d:

    # "Her begging takes you over the edge!"
    "Sự cầu xin của cô ấy khiến bạn đến cực khoái!" # TL

# game/characters/player/ev_4.rpy:276
translate vietnamese main_4_sex2_2f3d23c6:

    # "You are cumming!"
    "Bạn đang xuất tinh!" # TL

# game/characters/player/ev_4.rpy:285
translate vietnamese main_4_sex2_7be1ec02:

    # kiara "Hahaha, sooo good."
    kiara "Hahaha, thậttt tuyệttt." # TL

# game/characters/player/ev_4.rpy:286
translate vietnamese main_4_sex2_c3799345:

    # "You feel drained but satisfied. You two take a while to recover and clothe yourselves."
    "Bạn cảm thấy kiệt sức nhưng khá hài lòng. Cả hai phải mất một lúc để hồi phục và mặc quần áo của mình." # TL

# game/characters/player/ev_4.rpy:288
translate vietnamese main_4_sex2_40e4a716:

    # kiara "Now then, what are your questions?"
    kiara "Còn bây giờ thì, câu hỏi của anh là gì?" # TL

translate vietnamese strings:

    # game/characters/player/ev_4.rpy:78
    old "How about I pay right now?"
    new "Tôi thanh toán luôn lúc này thì sao nào?" # TL

    # game/characters/player/ev_4.rpy:78
    old "Tell me about this city"
    new "Kể cho tôi nghe về thành phố này" # TL

    # game/characters/player/ev_4.rpy:78
    old "Tell me about the Cult"
    new "Kể cho tôi nghe về Giáo phái" # TL

    # game/characters/player/ev_4.rpy:78
    old "What are those monsters outside?"
    new "Những con quái vật bên ngoài là gì?" # TL

    # game/characters/player/ev_4.rpy:78
    old "How do you know about these things?"
    new "Làm thế nào để cô biết về những điều này?" # TL

    # game/characters/player/ev_4.rpy:78
    old "Have you ever heard of Throdog R'luhhor?"
    new "Cô đã bao giờ nghe đến Throdog R'luhhor chưa?" # TL
# : Translation updated at 2022-02-06 08:10

translate vietnamese strings:

    # game/characters/player/ev_4.rpy:52
    old "She's the librarian of the Asthar Library."
    new "Cô ấy là thủ thư của Thư viện Asthar." # TL

    # game/characters/player/ev_4.rpy:103
    old "Asthar"
    new "Asthar" # TL # Victorius - AI

    # game/characters/player/ev_4.rpy:103
    old "Asthar is a city enormous beyond imagination, and also home to the headquarters of the Cult of The Black. What is curious is that it doesn't exist in a fixed place. When I asked Kiara about details, she said humans can't possibly grasp or imagine the power and means of Gods."
    new "Asthar là một thành phố to lớn ngoài sức tưởng tượng, và cũng là nơi đặt trụ sở chính của Giáo phái Bóng Đêm. Điều gây tò mò là nó không tồn tại ở một nơi cố định. Khi tôi hỏi Kiara về chi tiết, cô ấy nói rằng con người không thể nắm bắt hoặc tưởng tượng được sức mạnh và phương tiện của các vị thần.." # TL

    # game/characters/player/ev_4.rpy:121
    old "This cult made The End descend. It is shepherded by seven Elders, two High Elders and Aharon, the cult leader. All of them immortal and powerful beyond all imagination."
    new "Giáo phái này đã mang tận thế đến đây. Nó được dẫn dắt bởi bảy trưởng lão, hai Hiền Nhân và Aharon, người đứng đầu giáo phái. Tất cả chúng đều bất tử và mạnh mẽ ngoài sức tưởng tượng." # TL

    # game/characters/player/ev_4.rpy:125
    old "The library is outside of Gods perception."
    new "Thư viện nằm ngoài nhận thức của các vị thần." # TL

    # game/characters/player/ev_4.rpy:131
    old "He's close to being a God, saying His name is enough to draw His attention."
    new "Hắn gần như là một vị thần, nói tên của Hắn là đủ để thu hút sự chú ý của Hắn." # TL

    # game/characters/player/ev_4.rpy:143
    old "Monsters are in fact humans and animals that were corrupted."
    new "Quái vật thực chất là con người và động vật đã bị sa đọa." # TL

    # game/characters/player/ev_4.rpy:164
    old "Domains"
    new "Lĩnh vực" # TL

    # game/characters/player/ev_4.rpy:164
    old "Domains can be understood as a group of extraordinary powers and abilities belonging to similar authorities. Those abilities are fueled by its user spirituality."
    new "Lĩnh vực có thể được hiểu là một nhóm những sức mạnh và khả năng phi thường thuộc về những thế lực có khả năng tương tự. Những khả năng đó được thúc đẩy bởi năng lượng tâm linh người dùng." # TL

    # game/characters/player/ev_4.rpy:195
    old "The mere mention of Throdog R'luhhor caused her to drown in terror and throw me out immediately."
    new "Chỉ cần nhắc đến Throdog R'luhhor đã khiến cô ấy chết chìm trong nỗi kinh hoàng và ném tôi ra ngoài ngay lập tức." # TL
# : Translation updated at 2022-04-17 12:12

translate vietnamese strings:

    # game/characters/player/ev_4.rpy:12
    old "Enter"
    new "Đi vào" # TL
