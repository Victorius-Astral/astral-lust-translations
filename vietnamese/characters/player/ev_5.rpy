# : Translation updated at 2021-12-24 07:09

# game/characters/player/ev_5.rpy:6
translate vietnamese main_5_99b949dd:

    # "*whispers"
    "*thì thầm" # TL

# game/characters/player/ev_5.rpy:7
translate vietnamese main_5_29d68390:

    # "As you hear some faint voices whispering into your ears you slowly wake up."
    "Khi bạn nghe thấy một số giọng nói yếu ớt thì thầm vào tai bạn, bạn từ từ thức dậy." # TL

# game/characters/player/ev_5.rpy:9
translate vietnamese main_5_9ecc2b18:

    # "You open your eyes only to find yourself standing among the stars yet again."
    "Bạn mở mắt ra chỉ để thấy bản thân đang đứng giữa những vì sao một lần nữa." # TL

# game/characters/player/ev_5.rpy:10
translate vietnamese main_5_ab634556:

    # th "It always feels so real.."
    th "Nó luôn luôn cảm thấy rất thật..." # TL

# game/characters/player/ev_5.rpy:11
translate vietnamese main_5_99b949dd_1:

    # "*whispers"
    "*thì thầm" # TL

# game/characters/player/ev_5.rpy:13
translate vietnamese main_5_1300f80e:

    # throdog "We meet once again, mortal. Like a moth flying to the flames.."
    throdog "Chúng ta gặp lại nhau lần nữa rồi nhân loại. Như một con thiêu thân lao vào ngọn lửa..." # TL

# game/characters/player/ev_5.rpy:14
translate vietnamese main_5_d5f5c695:

    # "You hear It's voice again, it hurts just like before, this pain is not something one can get accustomed to."
    "Bạn nghe thấy giọng nói của Nó một lần nữa, nó vẫn đau như trước đây, nỗi đau này không phải là thứ mà người ta có thể quen được." # TL

# game/characters/player/ev_5.rpy:15
translate vietnamese main_5_e269123d:

    # th "Fuck!"
    th "Mẹ nó!" # TL

# game/characters/player/ev_5.rpy:16
translate vietnamese main_5_44e7d744:

    # me "Why do you speak to me?"
    me "Tại sao ông lại nói chuyện với tôi?" # TL

# game/characters/player/ev_5.rpy:18
translate vietnamese main_5_bb08ed3f:

    # throdog "You wished to speak with Us. You have simply been unaware until now."
    throdog "Ngươi muốn nói chuyện với chúng ta. Ngươi chỉ đơn giản là không biết cho đến bây giờ thôi." # TL

# game/characters/player/ev_5.rpy:19
translate vietnamese main_5_9f6fb829:

    # me "Why would I want to speak with you? Whenever you speak I feel like I'm breaking apart!"
    me "Tại sao tôi muốn nói chuyện với mấy người chứ? Bất cứ khi nào ông nói, tôi cảm thấy như mình đang bị vỡ ra thành từng mảnh vậy!" # TL

# game/characters/player/ev_5.rpy:21
translate vietnamese main_5_f735234f:

    # throdog "Your curiosity is stronger than your fear human. You wish to know so much yet you are able to understand so little."
    throdog "Sự tò mò của ngươi mạnh hơn nỗi sợ hãi của con người. Ngươi muốn biết rất nhiều nhưng ngươi có thể hiểu rất ít." # TL

# game/characters/player/ev_5.rpy:22
translate vietnamese main_5_5a8e9ba0:

    # "Its voice makes you shudder, but you can't deny it."
    "Giọng nó khiến bạn rùng mình nhưng không thể phủ nhận được." # TL

# game/characters/player/ev_5.rpy:23
translate vietnamese main_5_40b36cf5:

    # me "Why is the world like this? Is this because of the Outer Gods?"
    me "Tại sao thế giới lại như thế này? Đây có phải là do các vị thần bên ngoài không?" # TL

# game/characters/player/ev_5.rpy:25
translate vietnamese main_5_5f578893:

    # throdog "Everything that happens is but a game among higher beings. Your kind is yet another insignificant piece on this cosmic board."
    throdog "Mọi thứ xảy ra chỉ là một trò chơi giữa những thực thể cao hơn. Loài của ngươi chỉ là một hạt cát trong cái vũ trụ này thôi." # TL

# game/characters/player/ev_5.rpy:26
translate vietnamese main_5_215d6eef:

    # me "Why are gods so cruel? Don't our lives matter to them at all?"
    me "Tại sao các vị thần lại độc ác như vậy? Cuộc sống của bọn ta không quan trọng với họ chút nào sao?" # TL

# game/characters/player/ev_5.rpy:28
translate vietnamese main_5_4dc18287:

    # throdog "Hahaha!"
    throdog "Hahaha!" # TL # Victorius - AI

# game/characters/player/ev_5.rpy:29
translate vietnamese main_5_743333cd:

    # "Its laughter whips your mind, making you bend down in pain."
    "Tiếng cười của nó quất vào tâm trí bạn, khiến bạn cúi xuống vì đau." # TL

# game/characters/player/ev_5.rpy:35
translate vietnamese main_5_a974e9e6:

    # throdog "To gods mortals remain nothing more than ants crawling beneath their feet, insignificant worms without any consequence or meaning. Toys designed to be played with until broken."
    throdog "Đối với các vị thần, con người chỉ là những con kiến bò dưới chân họ, những con sâu tầm thường mà không có bất kỳ giá trị hay ý nghĩa nào. Một món đồ chơi được thiết kế để chơi cho đến khi hỏng." # TL

# game/characters/player/ev_5.rpy:36
translate vietnamese main_5_130627a5:

    # me "Shouldn't gods be merciful?"
    me "không phải Thần nên nhân từ sao?" # TL

# game/characters/player/ev_5.rpy:42
translate vietnamese main_5_8abe19d7:

    # throdog "Thou shalt never believe in god's love as gods represent no love, they represent authority and power."
    throdog "Ngươi đừng bao giờ tin vào tình yêu của thần vì thần tượng trưng cho sự vô tâm, họ tượng trưng cho quyền uy và sức mạnh." # TL

# game/characters/player/ev_5.rpy:43
translate vietnamese main_5_4ed90f1f:

    # me "Are we really so insignificant? What about nuclear weapons? What about all the progress humans have been making for thousands of years?"
    me "Bọn tôi có thực sự tầm thường như vậy không? Còn vũ khí hạt nhân thì sao? Còn về tất cả những tiến bộ mà con người đã đạt được trong hàng nghìn năm?" # TL

# game/characters/player/ev_5.rpy:45
translate vietnamese main_5_4221f7c0:

    # throdog "It's time for you to wake up lest your mind shatters."
    throdog "Đã đến lúc phải dậy rồi không khéo tâm trí ngươi sẽ tan nát luôn đấy." # TL

# game/characters/player/ev_5.rpy:47
translate vietnamese main_5_82b7127a:

    # "It doesn't answer and instead ends the dream. You feel a terrifying force pull you out of the space."
    "Nó không trả lời và thay vào đó kết thúc giấc mơ. Bạn cảm thấy một sức mạnh khủng khiếp kéo bạn ra khỏi không gian." # TL

translate vietnamese strings:

    # game/characters/player/ev_5.rpy:31
    old "Gods"
    new "Thần" # TL

    # game/characters/player/ev_5.rpy:31
    old "\"To gods mortals remain nothing more than ants crawling beneath their feet, insignificant worms without any consequence or meaning. Toys designed to be played until broken.\""
    new "\'Đối với các vị thần, con người chỉ là những con kiến bò dưới chân họ, những con sâu tầm thường mà không có bất kỳ giá trị hay ý nghĩa nào. Đồ chơi được thiết kế để chơi cho đến khi bị hỏng.\'" # TL

    # game/characters/player/ev_5.rpy:38
    old "\"Thou shalt never believe in god's love as gods represent no love, they represent authority and power.\""
    new "\'Ngươi đừng bao giờ tin vào tình yêu của thần vì thần tượng trưng cho không có tình yêu, mà tượng trưng cho quyền uy và sức mạnh.\'" # TL
