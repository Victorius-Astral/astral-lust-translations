# TODO: Translation updated at 2021-12-24 07:09

# game/characters/player/ev_6.rpy:4
translate vietnamese main_6_6ec07289:

    # "Suddenly everything turns black."
    "Đột nhiên mọi thứ chuyển sang màu đen."

# game/characters/player/ev_6.rpy:6
translate vietnamese main_6_b7374f55:

    # "You have returned to the library."
    "Bạn đã trở lại thư viện."

# game/characters/player/ev_6.rpy:7
translate vietnamese main_6_d69e2925:

    # th "It's this library again.."
    th "Lại là cái thư viện này..."

# game/characters/player/ev_6.rpy:18
translate vietnamese main_6_part2_963853f5:

    # "As you walk deeper in, [kiara] turns towards you."
    "Khi bạn đi vào sâu hơn, [kiara] quay về phía bạn."

# game/characters/player/ev_6.rpy:19
translate vietnamese main_6_part2_d8a4609a:

    # kiara "It's you again."
    kiara "Lại là anh à."

# game/characters/player/ev_6.rpy:20
translate vietnamese main_6_part2_450d9247:

    # th "It seems she isn't pleased to see me again."
    th "Có vẻ như cô ấy không vui khi gặp lại mình."

# game/characters/player/ev_6.rpy:26
translate vietnamese main_6_part2_4f216c1e:

    # kiara "You, you are the problem. If you want to die, don't bring me down with you."
    kiara "Anh, anh là vấn đề. anh bạn muốn chết, đừng kéo tôi xuống với anh."

# game/characters/player/ev_6.rpy:32
translate vietnamese main_6_part2_f718d2dd:

    # kiara "You should be, you could have killed us both."
    kiara "Anh nên như vậy, anh có thể đã giết cả hai chúng ta đấy."

# game/characters/player/ev_6.rpy:36
translate vietnamese main_6_part3_de69d1b4:

    # me "How come?"
    me "Nguy hiểm đến vạy sao?"

# game/characters/player/ev_6.rpy:38
translate vietnamese main_6_part3_9bdf44c2:

    # kiara "You almost caused potent beings to turn their gazes towards us. And They wouldn't leave us living."
    kiara "Anh suýt nữa đã khiến những thực thể mạnh mẽ phải hướng ánh nhìn về phía chúng ta. Và Họ sẽ không để chúng ta sống đạu."

# game/characters/player/ev_6.rpy:39
translate vietnamese main_6_part3_e484881f:

    # me "Didn't I just say someone's name?"
    me "Không phải tôi chỉ nói tên của một người nào đó thôi sao?"

# game/characters/player/ev_6.rpy:40
translate vietnamese main_6_part3_574bde44:

    # kiara "In mysticism names have heavy meaning. They are beacons through which higher beings connect with the Spiritual World."
    kiara "Trong huyền học, những cái tên có ý nghĩa tương đối nặng nề. Chúng là những ngọn hải đăng mà thông qua đó những thực thể cao hơn kết nối với Thế giới Tâm linh."

# game/characters/player/ev_6.rpy:41
translate vietnamese main_6_part3_15f165e1:

    # me "What does this mean?"
    me "Điều đó có nghĩa là gì?"

# game/characters/player/ev_6.rpy:43
translate vietnamese main_6_part3_b0cec30f:

    # kiara "You interact with Their spirituality when saying Their names. This connection allows Them to hear, find and respond to your call. To make it simpler, it's like calling Them on the phone while sharing your GPS location."
    kiara "Anh tương tác với năng lượng tâm linh của họ khi nói tên họ. Kết nối này cho phép Họ nghe, tìm và trả lời cuộc gọi của anh. Để đơn giản hơn, nó giống như gọi cho Họ trên điện thoại trong khi chia sẻ vị trí GPS của anh vậy."

# game/characters/player/ev_6.rpy:44
translate vietnamese main_6_part3_5bf149b6:

    # me "So it would be able to respond to me?"
    me "Vì vậy, nó sẽ có thể phản hồi cho tôi?"

# game/characters/player/ev_6.rpy:46
translate vietnamese main_6_part3_74dcbd7d:

    # kiara "Yes."
    kiara "Đúng rồi."

# game/characters/player/ev_6.rpy:47
translate vietnamese main_6_part3_6ca24395:

    # me "So you think I almost killed us because I called his name?"
    me "Thế cô nghĩ rằng tôi gần như giết cả hai vì tôi gọi tên hắn ta?"

# game/characters/player/ev_6.rpy:49
translate vietnamese main_6_part3_595ea02e:

    # kiara "Not his but Theirs. What you said is not a name, but rather a level, a status."
    kiara "Không phải hắn ấy mà là Họ. Những gì anh nói không chỉ là một cái tên, mà là một cấp độ, một thân phận."

# game/characters/player/ev_6.rpy:50
translate vietnamese main_6_part3_191c0a2f:

    # th "Well, he indeed said that just knowing his name would kill me."
    th "Ừ thì ông ta cũng nói là chỉ cần biết tên cũng đủ để giết mình."

# game/characters/player/ev_6.rpy:51
translate vietnamese main_6_part3_efb088e7:

    # me "That's not a name? Then what is the problem with me saying it?"
    me "Đó không phải là một cái tên? Thế có vấn đề gì khi tôi nói nó ra?"

# game/characters/player/ev_6.rpy:54
translate vietnamese main_6_part3_a5a6a244:

    # kiara "\"They\" would all hear this, Their gazes would turn to this place and we would perish."
    kiara "\"Họ\" sẽ nghe thấy tất cả điều này, ánh mắt của Họ sẽ hướng về nơi này và chúng ta sẽ chết."

# game/characters/player/ev_6.rpy:55
translate vietnamese main_6_part3_3820771a:

    # me "..."
    me "..."

# game/characters/player/ev_6.rpy:58
translate vietnamese main_6_part3_29ea326f:

    # kiara "There are many powerful, weird Beings. For some just knowing Their name is enough to draw Their attention."
    kiara "Có rất nhiều Thực thể mạnh mẽ, kỳ lạ. Chỉ cần một vài người biết tên của họ là đủ để thu hút sự chú ý của Họ rồi."

# game/characters/player/ev_6.rpy:59
translate vietnamese main_6_part3_6c7e0568:

    # me "You're kidding me right?"
    me "Cô đang đùa tôi đúng không?"

# game/characters/player/ev_6.rpy:61
translate vietnamese main_6_part3_859f068a:

    # kiara "Hihi, no. Curiosity can be much more dangerous than ignorance."
    kiara "Hihi, không. Sự tò mò có thể nguy hiểm hơn nhiều so với sự thiếu hiểu biết."

# game/characters/player/ev_6.rpy:62
translate vietnamese main_6_part3_3820771a_1:

    # me "..."
    me "..."

# game/characters/player/ev_6.rpy:64
translate vietnamese main_6_part3_fd782195:

    # kiara "It seems we are fated."
    kiara "Có vẻ như chúng ta đã được định trước rồi."

# game/characters/player/ev_6.rpy:65
translate vietnamese main_6_part3_0badb003:

    # kiara "I will tell you my honorary name, so you can appear here anytime." id main_6_part3_0badb003
    kiara "Tôi sẽ cho anh biết tên danh dự của mình, nên anh có thể xuất hiện ở đây bất cứ lúc nào." id main_6_part3_0badb003

# game/characters/player/ev_6.rpy:66
translate vietnamese main_6_part3_cbcd68ce:

    # me "Honorary name?"
    me "Tên danh dự?"

# game/characters/player/ev_6.rpy:68
translate vietnamese main_6_part3_2d2f6ede:

    # kiara "Yes, honorary name is a title of Higher Beings. You can use it to pray to a Higher Being. They would hear it, all honorary names need to be unique or they would point to any random Being that satisfies the description."
    kiara "Đúng rồi, tên danh dự là một danh hiệu của những Thực Thể cao hơn. Anh có thể sử dụng nó để cầu nguyện với những Thực Thể noài kia. Họ sẽ nghe thấy, tất cả các tên danh dự cần phải là duy nhất hoặc họ sẽ chỉ vào bất kỳ thực thể ngẫu nhiên nào thỏa mãn được mô tả."

# game/characters/player/ev_6.rpy:69
translate vietnamese main_6_part3_d5805db2:

    # me "So you are a higher being."
    me "Vậy cô là một thực thể sao."

# game/characters/player/ev_6.rpy:71
translate vietnamese main_6_part3_8d3289c4:

    # "She only smiles wordlessly."
    "Cô ấy chỉ cười không nói nên lời."

# game/characters/player/ev_6.rpy:73
translate vietnamese main_6_part3_49af79e3:

    # kiara "My name is \"Seeker of Wisdom, Eye of Secrets, Lorekeeper of Asthar\"."
    kiara "Tên tôi là \"Kẻ truy cầu tri thức, Con mắt của những bí mật, Asthar sử ký giả\"."

# game/characters/player/ev_6.rpy:74
translate vietnamese main_6_part3_42622049:

    # me "It indeed sounds.. higher."
    me "Nó thực sự nghe... cao hơn."

# game/characters/player/ev_6.rpy:76
translate vietnamese main_6_part3_385d634b:

    # kiara "Prepare the altar, you need.."
    kiara "Chuẩn bị bàn thờ, anh cần..."

# game/characters/player/ev_6.rpy:77
translate vietnamese main_6_part3_c0858940:

    # me "I already have an altar."
    me "Tôi đã có một cái bàn thờ rồi."

# game/characters/player/ev_6.rpy:79
translate vietnamese main_6_part3_95a70715:

    # kiara "Interesting. You know nothing about mysticism, yet you have an altar."
    kiara "Thú vị đấy. Anh không biết gì về huyền học, nhưng anh lại có một bàn thờ."

# game/characters/player/ev_6.rpy:80
translate vietnamese main_6_part3_f28cfddd:

    # me "It's not mine, it was kind of left to me."
    me "Nó không phải của tôi, mà kiểu là được bỏ lại cho tôi."

# game/characters/player/ev_6.rpy:82
translate vietnamese main_6_part3_821a63f9:

    # kiara "Oh, that explains it."
    kiara "Oh, thảo nào."

# game/characters/player/ev_6.rpy:83
translate vietnamese main_6_part3_ac75aba9:

    # kiara "So, after saying my name in the prayer, say: Open the gates to your kingdom."
    kiara "Vì vậy, sau khi nói tên tôi trong lời cầu nguyện, hãy nói: Mở ra cánh cổng đến với vương quốc của ngươi."

# game/characters/player/ev_6.rpy:85
translate vietnamese main_6_part3_6394c4fc:

    # kiara "The prayer is usually like this: Your devoted servant prays for your grace, Your devoted servant seeks your attention, Your devoted servant begs for your presence."
    kiara "Lời cầu nguyện thường như thế này: Đầy tớ tận tụy của anh cầu xin ân điển của anh, Tôi tớ tận tụy của anh tìm kiếm sự chú ý của anh, Tôi tớ tận tụy của anh cầu xin sự hiện diện của anh."

# game/characters/player/ev_6.rpy:87
translate vietnamese main_6_part3_ff532f76:

    # kiara "Have you memorized it?"
    kiara "Anh đã ghi nhớ nó chưa?"

# game/characters/player/ev_6.rpy:88
translate vietnamese main_6_part3_e60b9470:

    # me "Yes."
    me "Rồi."

# game/characters/player/ev_6.rpy:89
translate vietnamese main_6_part3_6316ce09:

    # kiara "Great, I'll send you back."
    kiara "Tốt lắm, tôi sẽ gửi an trở lại."

# game/characters/player/ev_6.rpy:90
translate vietnamese main_6_part3_6b4cfcbf:

    # kiara "Right, there is one thing. I won't hear your prayers at night."
    kiara "Phải tồi, có một thứ nữa. Tôi sẽ không nghe thấy lời cầu nguyện của anh vào ban đêm đâu."

# game/characters/player/ev_6.rpy:91
translate vietnamese main_6_part3_5c984960:

    # me "Why is that?"
    me "Why is that?"

# game/characters/player/ev_6.rpy:94
translate vietnamese main_6_part3_7686aaf2:

    # kiara "During the night, shadows come alive in the fogs of Asthar. They corrupt and distort everything, even prayers. Never try to enter the fog after the sun goes down."
    kiara "Trong đêm, bóng tối trở nên sống dậy trong sương mù của Asthar. Chúng làm tha hóa và bóp méo mọi thứ, kể cả những lời cầu nguyện. Đừng bao giờ cố đi vào sương mù sau khi mặt trời lặn."

# game/characters/player/ev_6.rpy:96
translate vietnamese main_6_part3_93d1d074:

    # kiara "Now, it's time for you to go."
    kiara "Bây giờ, đã đến lúc anh phải đi rồi."

# game/characters/player/ev_6.rpy:97
translate vietnamese main_6_part3_938544b9:

    # "You feel something pulling you backwards."
    "Bạn cảm thấy có thứ gì đó đang kéo bạn lại phía sau."

translate vietnamese strings:

    # game/characters/player/ev_6.rpy:22
    old "What's your problem?"
    new "Có chuyện gì với cô vậy?"

    # game/characters/player/ev_6.rpy:22
    old "I'm sorry for back then"
    new "Tôi xin lỗi vì hồi đó"

    # game/characters/player/ev_6.rpy:48
    old "Throdog R'luhhor is not a name, but rather a level, a status."
    new "Throdog R'luhhor không chỉ là một cái tên, mà là một cấp độ, một thân phận."

    # game/characters/player/ev_6.rpy:52
    old "\"They\""
    new "\"Họ\""

    # game/characters/player/ev_6.rpy:52
    old "Saying Throdog R'luhhor would make \"Them\" turn \"Their\" gazes toward the one calling."
    new "Nói Throdog R'luhhor sẽ làm \"Họ\" hướng \"ánh nhìn\" của Họ về phía người đang gọi."

    # game/characters/player/ev_6.rpy:57
    old "For some of \"Them\", just knowing of \"Their\" existence is enough to draw \"Their\" attention."
    new "Một vài trong sô bọn \"Họ\", chỉ cần biết sự tồn tại của \"Họ\" là đủ để kiến \"Họ\" chú ý rồi."

    # game/characters/player/ev_6.rpy:67
    old "Higher Beings"
    new "Thực Thể quyền năng"

    # game/characters/player/ev_6.rpy:67
    old "Honorary names are the titles of higher beings. One can use them to pray to a higher being."
    new "Tên danh dự là danh hiệu của những Thực Thể bậc cao hơn. Ta có thể sử dụng chúng để cầu nguyện với các thực thể."

    # game/characters/player/ev_6.rpy:72
    old "Kiara"
    new "Kiara"

    # game/characters/player/ev_6.rpy:72
    old "Her honorary name is \"Seeker of Wisdom, Eye of Secrets, Lorekeeper of Asthar\"."
    new "Tên danh dự của cô ấy là \"Kẻ truy cầu tri thức, Con mắt của những bí mật, Asthar sử ký giả\"."

    # game/characters/player/ev_6.rpy:84
    old "The prayer is usually like this: Your devoted servant prays for your grace, Your devoted servant seeks your attention, Your devoted servant begs for your presence."
    new "Lời cầu nguyện thường như vầy: Đầy tớ tận tụy của bạn cầu xin ân điển của bạn, Tôi tớ tận tụy của bạn tìm kiếm sự chú ý của bạn, Tôi tớ tận tụy của bạn cầu xin sự hiện diện của bạn."

    # game/characters/player/ev_6.rpy:93
    old "Night"
    new "Đêm"

    # game/characters/player/ev_6.rpy:93
    old "During the night, shadows come alive in the fogs of Asthar. They corrupt and distort everything, even prayers."
    new "Trong màn đêm, bóng tối trở nên sống dậy trong sương mù của Asthar. Chúng làm tha hóa và bóp méo mọi thứ, ngay cả những lời cầu nguyện."
# TODO: Translation updated at 2022-02-06 08:10

translate vietnamese strings:

    # game/characters/player/ev_6.rpy:42
    old "Names have heavy meaning. They are beacons through which higher beings connect with the spiritual world. They can hear, locate and respond to someone calling Their name."
    new "Những cái tên mang nặng ý nghĩa. Chúng là những ngọn hải đăng mà qua đó những thực thể kết nối với thế giới tâm linh. Họ có thể nghe, xác định vị trí và phản hồi khi ai đó gọi tên của họ."
