# : Translation updated at 2022-02-06 08:10

translate vietnamese strings:

    # game/variables/outfits.rpy:3
    old "School"
    new "Trường học" # TL

    # game/variables/outfits.rpy:3
    old "Night Gown"
    new "Áo ngủ" # TL

    # game/variables/outfits.rpy:3
    old "Pajamas"
    new "Đồ ngủ" # TL

    # game/variables/outfits.rpy:3
    old "Nightwear"
    new "Quần áo ngủ" # TL

    # game/variables/outfits.rpy:3
    old "Traditional"
    new "Truyền thống" # TL

    # game/variables/outfits.rpy:3
    old "Cum Stealer"
    new "Kẻ cắp tinh trùng" # TL

    # game/variables/outfits.rpy:3
    old "Magical Girl"
    new "Cô gái phép thuật" # TL

    # game/variables/outfits.rpy:3
    old "Cheongsam"
    new "Sườn xám" # TL # Victorius - AI

    # game/variables/outfits.rpy:85
    old "Affection: {}"
    new "Độ yêu mến: {}" # TL

    # game/variables/outfits.rpy:85
    old "Corruption: {}"
    new "Sa đọa: {}" # TL

    # game/variables/outfits.rpy:85
    old "Lust: {}"
    new "Ham muốn: {}" # TL

    # game/variables/outfits.rpy:91
    old "It won't provide warmth, but it'll at least cover private areas."
    new "Nó sẽ không giúp người mặc ấm hơn, nhưng ít nhất nó sẽ che được những vùng nhạy cảm." # TL

    # game/variables/outfits.rpy:91
    old "Come, daddy~ Your little girl has homework to do~"
    new "Đến đây~" # TL

    # game/variables/outfits.rpy:91
    old "It's [grace]'s job to clean the place, but [alice] is always happy to learn new stuff!"
    new "Công việc của [grace] là dọn dẹp nơi này, nhưng [alice] luôn vui vẻ khi học những thứ mới!" # TL

    # game/variables/outfits.rpy:91
    old "No one wants [lexi] to get corrupted, but I bet you would like to know how she would look with a few tentacles on her body."
    new "Không ai muốn [lexi] bị tha hóa cả, nhưng tôi cá là bạn muốn biết cô ấy sẽ trông như thế nào với vài xúc tu trên cơ thể đấy." # TL

    # game/variables/outfits.rpy:91
    old "She's an Angel. Literally. She doesn't seem to grow wings, though. Let's change that."
    new "Cô ấy là thiên thần. Theo đúng nghĩa đen. Nhưng có vẻ là cô ấy không mọc cánh. Hãy thay đổi điều đó nào." # TL

    # game/variables/outfits.rpy:91
    old "[kiara] would definitely love modern disco parties. It might be just a dream in Asthar, but you can still give her appropriate dress."
    new "[kiara] chắc chắn sẽ thích những bữa tiệc disco hiện đại. Đó có thể chỉ là một giấc mơ ở Asthar, nhưng bạn vẫn có thể chọn cho cô ấy chiếc váy phù hợp." # TL

    # game/variables/outfits.rpy:91
    old "Hundreds of years of holding back, and now she's hornier than ever."
    new "Hàng trăm năm kìm hãm, và giờ cô ấy già dặn hơn bao giờ hết." # TL
# : Translation updated at 2022-08-20 13:45

translate vietnamese strings:

    # game/variables/outfits.rpy:3
    old "Nun"
    new "Sơ" # TL

    # game/variables/outfits.rpy:92
    old "In the corruption filled Asthar, even the nuns are filled with unholy thoughts."
    new "Trong sự thối nát tràn ngập Asthar, ngay cả các nữ tu cũng chứa đầy những ý nghĩ xấu xa." # TL
# : Translation updated at 2022-09-08 19:14

translate vietnamese strings:

    # game/variables/outfits.rpy:3
    old "Bunny"
    new "Thỏ" # TL

    # game/variables/outfits.rpy:87
    old "Trust: {}"
    new "Tin tưởng: {}" # TL

    # game/variables/outfits.rpy:94
    old "Give her a carrot and make her happy."
    new "Cho cô ấy một củ cà rốt để làm cô ấy hạnh fuck." # TL
# : Translation updated at 2022-11-13 15:18

translate vietnamese strings:

    # game/variables/outfits.rpy:3
    old "Playmate"
    new "Bạn tình" # TL

    # game/variables/outfits.rpy:3
    old "Xmas"
    new "Giáng sinh" # TL

    # game/variables/outfits.rpy:3
    old "Magician"
    new "Ảo thuật gia" # TL

    # game/variables/outfits.rpy:129
    old "Had sex: {} times"
    new "Đã chịch: {} lần" # TL
