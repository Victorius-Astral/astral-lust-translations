# TODO: Translation updated at 2021-12-24 07:09

translate vietnamese strings:

    # game/variables/tips.rpy:4
    old "I have a gut feeling that I will have a good dream."
    new "Mình có cảm giác rằng mình sẽ có một giấc mơ đẹp."

    # game/variables/tips.rpy:4
    old "Who knows what awaits me outside?"
    new "Ai biết điều gì đang chờ mình bên ngoài chứ?"

    # game/variables/tips.rpy:4
    old "I need to take care of my mental health."
    new "Mình cần chăm sóc sức khỏe tinh thần của mình."

    # game/variables/tips.rpy:4
    old "Maybe someone can answer my questions out there."
    new "Có lẽ ai đó có thể trả lời câu hỏi của mình ở ngoài kia."

    # game/variables/tips.rpy:4
    old "I wonder if I would turn insane if it spoke to me again..."
    new "Không biết liệu mình có trở nên mất trí nếu nó nói chuyện với mình một lần nữa không..."

    # game/variables/tips.rpy:4
    old "My dreams are becoming more disturbing as of late."
    new "Những giấc mơ của mình ngày càng trở nên xáo trộn hơn."

    # game/variables/tips.rpy:4
    old "I should focus on my relationship with Lexi."
    new "Mình nên tập trung vào mối quan hệ của mình với Lexi."

    # game/variables/tips.rpy:4
    old "I need to ask Kiara about this... being..."
    new "Mình cần hỏi Kiara về... thực thể này..."

    # game/variables/tips.rpy:4
    old "I should wait for her to return, I hope, she's safe."
    new "Mình nên đợi cô ấy trở về, mình mong là cô ấy an toàn."

    # game/variables/tips.rpy:4
    old "More coming soon."
    new "Sắp có thêm."

    # game/variables/tips.rpy:4
    old "She might need some help."
    new "Cô ấy có thể cần trợ giúp."

    # game/variables/tips.rpy:4
    old "Maybe I should go out?"
    new "Có lẽ mình nên đi ra ngoài?"

    # game/variables/tips.rpy:4
    old "Maybe I'll find something she likes outside, should I ask her?"
    new "Có lẽ mình sẽ tìm thấy thứ gì đó cô ấy thích ở bên ngoài, mình có nên hỏi cô ấy không?"

    # game/variables/tips.rpy:4
    old "I have trouble falling asleep at night."
    new "Tôi khó ngủ vào ban đêm."

    # game/variables/tips.rpy:4
    old "I think I should hang out with Mina."
    new "Mình nghĩ tôi nên đi chơi với Mina."

    # game/variables/tips.rpy:4
    old "She seems to have something on her mind recently. I should ask."
    new "Gần đây cô ấy có vẻ có điều gì đó bận tâm. Mình nên hỏi xem."

    # game/variables/tips.rpy:4
    old "Maybe she knows another café nearby."
    new "Có lẽ cô ấy biết một quán cà phê khác gần đó."

    # game/variables/tips.rpy:4
    old "We should talk about last events."
    new "Chúng ta nên nói về những sự kiện trước."

    # game/variables/tips.rpy:4
    old "I should go out to explore."
    new "Mình nên đi ra ngoài để khám phá."

    # game/variables/tips.rpy:4
    old "Maybe I'll meet her outside."
    new "Có lẽ tôi sẽ gặp cô ấy ở ngoài."

    # game/variables/tips.rpy:4
    old "I should talk with her."
    new "Mình nên nói chuyện với cô ấy."

    # game/variables/tips.rpy:4
    old "We could go out to explore together again."
    new "Chúng ta có thể ra ngoài khám phá cùng nhau một lần nữa."

    # game/variables/tips.rpy:4
    old "Let's say hello and wait for her to find me."
    new "Hãy chào và đợi cô ấy tìm thấy mình."

    # game/variables/tips.rpy:4
    old "Something tells me that she will need me in near future."
    new "Có điều gì đó nói với mình rằng cô ấy sẽ cần mình trong tương lai gần."

    # game/variables/tips.rpy:4
    old "I should go out off my room sometimes."
    new "Thỉnh thoảng tôi mình ra khỏi phòng."

    # game/variables/tips.rpy:4
    old "When Alice likes me enough (Affection > 4, Trust > 0) I should talk with Mina and then Alice at the evening."
    new "Khi Alice đủ thích mình (Tình cảm> 4, tin tưởng > 0) mình nên nói chuyện với Mina và sau đó là Alice vào buổi tối."

    # game/variables/tips.rpy:4
    old "I wonder what's up with Lexi."
    new "Mình tự hỏi là có chuyện gì với Lexi."

    # game/variables/tips.rpy:4
    old "I should check on her."
    new "Mình nên kiểm tra cô ấy."

    # game/variables/tips.rpy:4
    old "It would be nice to accompany her a little."
    new "Sẽ rất vui nếu được đi cùng cô ấy một chút."

    # game/variables/tips.rpy:4
    old "I should give her a few days to think."
    new "Mình nên cho cô ấy một vài ngày để suy nghĩ."

    # game/variables/tips.rpy:4
    old "I should talk with Lexi."
    new "Mình nên nói chuyện với Lexi."

    # game/variables/tips.rpy:4
    old "I should hang out with her."
    new "Mình nên đi chơi với cô ấy."

    # game/variables/tips.rpy:4
    old "I kind of liked sitting with her at the sofa, we should do this again. I think she would find some time at the evening."
    new "Mình thích ngồi với cô ấy ở ghế sofa, cả hai nên làm điều này một lần nữa. Mình nghĩ cô ấy sẽ có một chút thời gian vào buổi tối."

    # game/variables/tips.rpy:4
    old "I should wait for her to settle her own thing with Alice."
    new "Mình nên đợi cô ấy giải quyết chuyện riêng của cô ấy với Alice."

    # game/variables/tips.rpy:4
    old "I bet she would want to talk about something in a few days, I should wait at my room for her."
    new "Mình cá là cô ấy sẽ muốn nói về điều gì đó trong vài ngày tới, mình nên đợi cô ấy tại phòng của mình."

    # game/variables/tips.rpy:4
    old "I should prepare before coming out of my room."
    new "Mình nên chuẩn bị trước khi ra khỏi phòng của mình."

    # game/variables/tips.rpy:4
    old "I should find her when she is in her room."
    new "Mình nên tìm cô ấy khi cô ấy ở trong phòng."

    # game/variables/tips.rpy:4
    old "I wonder what she does in her room during evenings."
    new "Mình tự hỏi cô ấy làm gì trong phòng của mình vào buổi tối."

    # game/variables/tips.rpy:4
    old "I suddenly feel like seeing her, should I check if she is at home?"
    new "Mình đột nhiên cảm thấy muốn nhìn thấy cô ấy, mình có nên kiểm tra xem cô ấy có ở nhà không?"

    # game/variables/tips.rpy:4
    old "Maybe she needs my help again."
    new "Có lẽ cô ấy cần mình giúp một lần nữa."

    # game/variables/tips.rpy:4
    old "I could use a meal, maybe Grace left something for me in the fridge."
    new "Mình đói quá, có lẽ Grace đã để lại thứ gì đó cho mình trong tủ lạnh."

    # game/variables/tips.rpy:4
    old "I wonder if she is cooking this afternoon."
    new "Không biết chiều nay cô ấy có nấu ăn không."

    # game/variables/tips.rpy:4
    old "Maybe there are some beautiful flowers outside. I'm sure she would appreciate them."
    new "Có thể có một số bông hoa đẹp bên ngoài. Mình chắc rằng cô ấy sẽ muốn nó."

    # game/variables/tips.rpy:4
    old "She probably misses her mom, I should check on her from time to time, maybe she needs me."
    new "Chắc cô ấy nhớ mẹ lắm, thỉnh thoảng mình nên đến kiểm tra cô ấy, có lẽ cô ấy cần mình."

    # game/variables/tips.rpy:4
    old "I should go and say hello."
    new "Mình nên đến và chào hỏi."

    # game/variables/tips.rpy:4
    old "I should wait and see."
    new "Mình nên chờ xem."

    # game/variables/tips.rpy:4
    old "I should go out sometimes."
    new "Thỉnh thoảng mình nên ra ngoài."

    # game/variables/tips.rpy:4
    old "Maybe I should wait \"for it\" a few days."
    new "Có lẽ mình nên \"đợi\" một vài ngày."

    # game/variables/tips.rpy:4
    old "I wonder how she deals with lust after blowing me so much."
    new "Không biết cô ấy đối phó với ham muốn như nào sau khi thổi kèn cho mình nhiều như vậy."

    # game/variables/tips.rpy:4
    old "I should train from time to time."
    new "Mình nên tập luyện liên tục."

    # game/variables/tips.rpy:4
    old "She looks down lately, I should speak with her."
    new "Cô ấy gần đây cỏ vẻ nuồn, tôi nên nói chuyện với cô ấy."

    # game/variables/tips.rpy:4
    old "She takes a shower sometimes during the evening, it might be fun if she likes me enough."
    new "Cô ấy thỉnh thoảng tắm vào buổi tối, sẽ rất vui nếu cô ấy thích tôi."

    # game/variables/tips.rpy:4
    old "I should check on her again in a while."
    new "Tôi nên kiểm tra lại cô ấy trong một thời gian."

    # game/variables/tips.rpy:4
    old "It was her first time, I should ask her how she's feeling."
    new "Đó là lần đầu tiên của cô ấy, tôi nên hỏi cô ấy cảm giác của cô ấy như thế nào."

    # game/variables/tips.rpy:4
    old "Who knows what awaits me in the morning."
    new "Ai biết được điều gì đang chờ tôi vào buổi sáng."

    # game/variables/tips.rpy:4
    old "I wonder if this wench will come to my bed again."
    new "Mình tự hỏi liệu thiếu nữ này có lên trên giường của mình nữa không."

    # game/variables/tips.rpy:4
    old "I want to have some fun with her. Playroom would be useful."
    new "Tôi muốn làm vài nháy với cô ấy. Phòng tình yêu sẽ hữu ích đấy."

    # game/variables/tips.rpy:4
    old "Who knows what awaits me outside? I should built next hotel floor first."
    new "Ai biết điều gì đang chờ tôi bên ngoài? Tôi nên xây tầng tiếp theo của khách sạn trước."

    # game/variables/tips.rpy:4
    old "I hope she stays in the hotel and recuperates. Although I have a feeling I'll meet her outside."
    new "Tôi hy vọng cô ấy ở lại khách sạn và hồi phục sức khỏe. Mặc dù tôi có cảm giác tôi sẽ gặp cô ấy ở bên ngoài."

    # game/variables/tips.rpy:4
    old "I should check on her after a few days."
    new "Tôi nên kiểm tra cô ấy sau vài ngày."

    # game/variables/tips.rpy:4
    old "Maybe a month would be enough for her leg to get better. Then, I might be able to find her bike."
    new "Có lẽ một tháng là đủ để chân của cô ấy tốt hơn. Sau đó, tôi có thể tìm thấy xe của cổ."

    # game/variables/tips.rpy:4
    old "I believe her leg would get better soon. She will let me know I guess."
    new "Tôi tin rằng chân của cô ấy sẽ sớm tốt hơn. Cô ấy sẽ cho tôi biết, chắc thế..."

    # game/variables/tips.rpy:4
    old "She left for home."
    new "Cô ấy bỏ về nhà."

    # game/variables/tips.rpy:4
    old "I hope her trip with Lexi went well."
    new "Tôi hy vọng chuyến đi của cô ấy với Lexi diễn ra tốt đẹp."

    # game/variables/tips.rpy:4
    old "If I'm lucky, a hidden treasure can turn out to be a new friend."
    new "Nếu tôi may mắn, một kho báu ẩn có thể trở thành một người bạn mới."

    # game/variables/tips.rpy:4
    old "I should find a way to build a shrine to make her move to the hotel."
    new "Tôi nên tìm cách xây điện thờ để làm cô ấy chuyển đến khách sạn..."

    # game/variables/tips.rpy:4
    old "She seems to be sulking lately I should ask her about it. Maybe Alice could help."
    new "Gần đây cô ấy có vẻ hờn dỗi, tôi nên hỏi cô ấy về điều đó. Có lẽ Alice có thể giúp."

    # game/variables/tips.rpy:4
    old "I wonder if she knows where to find more treasures. Maybe it's the time for a little adventure."
    new "Không biết cô ấy có biết tìm thêm kho báu ở đâu không. Có lẽ đã đến lúc cho một cuộc phiêu lưu nhỏ."

    # game/variables/tips.rpy:4
    old "I should help her with building the shrine."
    new "Tôi nên giúp cô ấy xây dựng ngôi đền."

    # game/variables/tips.rpy:4
    old "Rachel wants me to get magician outfit. Alice could help."
    new "Rachel muốn tôi lấy trang phục pháp sư. Alice có thể giúp."

    # game/variables/tips.rpy:4
    old "I should speak with her."
    new "Tôi nên nói chuyện với cô ấy."

    # game/variables/tips.rpy:4
    old "Perhaps only death could make her more interested in me."
    new "Có lẽ chỉ có cái chết mới khiến cô ấy quan tâm đến tôi hơn."

    # game/variables/tips.rpy:4
    old "I wonder if she has any way to help me?"
    new "Không biết cô ấy có cách nào giúp tôi không?"

    # game/variables/tips.rpy:4
    old "I should ask her about her powers."
    new "Tôi nên hỏi cô ấy về sức mạnh của cô ấy."

    # game/variables/tips.rpy:4
    old "Maybe someone will notice me if I travel often?"
    new "Có lẽ ai đó sẽ để ý đến tôi nếu tôi thường xuyên đi ra ngoài?"

    # game/variables/tips.rpy:4
    old "I should talk with her, didn't have a chance last time."
    new "Tôi nên nói chuyện với cô ấy, lần trước mình không có cơ hội làm vậy."
# TODO: Translation updated at 2022-02-06 08:10

translate vietnamese strings:

    # game/variables/tips.rpy:4
    old "I bet she would want to talk about something in a few days. I should wait at my room for her."
    new "Mình cá là cô ấy sẽ muốn nói về điều gì đó trong vài ngày tới. Mình nên đợi cô ấy ở phòng mình."

    # game/variables/tips.rpy:4
    old "She probably misses her mom. Maybe she needs me, so I should check on her from time to time."
    new "Chắc cô ấy nhớ mẹ. Có lẽ cô ấy cần tôi, mình nên kiểm tra cô ấy đều đều."

    # game/variables/tips.rpy:4
    old "She looks down lately. I should speak with her."
    new "Dạo này cô ấy xuống sắc quá. Tôi nên nói chuyện với cô ấy."

    # game/variables/tips.rpy:4
    old "She takes a shower sometimes during the evening. It might be fun if she likes me enough."
    new "Cô ấy thỉnh thoảng tắm vào buổi tối. Sẽ rất vui nếu cô ấy thích tôi."

    # game/variables/tips.rpy:4
    old "Rachel wants me to get a magician outfit. Alice could help."
    new "Rachel muốn mình có được một bộ trang phục pháp sư. Alice có thể giúp."

    # game/variables/tips.rpy:4
    old "I should ask her next day how she gets along with the girls."
    new "Mình nên hỏi cô ấy vào ngày hôm sau xem."

# TODO: Translation updated at 2022-02-18 12:36

translate vietnamese strings:

    # game/variables/tips.rpy:4
    old "I should ask her more about the extraordinary world. (Main Story at least 12)"
    new "Mình nên hỏi cô ấy nhiều hơn về thế giới thần thánh. (Cốt chuyện chính ít nhất 12)"
# TODO: Translation updated at 2022-02-19 01:56

translate vietnamese strings:

    # game/variables/tips.rpy:4
    old "I wonder if I would turn insane if it spoke to me again.."
    new "I wonder if I would turn insane if it spoke to me again.."

    # game/variables/tips.rpy:4
    old "I need to ask Kiara about this.. being.."
    new "I need to ask Kiara about this.. being.."

