# TODO: Translation updated at 2024-10-20 16:17

translate chinese_traditional strings:

    # game/screens.rpy:196
    old "CONFIRM"
    new "確認" # TL # Victorius - AI

    # game/screens.rpy:239
    old "Back"
    new "返回" # TL # Victorius - AI

    # game/screens.rpy:240
    old "History"
    new "歷史" # TL # Victorius - AI

    # game/screens.rpy:241
    old "Skip"
    new "跳過" # TL # Victorius - AI

    # game/screens.rpy:242
    old "Auto"
    new "自動" # TL # Victorius - AI

    # game/screens.rpy:243
    old "Save"
    new "保存" # TL # Victorius - AI

    # game/screens.rpy:244
    old "Q.Save"
    new "快速保存" # TL # Victorius - AI

    # game/screens.rpy:245
    old "Q.Load"
    new "快速加載" # TL # Victorius - AI

    # game/screens.rpy:246
    old "Settings"
    new "設置" # TL # Victorius - AI

    # game/screens.rpy:295
    old "Load"
    new "加載" # TL # Victorius - AI

    # game/screens.rpy:300
    old "Page {}"
    new "第 {} 頁" # TL # Victorius - AI

    # game/screens.rpy:300
    old "Automatic saves"
    new "自動保存" # TL # Victorius - AI

    # game/screens.rpy:300
    old "Quick saves"
    new "快速保存" # TL # Victorius - AI

    # game/screens.rpy:342
    old "{#file_time}%A, %B %d %Y, %H:%M"
    new "{#file_time}%%A, %%B %%d %%Y, %%H:%%M" # TL # Victorius - AI

    # game/screens.rpy:342
    old "empty slot"
    new "空槽" # TL # Victorius - AI

    # game/screens.rpy:359
    old "<"
    new "<" # TL # Victorius - AI

    # game/screens.rpy:361
    old "{#auto_page}A"
    new "{#auto_page}A" # TL # Victorius - AI

    # game/screens.rpy:364
    old "{#quick_page}Q"
    new "{#quick_page}Q" # TL # Victorius - AI

    # game/screens.rpy:370
    old ">"
    new ">" # TL # Victorius - AI

    # game/screens.rpy:424
    old "Game"
    new "遊戲" # TL # Victorius - AI

    # game/screens.rpy:425
    old "Video"
    new "視頻" # TL # Victorius - AI

    # game/screens.rpy:426
    old "Sound"
    new "聲音" # TL # Victorius - AI

    # game/screens.rpy:427
    old "Music"
    new "音樂" # TL # Victorius - AI

    # game/screens.rpy:431
    old "Mods"
    new "模組" # TL # Victorius - AI

    # game/screens.rpy:558
    old "The dialogue history is empty."
    new "對話歷史是空的。" # TL # Victorius - AI

    # game/screens.rpy:619
    old "Help"
    new "幫助" # TL # Victorius - AI

    # game/screens.rpy:628
    old "Keyboard"
    new "鍵盤" # TL # Victorius - AI

    # game/screens.rpy:629
    old "Mouse"
    new "滑鼠" # TL # Victorius - AI

    # game/screens.rpy:632
    old "Gamepad"
    new "遊戲手柄" # TL # Victorius - AI

    # game/screens.rpy:648
    old "Enter {#key}"
    new "輸入 {#key}" # TL # Victorius - AI

    # game/screens.rpy:649
    old "Advances dialogue and activates the interface."
    new "推進對話並啟動界面。" # TL # Victorius - AI

    # game/screens.rpy:652
    old "Space"
    new "空格鍵" # TL # Victorius - AI

    # game/screens.rpy:653
    old "Advances dialogue without selecting choices."
    new "推進對話而不選擇選項。" # TL # Victorius - AI

    # game/screens.rpy:656
    old "Arrow Keys"
    new "方向鍵" # TL # Victorius - AI

    # game/screens.rpy:657
    old "Navigate the interface."
    new "導航界面。" # TL # Victorius - AI

    # game/screens.rpy:660
    old "Escape {#key}"
    new "退出 {#key}" # TL # Victorius - AI

    # game/screens.rpy:661
    old "Accesses the game menu."
    new "進入遊戲選單。" # TL # Victorius - AI

    # game/screens.rpy:664
    old "Ctrl"
    new "Ctrl" # TL # Victorius - AI

    # game/screens.rpy:665
    old "Skips dialogue while held down."
    new "按住時跳過對話。" # TL # Victorius - AI

    # game/screens.rpy:668
    old "Tab"
    new "Tab" # TL # Victorius - AI

    # game/screens.rpy:669
    old "Toggles dialogue skipping."
    new "切換對話跳過。" # TL # Victorius - AI

    # game/screens.rpy:672
    old "Page Up"
    new "Page Up" # TL # Victorius - AI

    # game/screens.rpy:673
    old "Rolls back to earlier dialogue."
    new "回滾到早期對話。" # TL # Victorius - AI

    # game/screens.rpy:676
    old "Page Down"
    new "Page Down" # TL # Victorius - AI

    # game/screens.rpy:677
    old "Rolls forward to later dialogue."
    new "前進到後期對話。" # TL # Victorius - AI

    # game/screens.rpy:681
    old "Hides the user interface."
    new "隱藏用戶界面。" # TL # Victorius - AI

    # game/screens.rpy:685
    old "Takes a screenshot."
    new "截圖。" # TL # Victorius - AI

    # game/screens.rpy:689
    old "Toggles assistive {a=https://www.renpy.org/l/voicing}self-voicing{/a}."
    new "切換輔助 {a=https://www.renpy.org/l/voicing}自動語音{/a}。" # TL # Victorius - AI

    # game/screens.rpy:695
    old "Left Click"
    new "左鍵點擊" # TL # Victorius - AI

    # game/screens.rpy:699
    old "Middle Click"
    new "中鍵點擊" # TL # Victorius - AI

    # game/screens.rpy:703
    old "Right Click"
    new "右鍵點擊" # TL # Victorius - AI

    # game/screens.rpy:707
    old "Mouse Wheel Up\nClick Rollback Side"
    new "滑鼠滾輪向上\n點擊回滾側" # TL # Victorius - AI

    # game/screens.rpy:711
    old "Mouse Wheel Down"
    new "滑鼠滾輪向下" # TL # Victorius - AI

    # game/screens.rpy:718
    old "Right Trigger\nA/Bottom Button"
    new "右觸發器\nA/底部按鈕" # TL # Victorius - AI

    # game/screens.rpy:722
    old "Left Trigger\nLeft Shoulder"
    new "左觸發器\n左肩" # TL # Victorius - AI

    # game/screens.rpy:726
    old "Right Shoulder"
    new "右肩" # TL # Victorius - AI

    # game/screens.rpy:731
    old "D-Pad, Sticks"
    new "方向鍵、搖桿" # TL # Victorius - AI

    # game/screens.rpy:735
    old "Start, Guide"
    new "開始、指南" # TL # Victorius - AI

    # game/screens.rpy:739
    old "Y/Top Button"
    new "Y/頂部按鈕" # TL # Victorius - AI

    # game/screens.rpy:742
    old "Calibrate"
    new "校準" # TL # Victorius - AI

    # game/screens.rpy:790
    old "Skipping"
    new "跳過" # TL # Victorius - AI

    # game/screens.rpy:975
    old "Menu"
    new "選單" # TL # Victorius - AI

