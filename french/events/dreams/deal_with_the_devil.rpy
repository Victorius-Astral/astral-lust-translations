# TODO: Translation updated at 2021-05-12 10:31

# game/events/dreams/deal_with_the_devil.rpy:23
translate french d_Deal_with_the_Devil_3_d94f51a6:

    # "You find yourself inside your room, surrounded by a roaring fire."
    "Vous vous retrouvez dans votre chambre, entouré par un feu de cheminée."

# game/events/dreams/deal_with_the_devil.rpy:24
translate french d_Deal_with_the_Devil_3_18010e2b:

    # "Your bed is the only place not burning. On the sofa sits a dark silhouette with glowing red eyes."
    "Votre lit est le seul endroit qui ne brûle pas. Sur le canapé se trouve une silhouette sombre avec des yeux rouges brillants."

# game/events/dreams/deal_with_the_devil.rpy:27
translate french d_Deal_with_the_Devil_3_248e66e0:

    # me "Who are you?"
    me "Qui êtes-vous?"

# game/events/dreams/deal_with_the_devil.rpy:29
translate french d_Deal_with_the_Devil_3_8219b62f:

    # devil "I have many names. Some call me God, some call me Devil."
    devil "J'ai beaucoup de noms. Certains m'appellent Dieu, d'autres m'appellent Diable."

# game/events/dreams/deal_with_the_devil.rpy:30
translate french d_Deal_with_the_Devil_3_8c058f61:

    # "Its voice sounds like that of a young girl, almost playful and innocent. If you couldn't see it with your own eyes, you would think it was a child."
    "Sa voix ressemble à celle d'une petite fille, presque enjouée et innocente. si vous ne pouviez pas voir ça de vos propres yeux, vous penseriez que c'était une fille."

# game/events/dreams/deal_with_the_devil.rpy:32
translate french d_Deal_with_the_Devil_3_435f125f:

    # devil "I have an offer for you."
    devil "J'ai une offre pour toi."

# game/events/dreams/deal_with_the_devil.rpy:33
translate french d_Deal_with_the_Devil_3_86e225a7:

    # "You feel a flood of information entering your mind."
    "vous sentez qu'une avalanche d'informations entre dans votre esprit."

# game/events/dreams/deal_with_the_devil.rpy:34
translate french d_Deal_with_the_Devil_3_1d1111c9:

    # devil "So now you know my proposal, are you interested?"
    devil "Alors maintenant que vous connaissez ma proposition, êtes-vous intéressé?"

# game/events/dreams/deal_with_the_devil.rpy:52
translate french d_Deal_with_the_Devil_3_menu1_036dfd22:

    # devil "Done."
    devil "Bien."

# game/events/dreams/deal_with_the_devil.rpy:56
translate french d_Deal_with_the_Devil_3_menu1_36d49e85:

    # devil "You don't have the required price."
    devil "Vous n'avez pas le prix requis."

# game/events/dreams/deal_with_the_devil.rpy:66
translate french d_Deal_with_the_Devil_3_menu1_036dfd22_1:

    # devil "Done."
    devil "Bien."

# game/events/dreams/deal_with_the_devil.rpy:70
translate french d_Deal_with_the_Devil_3_menu1_36d49e85_1:

    # devil "You don't have the required price."
    devil "Vous n'avez pas le prix requis."

# game/events/dreams/deal_with_the_devil.rpy:77
translate french d_Deal_with_the_Devil_3_menu1_036dfd22_2:

    # devil "Done."
    devil "Bien."

# game/events/dreams/deal_with_the_devil.rpy:84
translate french d_Deal_with_the_Devil_3_menu1_036dfd22_3:

    # devil "Done."
    devil "Bien."

# game/events/dreams/deal_with_the_devil.rpy:91
translate french d_Deal_with_the_Devil_3_menu1_4d32fd8e:

    # devil "So be it."
    devil "Ainsi soit-il."

translate french strings:

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "Make me lucky [[Reset your deck and lose 40 random attribute points. Gain +2 luck, max 5 purchases]"
    new "Rendez-moi chanceux [[Redémarrez votre deck et perdez 40 points d'attributs aléatoires. Gain +2 de chance, max 5 achats]"

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "I want three powerful abilities [[Reset your deck and lose 20 random attribute points. Choose a card from three high rarity choices three times.]"
    new "Je veux trois compétences de puissances [[Redémarrez votre deck et perdez 20 points d'attributs aléatoires. Choisissez une carte de grande rareté parmi trois options, trois fois.]"

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "Remove my corruption [[-50% Sanity, -5% Corruption]"
    new "Supprimezma corruption [[-50 pour cent de santé mentale, -5 pour cent de corruption]"

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "Restore my clarity [[+100% Sanity, +5% Corruption]"
    new "Restaurez ma clarté [[+100 pour cent de santé mentale, + pour cent de corruption]"

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "Kill me [[Die]"
    new "Tuez-moi [[Mourir]"

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "I want nothing from you"
    new "Je ne veux rien de toi"

# TODO: Translation updated at 2021-05-17 12:56

# game/events/dreams/deal_with_the_devil.rpy:88
translate french d_Deal_with_the_Devil_3_menu1_3f9ebed0:

    # devil "I don't want to involve Myself into these games. But I can help you a little."
    devil "Je ne veux pas avoir à m'impliquer dans ces jeux. Mais je peux t'aider un peu."

# game/events/dreams/deal_with_the_devil.rpy:89
translate french d_Deal_with_the_Devil_3_menu1_2fc0e6f2:

    # me "How?"
    me "Comment?"

# game/events/dreams/deal_with_the_devil.rpy:90
translate french d_Deal_with_the_Devil_3_menu1_fd1dac3b:

    # devil "I'll weaken influence someone has on you."
    devil "Je vais affaiblir l'influence que quelqu'un a sur toi."

# game/events/dreams/deal_with_the_devil.rpy:91
translate french d_Deal_with_the_Devil_3_menu1_273f3d1f:

    # me "Who?!"
    me "Qui?!"

# game/events/dreams/deal_with_the_devil.rpy:92
translate french d_Deal_with_the_Devil_3_menu1_dc4cc646:

    # devil "You will know when you need to."
    devil "Tu sauras quand tu en auras besoin."

# game/events/dreams/deal_with_the_devil.rpy:93
translate french d_Deal_with_the_Devil_3_menu1_d7945924:

    # th "What the hell, someone is influencing me and I don't even know?"
    th "C'est quoi ce bordel, quelqu'un m'influence et je ne le sais même pas?"

# game/events/dreams/deal_with_the_devil.rpy:94
translate french d_Deal_with_the_Devil_3_menu1_65f639db:

    # me "What is the price?"
    me "Quel est le prix?"

# game/events/dreams/deal_with_the_devil.rpy:95
translate french d_Deal_with_the_Devil_3_menu1_ee5e4510:

    # devil "I'll take most of your sanity, and I'll take most of your abilities."
    devil "Je prendrais la plupart de ta santé mentale, et je prendrais la plupart de tes compétences."

# game/events/dreams/deal_with_the_devil.rpy:96
translate french d_Deal_with_the_Devil_3_menu1_3820771a:

    # me "..."
    me "..."

# game/events/dreams/deal_with_the_devil.rpy:105
translate french d_Deal_with_the_Devil_3_menu1_9539ab5a:

    # devil "This will give you more time. Check yourself often, as one day He might replace you if you're not careful."
    devil "Cela te donnera plus de temps. Vérifies souvent, car un jour Il pourrait te remplacer."

# game/events/dreams/deal_with_the_devil.rpy:106
translate french d_Deal_with_the_Devil_3_menu1_b5966159:

    # me "What do you mean?!"
    me "Que voulez-vous dire?!"

translate french strings:

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "I want to be a master of my own destiny"
    new "Je veux être le maître de mon propre destin"

    # game/events/dreams/deal_with_the_devil.rpy:97
    old "[[Sanity will become 1%, your deck will reset. Eldritch influence will weaken.]"
    new "[[La santé mentale reviendra à 1 pour cent, votre deck redémarrera. L'influence des Eldritch s'affaiblira.]"

    # game/events/dreams/deal_with_the_devil.rpy:97
    old "Deal"
    new "Bien"

    # game/events/dreams/deal_with_the_devil.rpy:97
    old "I won't take your offer"
    new "Je ne prendrais pas ton offre"
    
# TODO: Translation updated at 2022-01-25 22:41

translate french strings:

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "Make me lucky [[Lose 40 random attribute points. Gain +2 luck, max 5 purchases] {#visible}"
    new "Augmente ma chance [[Perdez 40 points d'attribut aléatoires. Gain +2 de chance, max 5 achats] {#visible}"

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "I want three powerful abilities [[Lose 20 random attribute points. Choose a card from three high rarity choices.]"
    new "Je veux trois capacités puissantes [[Perdez 20 points d'attribut aléatoires. Choisissez une carte parmi trois choix de rareté élevée.]"

    # game/events/dreams/deal_with_the_devil.rpy:36
    old "I want to be a master of my own destiny {#visible}"
    new "Je veux être maître de mon destin {#visible}"

    # game/events/dreams/deal_with_the_devil.rpy:74
    old "Sanity -50%\nCorruption -5%"
    new "Santé mentale -50 pour cent\nCorruption -5 pour cent"

    # game/events/dreams/deal_with_the_devil.rpy:81
    old "Sanity +100%\nCorruption +5%"
    new "Santé mentale +100 pour cent\nCorruption +5 pour cent"

    # game/events/dreams/deal_with_the_devil.rpy:95
    old "[[Sanity will become 1%, you'll lose all your cards. Eldritch influence will weaken by 4 points. Influence: [player.author_influence]/10]"
    new "[[Santé mentale va entre à 1 pour cent, vous perdrez toutes vos cartes. L'influence de l'Eldritch sera affaiblie de 4 points. Influence: [player.author_influence]/10]"
# TODO: Translation updated at 2022-11-13 15:21

# game/events/dreams/deal_with_the_devil.rpy:110
translate french d_Deal_with_the_Devil_3_menu1_75df7159:

    # devil "{evil}Hahaha!{/evil}"
    devil "{evil}Hahaha!{/evil}"

# game/events/dreams/deal_with_the_devil.rpy:111
translate french d_Deal_with_the_Devil_3_menu1_44eef04f:

    # devil "{evil}I shall grant you your will.{/evil}"
    devil "{evil}I shall grant you your will.{/evil}"

# game/events/dreams/deal_with_the_devil.rpy:118
translate french d_Deal_with_the_Devil_3_menu1_4af8bc15:

    # devil "{evil}Decadent Monarch, Scorching Nightmare, Father of Desires{/evil}"
    devil "{evil}Decadent Monarch, Scorching Nightmare, Father of Desires{/evil}"

# game/events/dreams/deal_with_the_devil.rpy:119
translate french d_Deal_with_the_Devil_3_menu1_67ed8805:

    # "Its name makes your head to throb in pain as illusory blood flows down your ears."
    "Its name makes your head to throb in pain as illusory blood flows down your ears."

# game/events/dreams/deal_with_the_devil.rpy:120
translate french d_Deal_with_the_Devil_3_menu1_6aba83f5:

    # me "Argh!"
    me "Argh!"

# game/events/dreams/deal_with_the_devil.rpy:121
translate french d_Deal_with_the_Devil_3_menu1_0110eb39:

    # devil "Haha, let this pain be the price."
    devil "Haha, let this pain be the price."

# game/events/dreams/deal_with_the_devil.rpy:122
translate french d_Deal_with_the_Devil_3_menu1_4bc02b39:

    # me "That's all?"
    me "That's all?"

# game/events/dreams/deal_with_the_devil.rpy:123
translate french d_Deal_with_the_Devil_3_menu1_9181b3a9:

    # devil "If your sacrifices please me, you'll receive my blessings. If they insult me-"
    devil "If your sacrifices please me, you'll receive my blessings. If they insult me-"

# game/events/dreams/deal_with_the_devil.rpy:124
translate french d_Deal_with_the_Devil_3_menu1_c465d04f:

    # devil "Hahaha!"
    devil "Hahaha!"

# game/events/dreams/deal_with_the_devil.rpy:125
translate french d_Deal_with_the_Devil_3_menu1_3820771a_1:

    # me "..."
    me "..."

# game/events/dreams/deal_with_the_devil.rpy:130
translate french d_Deal_with_the_Devil_3_menu1_d0e2f6d7:

    # devil "Very well."
    devil "Very well."

# game/events/dreams/deal_with_the_devil.rpy:131
translate french d_Deal_with_the_Devil_3_menu1_d424dc21:

    # devil "{evil}We shall met again. Do you agree?{/evil}"
    devil "{evil}We shall met again. Do you agree?{/evil}"

# game/events/dreams/deal_with_the_devil.rpy:132
translate french d_Deal_with_the_Devil_3_menu1_17dbf313:

    # "Its tone suddenly changed into a monstrous and evil raving, making your soul and mind shiver in horror." id d_Deal_with_the_Devil_3_menu1_17dbf313
    "Its tone suddenly changed into a monstrous and evil raving, making your soul and mind shiver in horror." id d_Deal_with_the_Devil_3_menu1_17dbf313

# game/events/dreams/deal_with_the_devil.rpy:133
translate french d_Deal_with_the_Devil_3_menu1_0e4aef8c:

    # th "There is something wrong with what 'It' said..."
    th "There is something wrong with what 'It' said..."

# game/events/dreams/deal_with_the_devil.rpy:134
translate french d_Deal_with_the_Devil_3_menu1_a2f309d6:

    # me "Yes..."
    me "Yes..."

translate french strings:

    # game/events/dreams/deal_with_the_devil.rpy:34
    old "I wish to know your honorary name, to seek your blessings in the rituals"
    new "I wish to know your honorary name, to seek your blessings in the rituals"

    # game/events/dreams/deal_with_the_devil.rpy:34
    old "Can we make the deal another time?"
    new "Can we make the deal another time?"

