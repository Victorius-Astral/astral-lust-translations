# TODO: Translation updated at 2022-06-17 16:01

# game/places/hotel/expansions/gym/vit.rpy:5
translate french gym_vit_873b9e9f:

    # "It's too late."
    "C'est trop tard." # TL

# game/places/hotel/expansions/gym/vit.rpy:16
translate french gym_vit_4a739fa7:

    # "You train your vitality." id gym_vit_4a739fa7
    "Vous entraînez votre vitalité." id gym_vit_4a739fa7 # TL

# game/places/hotel/expansions/gym/vit.rpy:21
translate french gym_vit_ac89b13d:

    # "You feel tired after training today, don't push yourself."
    "Vous vous sentez fatigué après l'entraînement d'aujourd'hui, ne vous surmenez pas." # TL

translate french strings:

    # game/places/hotel/expansions/gym/vit.rpy:2
    old "Train vitality"
    new "Entraîne votre vitalité" # TL

