# : Translation updated at 2022-01-25 22:41

translate spanish strings:

    # game/variables/classes/Inventory.rpy:32
    old "inventory"
    new "inventario" # TL

    # game/variables/classes/Inventory.rpy:149
    old "Lost all cards."
    new "Todas las cartas fueron perdidas." # TL

    # game/variables/classes/Inventory.rpy:155
    old "Lost whole content of the backpack."
    new "Todo el contenido de la mochila fue perdido." # TL

    # game/variables/classes/Inventory.rpy:163
    old "Lost all materials."
    new "Todos los materiales fueron perdidos." # TL

# : Translation updated at 2022-01-29 20:12

translate spanish strings:

    # game/variables/classes/Inventory.rpy:132
    old "Lost {}"
    new "{} perdido" # TL

    # game/variables/classes/Inventory.rpy:136
    old "Required item missing."
    new "Falta el elemento requerido." # TL
# : Translation updated at 2022-02-26 14:57

translate spanish strings:

    # game/variables/classes/Inventory.rpy:28
    old "Items obtained:"
    new "Objetos obtenidos:" # TL

    # game/variables/classes/Inventory.rpy:54
    old "\n • {} {{color={}}}{}{{/color}},"
    new "\n • {} {{color={}}}{}{{/color}}," # TL # Victorius - AI

    # game/variables/classes/Inventory.rpy:170
    old "Lost all items."
    new "Todos los objetos perdidos." # TL
# : Translation updated at 2022-03-30 13:03

translate spanish strings:

    # game/variables/classes/Inventory.rpy:71
    old "Missing {} {}"
    new "No hay {} {}" # TL
# TODO: Translation updated at 2024-10-18 07:38

translate spanish strings:

    # game/variables/classes/Inventory.rpy:33
    old "Error looting items, unknown item: {}"
    new "Error al saquear objetos, objeto desconocido: {}" # TL # Victorius - AI

