# : Translation updated at 2022-01-29 20:12

translate spanish strings:

    # game/variables/classes/Being.rpy:1116
    old "Health +{} ({}/{})"
    new "Salud +{} ({}/{})" # TL
# : Translation updated at 2022-08-16 18:44

translate spanish strings:

    # game/variables/classes/Being.rpy:1590
    old "Buff granted: {buff} {{image={image}}} ({amount})"
    new "Buff dado: {buff} {{image={image}}} ({amount})" # TL
# : Translation updated at 2022-09-23 12:55

translate spanish strings:

    # game/variables/classes/Being.rpy:1598
    old "Buff granted: {buff2} {{image={image2}}} ({amount})\nBuff removed: {buff} {{image={image}}}"
    new "Buff dado: {buff2} {{image={image2}}} ({amount})\nBuff removido: {buff} {{image={image}}}" # TL

    # game/variables/classes/Being.rpy:1603
    old "Buff reduced: {buff} {{image={image}}} ({amount})"
    new "Buff reducido: {buff} {{image={image}}} ({amount})" # TL

    # game/variables/classes/Being.rpy:1608
    old "Buff removed: {buff} {{image={image}}}"
    new "Buff removido: {buff} {{image={image}}}" # TL
# : Translation updated at 2022-10-11 16:03

translate spanish strings:

    # game/variables/classes/Being.rpy:1598
    old "Effect granted: {buff2} {{image={image2}}} ({amount})\\Effect removed: {buff} {{image={image}}}"
    new "Efecto dado: {buff2} {{image={image2}}} ({amount})\\Efecto removido: {buff} {{image={image}}}" # TL

    # game/variables/classes/Being.rpy:1603
    old "Effect reduced: {buff} {{image={image}}} ({amount})"
    new "Efecto reducido: {buff} {{image={image}}} ({amount})" # TL

    # game/variables/classes/Being.rpy:1608
    old "Effect removed: {buff} {{image={image}}}"
    new "Efecto removido: {buff} {{image={image}}}" # TL

    # game/variables/classes/Being.rpy:1627
    old "Effect granted: {buff} {{image={image}}} ({amount})"
    new "Efecto dado: {buff} {{image={image}}} ({amount})" # TL
