# : Translation updated at 2022-01-25 22:41

translate spanish strings:

    # game/variables/classes/Astral.rpy:95
    old "Insert wallpaper code"
    new "Inserte código de fondo de pantalla" # TL

    # game/variables/classes/Astral.rpy:106
    old "Code correct, unlocked wallpaper: '{}'"
    new "Código correcto, fondo de pantalla desbloqueado: '{}'" # TL

    # game/variables/classes/Astral.rpy:122
    old "Warning: Non existent wallpaper {} in {} group."
    new "Advertencia: Fondo de pantalla no existente {} en el grupo {}." # TL

    # game/variables/classes/Astral.rpy:124
    old "Code correct, unlocked group of wallpapers: '{}'"
    new "Código correcto, desbloqueado grupo de fondos de pantalla: '{}'" # TL

    # game/variables/classes/Astral.rpy:129
    old "Code doesn't match any wallpaper."
    new "El código no coincide con ningún fondo de pantalla." # TL

    # game/variables/classes/Astral.rpy:273
    old "This is the end of {who} story in {ver}, this story will be continued soon! You can check for news, and new version on my {{patreon1}}{{a=https://www.patreon.com/Victorius}}Patreon{{/a}}{{/patreon1}}."
    new "Este es el final de la historia de {who} en {ver}. ¡Esta historia continuará pronto! Puede consultar las noticias y la nueva versión en mi {{patreon1}}{{a=https://www.patreon.com/Victorius}}Patreon{{/a}}{{/patreon1}}." # TL

    # game/variables/classes/Astral.rpy:457
    old "Unique cards obtained: {}"
    new "Cartas únicas obtenidas: {}" # TL

    # game/variables/classes/Astral.rpy:461
    old "Unique card obtained: {}"
    new "Carta única obtenida: {}" # TL

    # game/variables/classes/Astral.rpy:477
    old "HELP"
    new "AYUDA" # TL
# : Translation updated at 2022-02-16 09:41

translate spanish strings:

    # game/variables/classes/Astral.rpy:311
    old "This is the end of {who} story in {ver}. This story will be continued soon! You can check for news, and new version on my {{patreon1}}{{a=https://www.patreon.com/Victorius}}Patreon{{/a}}{{/patreon1}}."
    new "Este es el final de la historia de {who} en {ver}. ¡Esta historia continuará pronto! Puedes consultar las noticias y la nueva versión en mi {{patreon1}}{{a=https://www.patreon.com/Victorius}}Patreon{{/a}}{{/patreon1}}." # TL
# : Translation updated at 2022-03-30 13:03

translate spanish strings:

    # game/variables/classes/Astral.rpy:314
    old "her"
    new "ella" # TL

    # game/variables/classes/Astral.rpy:316
    old "This is the end of {who} story in {ver}. The game remains in the active development, though! If you want to check or get new releases, or you just want to support Astral Lust check our {{patreon1}}{{a=https://www.patreon.com/Victorius}}Patreon{{/a}}{{/patreon1}}! Have fun :)"
    new "Este es el fin da la historia de {who} en la versión {ver}. Sin embargo, el juego permanece en desarrollo activo! Si desea comprobar u obtener nuevas versiones, o simplemente apoyar a Astral Lust, consulte nuestro {{patreon1}}{{a=https://www.patreon.com/Victorius}}Patreon{{/a}}{{/patreon1}}! Diviértete :)" # TL
# : Translation updated at 2022-06-15 06:51

translate spanish strings:

    # game/variables/classes/Astral.rpy:465
    old "Lost items:"
    new "Objetos perdidos:" # TL
# : Translation updated at 2022-08-16 18:44

translate spanish strings:

    # game/variables/classes/Astral.rpy:490
    old "\n • {{color={color}}}{amount}{{/color}} {{image={image}}}"
    new "\n • {{color={color}}}{amount}{{/color}} {{image={image}}}" # TL # Victorius - AI
# TODO: Translation updated at 2024-10-18 07:38

translate spanish strings:

    # game/variables/classes/Astral.rpy:233
    old "Code already claimed on this save."
    new "Código ya reclamado en esta partida guardada." # TL # Victorius - AI

    # game/variables/classes/Astral.rpy:242
    old "Code '{}' not found, try again, codes are case sensitive."
    new "Código '{}' no encontrado, inténtalo de nuevo, los códigos distinguen entre mayúsculas y minúsculas." # TL # Victorius - AI

