translate chinese strings:

    # game/variables/lists/sexpos.rpy:3
    old "Kiss - Standing"
    new "轻吻 - 站姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Kiss - Forced"
    new "轻吻 - 强吻" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Kiss - Masturbation"
    new "轻吻 - 自慰" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Kiss - Kneeling"
    new "轻吻 - 跪姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Kiss - Lying"
    new "轻吻 - 卧姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Kiss - Lesbian"
    new "轻吻 - 女同" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - Cowgirl"
    new "性交 - 女上位" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - Doggy"
    new "性交 - 后入式" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - Missionary"
    new "性交 - 正常体位" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - Pilediver"
    new "性交 - 打桩机姿势" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - On the Desk"
    new "性交 - 在桌子上" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - Reverse Pilediver"
    new "性交 - 反向打桩机姿势" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - Against the Wall"
    new "性交 - 靠墙" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - Lifting"
    new "性交 - 举高" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Anal - Doggy"
    new "肛交 - 后入式" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Anal - Cowgirl"
    new "肛交 - 女上位" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Anal - Missionary"
    new "肛交 - 正常体位" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Anal - Lifting"
    new "肛交 - 举高" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Anal - Reverse Cowgirl"
    new "肛交 - 反向女上位" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Boobjob - Sucking"
    new "乳交 - 吮吸式" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Boobjob - Lying"
    new "乳交 - 卧姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Boobjob - Kneeling"
    new "乳交 - 跪姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Boobjob - Sitting"
    new "乳交 - 坐姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Blowjob - Lying"
    new "口交 - 卧姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Blowjob - Forced"
    new "口交 - 强迫式" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Blowjob - Sitting"
    new "口交 - 坐姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Blowjob - Kneeling"
    new "口交 - 跪姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Blowjob - 69"
    new "口交 - 69 式" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Handjob - Lying"
    new "手交 - 卧姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Handjob - Licking Nipple"
    new "手交 - 舔乳头" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Handjob - Kneeling"
    new "手交 - 跪姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Handjob - Sitting"
    new "手交 - 坐姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Footjob - Standing"
    new "足交 - 站姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Footjob - Sitting"
    new "足交 - 坐姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Footjob - Sleeping"
    new "足交 - 睡姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Thighjob - Standing"
    new "腿交 - 站姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Cunnilingus - Lying"
    new "舔阴 - 卧姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Cunnilingus - Pilediver"
    new "舔阴 - 打桩机姿势" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Cunnilingus - Standing"
    new "舔阴 - 站姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Cunnilingus - Against the Wall"
    new "舔阴 - 靠墙" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Cunnilingus - Sitting"
    new "舔阴 - 坐姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Cunnilingus - 69"
    new "舔阴 - 69 式" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Cunnilingus - Cowgirl"
    new "舔阴 - 女上位" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Masturbation - Sitting"
    new "自慰 - 坐姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Masturbation - Dildo"
    new "自慰 - 假阳具" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Masturbation - Lying"
    new "自慰 - 卧姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Masturbation - Standing"
    new "自慰 - 站姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Masturbation - Against the Corner"
    new "自慰 - 靠墙角" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Massage - Lying"
    new "按摩 - 卧姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Massage - Standing"
    new "按摩 - 站姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Massage - From Behind"
    new "按摩 - 从背后" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Massage - Sitting"
    new "按摩 - 坐姿" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Massage - Breasts"
    new "按摩 - 胸部" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Massage - Pussy"
    new "按摩 - 阴部" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Massage - Lesbian"
    new "按摩 - 女同" # TL

    # game/variables/lists/sexpos.rpy:3
    old "BDSM - Wooden Horse"
    new "SM - 木马" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Group - Threesome"
    new "群交 - 3P" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Group - Threesome - Two Males"
    new "群交 - 3P - 两男一女" # TL

    # game/variables/lists/sexpos.rpy:3
    old "Group - Threesome - Two Females"
    new "群交 - 3P - 一男两女" # TL

# TODO: Translation updated at 2022-05-11 14:54

translate chinese strings:

    # game/variables/lists/sexpos.rpy:3
    old "Kiss - Sitting"
    new "接吻 - 坐着" # TL # Victorius - AI

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - Reverse Doggy"
    new "性交 - 反向后入" # TL # Victorius - AI

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - Facing each Other"
    new "性交 - 面对面" # TL # Victorius - AI

    # game/variables/lists/sexpos.rpy:3
    old "Rubbing - Cowgirl"
    new "摩擦 - 女上位" # TL # Victorius - AI

# TODO: Translation updated at 2022-06-15 06:53

translate chinese strings:

    # game/variables/lists/sexpos.rpy:3
    old "Cunnilingus - Against the Counter"
    new "口交 - 靠在柜台" # TL # Victorius - AI

# TODO: Translation updated at 2022-11-13 15:21

translate chinese strings:

    # game/variables/lists/sexpos.rpy:3
    old "Vaginal - Behind"
    new "性交 - 从后面" # TL # Victorius - AI

