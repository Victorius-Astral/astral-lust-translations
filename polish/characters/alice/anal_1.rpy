# TODO: Translation updated at 2024-10-16 04:34

# game/characters/alice/anal_1.rpy:12
translate polish alice_anal_1_4877092e:

    # "You enter her. She's incredibly tight, yet you went in without problem."
    "Wchodzisz w nią. Jest niesamowicie ciasna, ale wszedłeś bez problemu.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:17
translate polish alice_anal_1_df37bd7e:

    # alice "Mnmm~"
    alice "Mnmm~{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:18
translate polish alice_anal_1_ff41d66b:

    # me "Fuck! You are tight."
    me "Kur**! Jesteś ciasna.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:24
translate polish alice_anal_1_007f1501:

    # alice "Fuck me~ Fuck me~ I want you soooo much!"
    alice "Pieprz mnie~ Pieprz mnie~ Pragnę cię tak bardzo!{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:29
translate polish alice_anal_1_7f7259f6:

    # "You start moving harder."
    "Zaczynasz poruszać się mocniej.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:33
translate polish alice_anal_1_117a772e:

    # alice "Ahh~"
    alice "Ahh~{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:38
translate polish alice_anal_1_617fc709:

    # alice "Yeeesss~"
    alice "Taaak~{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:39
translate polish alice_anal_1_6015a7a4:

    # me "Her wild moans make your desire burn even more intensely."
    me "Jej dzikie jęki sprawiają, że twoje pożądanie płonie jeszcze intensywniej.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:40
translate polish alice_anal_1_a91a69ba:

    # me "Whose girl are you?"
    me "Czyja jesteś dziewczyną?{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:44
translate polish alice_anal_1_f8d4a858:

    # alice "Your girl! I'm yours~"
    alice "Twoją dziewczyną! Jestem twoja~{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:45
translate polish alice_anal_1_aa93b10c:

    # me "Good girl."
    me "Dobra dziewczynka.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:49
translate polish alice_anal_1_3d79359c:

    # alice "More~ Just a little more~"
    alice "Więcej~ Jeszcze trochę~{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:60
translate polish alice_anal_1_46acb026:

    # "You are cumming together with [alice]!"
    "Dochodzi razem z [alice]!{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:63
translate polish alice_anal_1_2ec231b7:

    # alice "Ahhh~"
    alice "Ahhh~{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:66
translate polish alice_anal_1_d0de85e1:

    # "You don't feel tired after coming, but [alice] appears absolutely exhausted."
    "Nie czujesz się zmęczony po dojściu, ale [alice] wygląda na całkowicie wyczerpaną.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:67
translate polish alice_anal_1_8b6f0399:

    # me "You okay?"
    me "Wszystko w porządku?{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:70
translate polish alice_anal_1_1eda25ab:

    # alice "{size=-10}Mm~.{/size}"
    alice "{size=-10}Mm~.{/size}{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:73
translate polish alice_anal_1_ae7efb49:

    # "She only groans wordlessly."
    "Tylko jęczy bez słowa.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:76
translate polish alice_anal_1_61660bd0:

    # me "[alice]?"
    me "[alice]?{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:77
translate polish alice_anal_1_eba39e74:

    # th "It seems she's fallen asleep."
    th "Wygląda na to, że zasnęła.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:80
translate polish alice_anal_1_dd017547:

    # "After clothing yourself and putting a towel on [alice] you piggyback her."
    "Po ubraniu się i położeniu ręcznika na [alice], bierzesz ją na barana.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:81
translate polish alice_anal_1_18b07e92:

    # me "I'll take you to bed."
    me "Zabiorę cię do łóżka.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:84
translate polish alice_anal_1_34cff1b3:

    # "You gently lay her down to rest."
    "Delikatnie kładziesz ją, aby odpoczęła.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:85
translate polish alice_anal_1_8d5c5cb3:

    # th "She appears so innocent right now, totally unlike her usual playful self."
    th "Wygląda teraz tak niewinnie, zupełnie inaczej niż zwykle.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:89
translate polish alice_anal_1_b109c02e:

    # "You kiss her forehead."
    "Całujesz ją w czoło.{#mtl}{#tl}"

# game/characters/alice/anal_1.rpy:90
translate polish alice_anal_1_446fd9a0:

    # me "Sleep tight my beautiful girl."
    me "Śpij dobrze, moja piękna dziewczyno.{#mtl}{#tl}"

