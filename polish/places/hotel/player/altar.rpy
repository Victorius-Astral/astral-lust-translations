# TODO: Translation updated at 2021-05-12 10:09

# game/places/hotel/player/altar.rpy:12
translate polish room_player_altar_a864fc90:

    # me "What have I wanted to say?"
    me "Co chciałem powiedzieć?{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:13
translate polish room_player_altar_0dd984d6:

    # me ".. Weird..."
    me ".. Dziwny...{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:24
translate polish room_player_altar_63569e9c:

    # "There is no respone."
    "Nie ma odpowiedzi.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:25
translate polish room_player_altar_fac33955:

    # me "Perhaps I should wait more. I hope she's alright."
    me "Może powinienem jeszcze poczekać. Mam nadzieję, że nic jej nie jest.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:29
translate polish room_player_altar_02089ca2:

    # me "Seeker of Wisdom, Eye of Secrets, Lorekeeper of Asthar."
    me "Poszukiwacz Mądrości, Oko Tajemnic, Strażnik Astharu.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:32
translate polish room_player_altar_365931bb:

    # me "Open the gates to your kingdom."
    me "Otwórz bramy swojego królestwa.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:34
translate polish room_player_altar_97991f8f:

    # "There is no response."
    "Nie ma odpowiedzi.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:45
translate polish room_player_altar_1c156d3b:

    # "You look at the altar from all sides and compliment its craftsmanship."
    "Oglądasz ołtarz ze wszystkich stron i chwalisz jego kunszt.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:46
translate polish room_player_altar_6c96b5fe:

    # "It's simply exquisite. You can't point to any flaw in it."
    "Jest po prostu wyśmienity. Nie można wskazać w nim żadnej wady.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:47
translate polish room_player_altar_04b01469:

    # "As you continue looking at it, you feel the need to fall to your knees and pray."
    "Patrząc na to, czujesz potrzebę uklęknięcia i modlitwy.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:49
translate polish room_player_altar_ff051e5d:

    # th "I wonder to whom it is dedicated."
    th "Ciekawe komu jest dedykowany.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:53
translate polish room_player_altar_38b3b4bf:

    # "You carefully check the knife. It's exquisite."
    "Ostrożnie sprawdzasz nóż. To jest wspaniałe.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:54
translate polish room_player_altar_3d8530f1:

    # th "I wonder what its purpose is."
    th "Zastanawiam się, jaki jest jego cel.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:56
translate polish room_player_altar_ec767004:

    # "For some weird reason you feel lighter."
    "Z jakiegoś dziwnego powodu czujesz się lżejszy.{#mtl}{#tl}"

translate polish strings:

    # game/places/hotel/player/altar.rpy:2
    old "Pray"
    new "Modlić się{#mtl}{#tl}"

    # game/places/hotel/player/altar.rpy:2
    old "Check altar"
    new "Sprawdź ołtarz{#mtl}{#tl}"

    # game/places/hotel/player/altar.rpy:2
    old "Check the knife"
    new "Sprawdź nóż{#mtl}{#tl}"

    # game/places/hotel/player/altar.rpy:30
    old "Open the gates to your kingdom."
    new "Otwórz bramy swojego królestwa.{#mtl}{#tl}"

# TODO: Translation updated at 2021-07-11 16:45

# game/places/hotel/player/altar.rpy:43
translate polish room_player_altar_kiara_a864fc90:

    # me "What have I wanted to say?"
    me "Co chciałem powiedzieć?{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:44
translate polish room_player_altar_kiara_0dd984d6:

    # me ".. Weird..."
    me ".. Dziwny...{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:55
translate polish room_player_altar_kiara_97991f8f:

    # "There is no response."
    "Nie ma odpowiedzi.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:56
translate polish room_player_altar_kiara_fac33955:

    # me "Perhaps I should wait more. I hope she's alright."
    me "Może powinienem jeszcze poczekać. Mam nadzieję, że nic jej nie jest.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:60
translate polish room_player_altar_kiara_02089ca2:

    # me "Seeker of Wisdom, Eye of Secrets, Lorekeeper of Asthar."
    me "Poszukiwacz Mądrości, Oko Tajemnic, Strażnik Astharu.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:61
translate polish room_player_altar_kiara_365931bb:

    # me "Open the gates to your kingdom."
    me "Otwórz bramy swojego królestwa.{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:63
translate polish room_player_altar_kiara_97991f8f_1:

    # "There is no response."
    "Nie ma odpowiedzi.{#mtl}{#tl}"

# TODO: Translation updated at 2022-01-31 15:20

# game/places/hotel/player/altar.rpy:43
translate polish room_player_altar_kiara_da7c91f7:

    # me "What did I want to say?"
    me "Co chciałem powiedzieć?{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:44
translate polish room_player_altar_kiara_3820771a:

    # me "..."
    me "...{#mtl}{#tl}"

# game/places/hotel/player/altar.rpy:45
translate polish room_player_altar_kiara_1228f336:

    # me "Weird..."
    me "Dziwny...{#mtl}{#tl}"

# TODO: Translation updated at 2022-11-13 15:23

translate polish strings:

    # game/places/hotel/player/altar.rpy:2
    old "Perform a ritual{#visible}"
    new "Wykonaj rytuał{#visible}{#mtl}{#tl}"

