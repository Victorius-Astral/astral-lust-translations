# TODO: Translation updated at 2024-10-18 07:12

translate polish strings:

    # game/updates/codex.rpy:22
    old "She brought me to watch her back when she went out to get a.. \"friend\"...."
    new "Zabrała mnie, żebym jej pilnował, kiedy wychodziła po... \'przyjaciela\'...." # TL # Victorius - AI

    # game/updates/codex.rpy:34
    old "This silly girl seems to despise men to the bone. I wonder why?"
    new "Ta głupiutka dziewczyna zdaje się nienawidzić mężczyzn do szpiku kości. Ciekawe dlaczego?" # TL # Victorius - AI

    # game/updates/codex.rpy:51
    old "I encountered a terrifying abomination on my first trip outside, it was gigantic and looked like a lion."
    new "Podczas mojej pierwszej wyprawy na zewnątrz natknąłem się na przerażającą abominację, była gigantyczna i wyglądała jak lew." # TL # Victorius - AI

