# Translation updated at 2021-07-11 16:45

translate polish strings:

    # game/screens/menus/girls/succa.rpy:17
    old "{gold}[succA]{/gold}"
    new "{gold}[succA]{/gold}{#mtl}{#tl}"

    # game/screens/menus/girls/succa.rpy:22
    old "• {lust}Critical lust: 100{/lust}"
    new "• {lust}Krytyczne pożądanie: 100{/lust}{#mtl}{#tl}"

    # game/screens/menus/girls/succa.rpy:33
    old "• BDSM: "
    new "• BDSM:{#mtl}{#tl}"

    # game/screens/menus/girls/succa.rpy:34
    old "• Orgasm Denial: "
    new "• Wstrzymujące Orgazmu:{#mtl}{#tl}"

    # game/screens/menus/girls/succa.rpy:36
    old "• Lesbian: "
    new "• Lesbijskie:{#mtl}{#tl}"
