# TODO: Translation updated at 2024-10-17 01:25

translate polish strings:

    # game/functions/modding/triggers.rpy:110
    old "One of your mods' code is broken! It's not recommended to ignore the problem.\nFault at trigger -> "
    new "Jeden z kodów twoich modów jest uszkodzony! Nie zaleca się ignorowania problemu.\nBłąd przy wyzwalaczu -> " # TL # Victorius - AI

    # game/functions/modding/triggers.rpy:110
    old "\nload order -> "
    new "\nKolejność ładowania -> " # TL # Victorius - AI

